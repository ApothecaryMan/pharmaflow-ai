...
export const IntelligenceDashboard: React.FC<IntelligenceDashboardProps> = ({ t, language }) => {
    ...
    () => [
      {
        ...
        icon: 'shopping_cart',
        permission: 'reports.view_intelligence' as const,
      },
      {
        label: t.intelligence.dashboard.tabs.financials,
        value: 'financials' as const,
        icon: 'payments',
        permission: 'reports.view_financial' as const,
      },
      {
  ...
  useEffect(() => {
    const isForbidden = !permissionsService.can(
      tabOptions.find((t) => t.value === activeTab)?.permission || 'reports.view_intelligence'
    );
    if (isForbidden) {
    ...
    switch (activeTab) {
      case 'procurement':
        return (
          <button
            onClick={() =>
              (window as any).dispatchGlobalEvent?.(
                'OPEN_PO_MODAL',
                procurement.filteredItems.map((i) => i.product_id)
              )
            }
          ...
          <div className='w-full max-w-md'>
            <SearchInput
              ...
              onUpdateFilter={(id, vals) => {
                if (id === 'period')
                  setSelectedPeriod(vals.length > 0 ? (vals[0] as FinancialPeriod) : null);
      ...
      case 'procurement':
        return (
          <ProcurementKPIs
            ...
            t={t}
            isLoading={procurement.loading && !procurement.summary}
          />
        );
      case 'risk':
      ...
      case 'financials':
        return (
          <FinancialsKPIs
            ...
            t={t}
            isLoading={financials.loading && !financials.kpis}
          />
        );
      default:
        return null;
    ...
    <div className='h-full flex flex-col overflow-hidden' dir={language === 'AR' ? 'rtl' : 'ltr'}>
      ...
      <div className='flex-1 px-page pb-page overflow-y-auto main-content-scroll'>
        {activeTab === 'procurement' && (
          <ProcurementPage t={t} language={language} {...procurement} />
        )}
        ...
        {activeTab === 'risk' && <RiskPage t={t} language={language} {...risk} />}
        {activeTab === 'audit' && (
          <AuditPage t={t} language={language} globalFilter={auditSearch} {...audit} />
        )}
      </div>
    </div>
  );
