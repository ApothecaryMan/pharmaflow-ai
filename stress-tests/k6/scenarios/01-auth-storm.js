...
export default function () {
  group('🔐 Auth Login Flow', () => {
    ...
    const verifyRes = http.get(`${SUPABASE_AUTH_URL}/user`, {
      headers: {
        'Content-Type': 'application/json',
        apikey: SUPABASE_ANON_KEY,
    ...
    const dataRes = http.get(`${SUPABASE_REST_URL}/employees?select=id,name,role&limit=10`, {
...
export function handleSummary(data) {
  ...
  console.log(
    `Fail Rate:        ${((data.metrics.auth_login_fail_rate?.values?.rate || 0) * 100).toFixed(2)}%`
  );
