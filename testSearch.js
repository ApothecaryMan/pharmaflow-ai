...
const testCases = [
  ...
  { term: 'pan', target: 'Japan', expected: false, desc: 'Middle of string (no space)' },
  {
    ...
    target: 'Japan',
    expected: false,
    desc: 'Middle of string (with space, but Japan has no word boundary)',
  },
  ...
  {
    term: ' pan adv',
    target: 'Original Panadol Advance',
    expected: true,
    desc: 'Multi-word boundary',
  },
  { term: '@para', target: 'Paracetamol', expected: true, desc: 'Ingredient start' },
  {
    ...
    target: 'Paracetamol',
    expected: false,
    desc: 'Ingredient middle (no match for start)',
  }, // Wait, "@ para" -> " para" -> \bpara. "Paracetamol" starts with P. \bPara matches.
...
testCases.forEach(({ term, target, expected, desc }) => {
  ...
  console.log(
    `[${status}] Term: '${term}' | Target: '${target}' | Expected: ${expected} | Got: ${result}`
  );
  if (status === 'FAIL') {
