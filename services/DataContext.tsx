    ...
    const batchesChannel = supabase
      ...
      .on(
        ...
        (payload: any) => {
          ...
          if (payload.eventType === 'INSERT' || payload.eventType === 'UPDATE') {
            // Map the snake_case DB payload to camelCase StockBatch object
            ...
            setBatchesState(prev => [newBatch, ...prev.filter(b => b.id !== newBatch.id)]);
          } else if (payload.eventType === 'DELETE') {
            setBatchesState((prev) => prev.filter((b) => b.id !== payload.old.id));
          }
        }
      )
      .subscribe();
