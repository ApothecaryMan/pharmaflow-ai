...
describe('financialService', () => {
  ...
  describe('getFinancialSummary', () => {
    it('uses the compute_financial_summary_with_snapshots RPC', async () => {
      ...
      expect(supabase.rpc).toHaveBeenCalledWith('compute_financial_summary_with_snapshots', expect.any(Object));
    ...
    it('falls back to client-side calculations when RPC fails', async () => {
      ...
      const spyFallback = vi.spyOn(financialService, 'fallbackFinancialSummary').mockResolvedValueOnce({
        gross_revenue: 200,
        total_refunds: 20,
        net_revenue: 180,
        gross_cogs: 100,
