...
function calculateChange(current: number, previous: number): { percent: number; direction: 'up' | 'down' | 'unchanged' } {
  if (previous === 0) {
...
export const intelligenceService = {
  ...
  getProcurementItems: async (): Promise<ProcurementItem[]> => {
    ...
    const items: ProcurementItem[] = drugs.map(drug => {
      ...
      const confidenceScore = hasRecentSales && hasConsistentSales ? 85 : hasConsistentSales ? 70 : 50;
      ...
      return {
        ...
        skip_reason: stockStatus === 'OVERSTOCK' ? 'مخزون زائد' : stockStatus === 'NORMAL' ? 'مخزون كافي' : null,
        ...
        data_quality_flag: hasConsistentSales ? 'GOOD' : velocity.last30 > 0 ? 'SPARSE' : 'NEW_PRODUCT'
      };
    });
  ...
  getExpiryRiskItems: async (): Promise<ExpiryRiskItem[]> => {
    ...
    const riskItems: ExpiryRiskItem[] = expiringBatches.map(batch => {
      ...
      let recommendedAction: 'DISCOUNT_AGGRESSIVE' | 'DISCOUNT_MODERATE' | 'MONITOR' | 'RETURN' | 'WRITE_OFF';
      ...
      return {
        ...
        product_name: drug ? getDisplayName({ name: drug.name, dosageForm: drug.dosageForm }) : 'Unknown',
  ...
  getFinancialKPIs: async (period: FinancialPeriod = 'this_month'): Promise<FinancialKPIs> => {
    ...
    const currentMargin = currentMetrics.revenue > 0 
      ? (currentMetrics.grossProfit / currentMetrics.revenue) * 100 
      : 0;
    const prevMargin = prevMetrics.revenue > 0 
      ? (prevMetrics.grossProfit / prevMetrics.revenue) * 100 
      : 0;
   ...
   */
  getProductFinancials: async (period: FinancialPeriod = 'this_month'): Promise<ProductFinancialItem[]> => {
    const allSales = await salesService.getAll();
    const drugs = await inventoryService.getAll();
    const drugMap = new Map(drugs.map(d => [d.id, d]));
    ...
    const products = Array.from(productAgg.values())
      .map(p => ({
        ...
        abc_class: classifyABC(p.revenue, Array.from(productAgg.values()).reduce((sum, x) => sum + x.revenue, 0)) as 'A' | 'B' | 'C',
    ...
    for (const sale of sales) {
      ...
      for (const item of sale.items) {
        transactions.push({
          id: `${sale.id}-${item.id}`,
          timestamp: sale.date,
          invoice_number: sale.dailyOrderNumber ? `INV-${sale.dailyOrderNumber}` : sale.id.slice(-6),
          type: 'SALE',
    ...
    for (const ret of returns) {
      ...
      for (const item of ret.items) {
        transactions.push({
          ...
          anomaly_reason: ret.reason === 'other' ? 'سبب غير محدد' : 
                          ret.reason === 'damaged' ? 'منتج تالف' : undefined
        });
      }
    }
