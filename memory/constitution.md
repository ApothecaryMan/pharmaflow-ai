...
## Article IV: Code Health & Standards

- **No Legacy Patterns**: Use modern syntax (React 18+, TypeScript).
- **PharmaFlow Standards**:
  - All text MUST be in `i18n/translations.ts` (EN + AR).
  - Use standard components (`SmartInputs`, `TanStackTable`, etc.).
  - **Forbidden**: Native `<select>`, raw `<input>`, HTML `<table>`.
  - Ensure RTL compatibility and iOS Safari button styling.
