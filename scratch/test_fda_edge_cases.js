...
async function testEdgeCases() {
  ...
  for (const tc of edgeCases) {
    ...
    try {
      ...
      console.log(`✅ ${tc.name.padEnd(40)} | interactions: ${interLen > 0 ? interLen + ' chars' : 'NONE'} | warnings: ${warnLen > 0 ? warnLen + ' chars' : 'NONE'} | contraindications: ${contraLen > 0 ? contraLen + ' chars' : 'NONE'}`);
    } catch (error) {
  ...
  console.log(`NOT found:        ${notFound}  (${Math.round(notFound/edgeCases.length*100)}%)`);
