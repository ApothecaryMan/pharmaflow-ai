...
const drugs = [
  ...
  'empagliflozin', 'baricitinib', 'amlodipine', 'carvedilol',
  'meloxicam', 'febuxostat', 'glimepiride', 'famotidine',
  'clonazepam', 'midazolam',
  // Drugs that may NOT be FDA-approved (not sold in US)
  'dapoxetine', 'piracetam', 'ethamsylate', 'trimebutine',
...
async function test() {
  ...
  for (const name of drugs) {
    ...
    try {
      ...
      if (data.error) {
        ...
        const flags = [has_inter ? 'interactions' : null, has_warn ? 'warnings' : null, has_contra ? 'contraindications' : null].filter(Boolean).join(', ');
  ...
  console.log(`\n=== RESULTS ===`);
  console.log(`Match rate: ${results.found}/${drugs.length} (${Math.round(results.found/drugs.length*100)}%)`);
