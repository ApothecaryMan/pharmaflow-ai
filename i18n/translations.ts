// @skill: translation-hunter
// This file is governed by the Translation Hunter protocol.
// 1. All keys must exist in both EN and AR.
// 2. Types are inferred from EN. Do not break the structure.

export const TRANSLATIONS = {
  EN: {
    appTitle: 'ZINC',
    common: {
      loading: 'Loading...',
      edit: 'Edit',
      save: 'Save',
      cancel: 'Cancel',
      delete: 'Delete',
      selectMonth: 'Select Month',
      close: 'Close',
      actions: 'Actions',
      am: 'AM',
      pm: 'PM',
      add: 'Add',
      reset: 'Reset',
      confirmReset: 'Confirm Reset',
      search: 'Search',
    },
    zincAcronym: {
      Z: {
        title: 'Zero Errors',
        desc: 'Ensures maximum precision in dispensing and inventory tracking to guarantee patient safety.',
      },
      I: {
        title: 'Integrated Solutions',
        desc: 'A seamless ecosystem connecting sales, purchases, and insurance claims in one place.',
      },
      N: {
        title: 'Next-Gen Innovation',
        desc: 'Empowering pharmacies with AI-driven insights and smart automation for a faster workflow.',
      },
      C: {
        title: 'Comprehensive Control',
        desc: 'Full authority over your stock, finances, and staff performance with real-time analytics.',
      },
    },
    location: {
      governorate: 'Governorate',
      city: 'City',
      area: 'Area',
      selectGovernorate: 'Select Governorate',
      selectCity: 'Select City',
      selectArea: 'Select Area',
    },
    date: 'Date',
    orderId: 'Order ID',
    orderTotal: 'Order Total',
    nav: {
      dashboard: 'Dashboard',
      inventory: 'Inventory',
      pos: 'Point of Sale',
      sales: 'Sales',
      purchase: 'Purchase',
      salesHistory: 'Sales History',
      suppliers: 'Suppliers',
      purchases: 'Purchases / Stock In',
      barcodeStudio: 'Barcode Studio',
      barcodePrinter: 'Barcode Printer',
      stockAdjustment: 'Stock Adjustment',
      stockMovement: 'Stock Movement',
      desktop: 'Desktop Integration',
    },
    menu: {
      modules: 'Modules',
      sections: 'sections',
      totalItems: 'Total Items',
      searchIn: 'Search in',
      pack: 'Pack',
      unit: 'Unit',
    },
    settings: {
      tools: 'Tools',
      currencyConverter: {
        title: 'Currency Converter',
        amount: 'Amount',
        from: 'From',
        to: 'To',
        result: 'Result',
        loading: 'Loading rates...',
        error: 'Failed to fetch rates',
      },
      calculator: {
        title: 'Calculator',
      },
      holidays: {
        title: 'Holidays & Events',
        all: 'All',
        upcoming: 'Upcoming',
        past: 'Past',
        shifted: 'Shifted',
        originalDate: 'Original Date',
        actualDate: 'Actual Date',
        daysLeft: 'days left',
        daysAgo: 'days ago',
        today: 'Today',
        tomorrow: 'Tomorrow',
        yesterday: 'Yesterday',
        ramadanSpecial: 'Ramadan Season',
        shiftedToThursday: 'Shifted to Thursday',
        nationalType: 'National Holiday',
        religiousType: 'Religious',
        observanceType: 'Observance',
        rescheduleRule: 'Thursday Shift Rule',
      },
      settings: 'Settings',
      theme: 'Theme',
      language: 'Language',
      darkMode: 'Dark Mode',
      lightMode: 'Light Mode',
      appearance: 'Appearance & Language',
      fullscreen: 'Toggle Fullscreen',
      reload: 'Reload App',
      textTransform: 'Uppercase Names',
      focusMode: 'Focus Mode',
      navigationLayout: 'Navigation Layout',
      sidebarSettings: 'Sidebar',
      navbarMenuLayout: 'Navbar Menu',
      navbarMenuSingle: 'Single',
      navbarMenuMulti: 'Multi',
      designStyle: 'Design Style',
      designStyleFull: 'Full',
      designStyleNavbar: 'Navbar',
      developerMode: 'Developer Mode',
      themesMenu: 'Theme',
      accent: 'Accent Color',
      statusBarSettings: 'Status Bar Settings',
      showNotificationBell: 'Notification Bell',
      showNotificationToast: 'Notification Toast',
      quickStatuses: 'Quick Statuses',
      showSales: 'Sales',
      showInventory: 'Inventory',
      showCustomers: 'New Customers',
      showTopSeller: 'Top Seller',
      comingSoon: 'Coming Soon',
      typography: 'Typography',
      fontEN: 'English Font',
      fontAR: 'Arabic Font',
      graphicStyle: 'Graphic Heading Style',
      borderRadius: 'Border Radius',
      radiusDefault: 'Modern',
      radiusSharp: 'Sharp',
      radiusFull: 'Soft',
      dataManagement: 'Data Management',
      exportBackup: 'Export Backup',
      importBackup: 'Import Backup',
      importWarning: 'Warning: Importing a backup will overwrite all current data. Proceed?',
      storageQuota: {
        warningTitle: 'Storage Space Warning',
        warningMessage:
          'Local storage usage is high ({{percentage}}% of {{limit}}MB used). Please clear older branches or browser cache to avoid losing data.',
        criticalTitle: 'Storage Full',
        criticalMessage:
          'Failed to save data. Local storage limit exceeded. Please free up space immediately.',
      },
      branchManagement: 'Branch Management',
      addBranch: 'Add New Branch',
      branchName: 'Branch Name',
      branchCode: 'Branch Code',
      branchAddress: 'Branch Address',
      branchPhone: 'Branch Phone',
      branchStatus: 'Status',
      assignEmployees: 'Assign Employees',
      saveBranch: 'Save Branch',
      deleteBranch: 'Delete Branch',
      deleteBranchConfirm:
        'Are you sure you want to delete this branch? This will affect all data linked to it.',
      sidebarStyle: 'Sidebar Mode',
      sidebarFixed: 'Fixed',
      sidebarMini: 'Mini',
      borderStyle: 'Border Style',
      customCss: 'Custom CSS',
      numeralSystem: 'Numeral System',
      numeralArabic: 'Arabic (١٢٣)',
      numeralLatin: 'Latin (123)',
      cardStyle: 'Card Style',
      cardThick: 'Thick',
      cardThin: 'Thin',
      cardNone: 'None',
      customCardCss: 'Custom Card CSS',
      badgeStyle: 'Badge Shape',
      badgeStyleDefault: 'Rounded',
      badgeStyleSharp: 'Sharp',
      badgeStylePill: 'Pill',
      badgeStyleSlim: 'Slim',
      modalPresentationMode: 'Modal Presentation',
      modalPresentationModeModal: 'Popup Modal',
      modalPresentationModeSidebar: 'Side Drawer',
      modalSettings: 'Modal Settings',
      modalPresentationStyle: 'Presentation Style',
      sidebarModalWidth: 'Sidebar Width',
      sidebarModalWidthNarrow: 'SM',
      sidebarModalWidthStandard: 'MD',
      sidebarModalWidthWide: 'LG',
      sidebarModalWidthExtraWide: 'XL',
      reducedMotion: 'Disable Motion',
      disableCSSTransitions: 'Disable CSS Transitions',
      backgroundPattern: 'Background Pattern',
      patternNone: 'None',
      patternDots: 'Dots',
      patternGrid: 'Grid',
      patternThickGrid: 'Thick Grid',
      patternBlueprint: 'Blueprint',
      patternRings: 'Rings',
      patternMesh: 'Mesh',
      patternNoise: 'Noise',
      patternMandala: 'Mandala',
      patternDiamond: 'Diamond',
      patternCorners: 'Corners',
      patternCross: 'Cross',
      patternStars: 'Stars',
      patternGlowingStars: 'Glowing Stars',
      patternHearts: 'Hearts',
      patternBricks: 'Bricks',
      patternAbstract: 'Skull',
      patternCircuit: 'Circuit',
      patternOrnate: 'Ornate',
      patternDiagonalStripes: 'Diagonal Stripes',
      patternOpacity: 'Pattern Opacity',
      vividBg: 'Background Mix',
      posSettings: 'POS Settings',
      defaultDeliveryFee: 'Default Delivery Fee',
      egp: 'EGP',
      active: 'Active',
      inactive: 'Inactive',
      noStreetAddress: 'No street address set',
      noLocationSet: 'No location set',
      cannotDeactivateActive: 'Cannot deactivate the currently active branch',
      saveSuccess: 'Branch data saved successfully',
      deactivated: 'Branch deactivated',
      streetAddress: 'Street Address',
      streetAddressPlaceholder: 'Building no, street name...',
      all: 'All',
      selected: 'Selected',
      noSelectedEmployees: 'No selected employees',
      noResults: 'No results found',
      saveFirstForToken: 'Save the branch first to generate an attendance token',
      current: 'Current',
      noBranchesYet: 'No branches yet',
      startAddingFirst: 'Start by adding your first branch to the organization',
      tabGeneral: 'General',
      tabEmployees: 'Employees',
      tabAttendance: 'Attendance',
    },
    orgManagement: {
      title: 'Organization Management',
      subtitle: 'Centralized control for all branches and members',
      permissionsTitle: 'Manager Permissions',
      permissionsSubtitle: 'Manage access and roles across all branches',
      roleOwner: 'Owner',
      roleAdmin: 'Admin',
      roleMember: 'Member',
      primaryBranch: 'Primary Branch',
      globalRole: 'Global Role',
      member: 'Member',
      email: 'Email',
      status: 'Status',
      active: 'Active',
      inactive: 'Inactive',
      saveSuccess: 'Permissions updated successfully',
      saveError: 'Failed to update permissions',
      multiBranchAccessNote: 'Roles (Owner/Admin) grant full access to all branches',
      searchPlaceholder: 'Search member...',
      lastOwnerWarning: 'Cannot change role: This is the last owner of the organization.',
      noAccountNote:
        'This employee does not have a system account and cannot be assigned a global role.',
      fetchError: 'Failed to fetch data',
      organization: 'Organization',
      branches: 'Branches',
      allStaff: 'All Staff',
      allStaffSubtitle: 'List of employees registered across all branches',
      managers: 'Managers',
      staff: 'Staff',
    },
    organization: {
      inviteMember: 'Invite Member',
      invites: {
        title: 'Invite New Member',
        subtitle: 'Send an invitation link to a new employee',
        email: 'Email Address',
        role: 'Role',
        send: 'Send Invite',
        success: 'Invitation link generated!',
        copy: 'Copy Link',
        copied: 'Copied!',
        close: 'Close',
      },
    },

    profile: {
      role: 'Pharmacist',
      changePhoto: 'Change Photo',
      reset: 'Reset',
      signOut: 'Sign Out',
      updateProfile: 'Update Profile',
      avatar: 'Profile Avatar',
      contactInfo: 'Contact Information',
      adjustBanner: 'Adjust Banner Position',
      dragToPan: 'Drag to position background',
      bannerZoom: 'Zoom',
      resetPosition: 'Reset',
      done: 'Done',
    },

    global: {
      actions: {
        theme: 'Switch Theme',
        fullscreen: 'Fullscreen',
        reload: 'Reload',
        help: 'Help & Support',
        close: 'Close',
        asc: 'Ascending',
        desc: 'Descending',
        all: 'All',
        import: 'Import',
        print: 'Print',
        openInWindow: 'Open in Window',
        showStats: 'Show Statistics',
        hideStats: 'Hide Statistics',
        // DataPort (Export/Import)
        exportCSV: 'Export CSV',
        exportExcel: 'Export Excel',
        exportPDF: 'Export PDF',
        importCSV: 'Import CSV/Excel',
        exportData: 'Export Data',
        importData: 'Import Data',
        importSuccess: 'Imported successfully',
        importError: 'Import failed',
        rowsImported: 'rows imported',
      },
      datePicker: {
        cancel: 'Cancel',
        ok: 'OK',
        hour: 'Hour',
        minute: 'Minute',
      },
      currency: 'L.E',
      loading: 'Loading...',
      checkingAuth: 'Checking authentication...',
      pageNotFound: 'Page not found',
      table: {
        columns: 'Columns',
        alignment: 'Alignment',
        noColumnsVisible: 'No columns visible',
        manageColumnsHint: 'Right-click anywhere to manage columns',
        loadingData: 'Loading data...',
        page: 'Page',
        of: 'of',
        showAll: 'Show All',
        filters: 'Filters',
        addFilter: 'Add Filter',
        clearAllFilters: 'Clear All Filters',
        clearFilter: 'Clear Filter',
        selectedCount: 'selected',
        showing: 'Showing',
        showingAll: 'Showing All Items',
        total: 'Total',
      },
    },
    login: {
      title: 'Welcome back!',
      subtitle: 'Enter your credentials to access the account.',
      username: 'Username',
      usernamePlaceholder: 'Enter username',
      password: 'Password',
      passwordPlaceholder: '••••••••',
      rememberMe: 'Remember for 30 days',
      forgotPassword: 'Forgot password?',
      submit: 'Continue',
      signingIn: 'Signing in...',
      successTitle: 'Login Successful!',
      redirecting: 'Redirecting to dashboard...',
      errorInvalidCredentials: 'Invalid username or password',
      errorGeneric: 'An unexpected error occurred. Please try again.',
      validationUsernameRequired: 'Username is required',
      validationPasswordRequired: 'Password is required',
      validationPasswordLength: 'Password must be at least 4 characters',
      authorizedUserNotice: 'By continuing, you verify that you are an authorized user.',
      email: 'Email Address',
      emailPlaceholder: 'you@example.com',
      fullName: 'Full Name',
      fullNamePlaceholder: 'John Doe',
      confirmPassword: 'Confirm Password',
      signUpTitle: 'Create an Account',
      signUpSubtitle: 'Join us to access the system.',
      signUpSubmit: 'Sign Up',
      dontHaveAccount: "Don't have an account?",
      signUpLink: 'Sign up',
      haveAccount: 'Already have an account?',
      loginLink: 'Login',
      resetTitle: 'Reset Password',
      resetSubtitle: 'Enter your email to receive a reset link.',
      resetSubmit: 'Send Reset Link',
      backToLogin: 'Back to Login',
      passwordsDoNotMatch: 'Passwords do not match',
      resetLinkSent: 'Reset link has been sent to your email',
      tempPasswordSent: 'A temporary password has been sent to your email.',
      employeePortal: 'Employee Portal',
      manageEmployment: 'Manage your employment across ZINC organizations',
      pendingRequests: 'Pending Employment Requests',
      noPendingRequests: 'No pending requests',
      accept: 'Accept',
      reject: 'Reject',
      selectWorkspace: 'Select Workspace',
      selectWorkspaceDesc:
        'You have access to multiple pharmacy organizations. Choose which one you want to enter.',
      confirmReject: 'Are you sure you want to reject this employment request?',
      errorRejectingRequest: 'Failed to reject request',
      errorAcceptingRequest: 'Failed to accept request',
      clockSkewTitle: 'System clock out of sync',
      clockSkewMessage:
        'Your device time is incorrect, which prevents secure connection to our servers. Please update your system date, time, and timezone, then refresh the page.',
    },
    statusBar: {
      ready: 'Ready',
      online: 'Online',
      offline: 'Offline',
      notifications: 'Notifications',
      noNotifications: 'No notifications',
      clearAll: 'Clear all',
      dismiss: 'Dismiss',
      shiftOpen: 'Shift Open',
      shiftClosed: 'Shift Closed',
      shiftSince: 'Since',
      userLabel: 'User',
      quickLogin: {
        login: 'Login',
        username: 'Username',
        password: 'Password',
        newPassword: 'New Password',
        forgotPassword: 'Forgot Password?',
        notFound: 'Not found',
        resetSent: 'Reset link sent to your email',
        passwordUpdated: 'Password updated successfully',
        changeSuccess: 'Password changed successfully',
        biometricUnsupported: 'Browser does not support Passkeys. Ensure you are on HTTPS.',
        contactManager: 'Contact manager to reset password',
        passkeyTooltip: 'Sign in with Passkey',
      },
      messages: {
        outOfStock: 'Out of Stock: {{name}} {{form}}',
        saleComplete: 'Sale completed: {{total}}',
      },
      ticker: {
        todaySales: 'Today',
        invoices: 'Invoices',
        completed: 'Done',
        pending: 'Pending',
        lowStock: 'Low Stock',
        shortages: 'Shortages',
        newCustomers: 'New Customers',
        topSeller: 'Top Seller',
      },
    },
    loginAudit: {
      title: 'Login Audit Trail',
      subtitle: 'Track all authentication and session events across the system',
      searchPlaceholder: 'Search username or ID...',
      emptyMessage: 'No audit records found',
      showAllBranches: 'All Branches',
      globalView: 'Global View',
      yesterday: 'Yesterday',
      today: 'Today',
      am: 'AM',
      pm: 'PM',
      actions: {
        login: 'Login',
        logout: 'Logout',
        switch: 'Switch',
        system_login: 'System Access',
        system_logout: 'System Exit',
        switchBranch: 'Switch Branch',
        forceLogout: 'Force Logout',
        switchOrg: 'Switch Org',
        employeeLogout: 'Employee Logout',
      },
      headers: {
        id: 'ID',
        timestamp: 'Timestamp',
        user: 'User',
        code: 'Code',
        action: 'Action',
        branch: 'Branch',
        details: 'Details',
      },
      detailPatterns: {
        switchedFrom: 'Switched from {{name}}',
        switchedBranch: 'Switched from {{from}} to {{to}}',
        account: 'Account: {{name}}',
        employeeSignedOut: 'Employee signed out',
        accountLogout: 'Account Logout',
        systemSessionStarted: 'System session started',
        employeeSessionStarted: 'Employee session started',
        terminatedBy: 'Session terminated by {{name}}',
        employeeLoggedOutBy: 'Employee {{name}} logged out by {{admin}}',
        remotelyBadge: 'Remotely',
      },
    },
    activeSessions: {
      title: 'Active Sessions',
      errorLoading: 'Failed to load sessions',
      confirmEndAll: 'Are you sure you want to end all other sessions?',
      sessionsCount: 'sessions',
      activeCount: 'Active',
      online: 'Online',
      offline: 'Offline',
      stale: 'Stale',
      current: 'Current',
      thisDevice: 'this device',
      yourCurrentDevice: 'Your current device',
      runningMoreThan24h: 'Running for more than 24h',
      considerEndingStale: 'Consider ending stale sessions',
      unassigned: 'Unassigned',
      searchPlaceholder: 'Search by name, device, or IP...',
      refresh: 'Refresh',
      endAllOthers: 'End All Others',
      noSearchResults: 'No sessions match your search',
      noActiveSessions: 'No active sessions found',
      onlineNow: 'Online Now',
      action: 'Action',
      terminate: 'Terminate',
      end: 'End',
      sessionInfo: 'Session Info',
      loggedIn: 'Logged in',
      previously: 'Previously',
      startedLabel: 'Started:',
      durationLabel: 'Duration:',
      seenLabel: 'Seen:',
      sort: {
        user: 'User',
        device: 'Device',
        browser: 'Browser',
        started: 'Started',
        duration: 'Duration',
        ip: 'IP Address',
        lastSeen: 'Last Seen',
      },
    },
    pendingBranch: {
      title: 'Branch Assignment Pending',
      subtitle: 'Your profile is active, but you are not assigned to a branch yet.',
      employeeMessage:
        'Please ask your system administrator or organization owner to assign your profile to a branch in the HR/Staff settings.',
      switchProfile: 'Switch Profile',
      signOut: 'Sign Out',
      adminTitle: 'Activate a Branch',
      adminSubtitle:
        'As an administrator, you can choose a branch to manage or create one if none exist.',
      selectBranch: 'Select Branch',
      selectBranchPlaceholder: 'Choose a branch to activate...',
      noBranches: 'No branches have been created yet.',
      createBranchBtn: 'Go to Branch Configuration',
      loadingBranches: 'Loading organization branches...',
      errorLoading: 'Failed to load branches',
      activateBtn: 'Activate Branch',
    },
    inventory: {
      tabs: {
        inventory: 'Inventory',
        addProduct: 'Add Product',
        stockMovement: 'Stock Movement',
        shortages: 'Shortages',
      },
      title: 'Inventory',
      subtitle: 'Manage your pharmaceutical stock',
      outOfStock: 'Out of Stock',
      available: 'Available',
      shortage: 'Shortage',
      all: 'All',
      active: 'Active',
      inactive: 'Inactive',
      discontinued: 'Discontinued',
      summary: {
        totalItems: 'Total Items',
        totalCost: 'Inventory Cost',
        saleValue: 'Total Sale Value',
        restock: 'Critical Restock',
        nearReorder: 'Near Reorder',
        discontinued: 'Discontinued Items',
      },
      addDrug: 'Add New Drug',
      allProducts: 'All Products',
      addNewProduct: 'Add New Product',
      addProductSubtitle: 'Add a new item to your inventory',
      productAddedSuccess: 'Product added successfully!',
      basicInfo: 'Basic Information',
      productCodes: 'Product Codes',
      stockDetails: 'Stock Details',
      pricing: 'Pricing',
      additionalInfo: 'Additional Information',
      autoGenerate: 'Auto-Generate',
      clearForm: 'Clear Form',
      saveAndAddAnother: 'Save & Add Another',
      searchPlaceholder: 'Search name, code, category, or desc...',
      headers: {
        name: 'Name',
        codes: 'Codes',
        category: 'Category',
        status: 'Status',
        stock: 'Stock',
        publicPrice: 'Price',
        cost: 'Cost',
        expiry: 'Expiry',
        actions: 'Actions',
      },
      fields: {
        status: 'Product Status',
        origin: 'Origin',
        originLocal: 'Local',
        originImported: 'Imported',
      },
      actionsMenu: {
        edit: 'Edit Drug',
        delete: 'Remove Drug',
        view: 'View Details',
        printBarcode: 'Print Barcode',
        duplicate: 'Duplicate Drug',
        adjustStock: 'Quick Stock Adjust',
        printQtyPrompt: 'How many labels to print?',
      },
      noResults: 'No drugs found matching your search.',
      modal: {
        edit: 'Edit Drug',
        add: 'Add New Drug',
        brand: 'Brand Name',
        nameArabic: 'Arabic Name',
        generic: 'Generic Name',
        category: 'Category',
        expiry: 'Expiry Date',
        publicPrice: 'Selling Price ($)',
        cost: 'Cost Price ($)',
        stock: 'Stock Qty',
        desc: 'Description',
        barcode: 'Barcode (Global)',
        internalCode: 'Internal Code',
        unitsPerPack: 'Units Per Pack',
        cancel: 'Cancel',
        save: 'Save Drug',
        tabGeneral: 'General Info',
        tabInventoryPricing: 'Inventory & Pricing',
      },
      details: {
        stockLevel: 'Stock Level',
        packs: 'packs',
        sellingPrice: 'Selling Price',
        costPrice: 'Cost Price',
        unitsPerPack: 'Units Per Pack',
        barcode: 'Barcode',
        internalCode: 'Internal Code',
        description: 'Description',
        noDescription: 'No description provided.',
      },
      expiryStatus: 'Expiry Status',
      allExpiry: 'Show All',
      nearExpired: 'Near Expired (6 months)',
      expired: 'Expired',
      addProduct: {
        title: 'Add New Product',
        subtitle: 'Add a new item to your inventory',
        active: 'Active',
        inactive: 'Inactive',
        discontinued: 'Discontinued',
        sections: {
          identification: 'Identification & Basic Info',
          classification: 'Categorization & Manufacture',
          usage: 'Extra Details & Usage',
          inventory: 'Inventory & Storage',
          financial: 'Financial Details',
        },
        fields: {
          brandName: 'Product Brand Name',
          nameArabic: 'Arabic Name',
          genericName: 'Scientific / Generic Name',
          category: 'Main Category',
          dosageForm: 'Dosage Form / Type',
          barcode: 'Primary Barcode',
          internalCode: 'System Internal Code',
          manufacturer: 'Producing Company',
          status: 'Product Status',
          origin: 'Product Origin',
          description: 'Description',
          stock: 'Initial Stock (Packs)',
          unitsPerPack: 'Units / Pack (Units)',
          minStock: 'Min Stock Alert',
          expiryDate: 'Expiry Date',
          publicPrice: 'Public Selling Price (Pack)',
          tax: 'Tax (%)',
          costPrice: 'Cost Price (Purchase)',
          liveProfit: 'Live Profit',
          margin: 'Margin',
          currency: 'EGP',
          gross: 'GROSS',
          originLocal: 'Local',
          originImported: 'Imported',
        },
        placeholders: {
          brandName: 'e.g., Augmentin 1g Tablets',
          genericName: 'e.g., Amlodipine, Valsartan',
          barcode: 'Scan to add...',
          internalCode: 'Auto-gen or manual',
          category: 'Select Category',
          dosageForm: 'Select Form',
          manufacturer: 'e.g., Pfizer, Novartis',
          description: 'Enter indications, warnings, or extra information here...',
          langDetect: 'Auto-detecting language...',
          nameArabic: 'مثال: أوجمنتين ١ جرام',
        },
        tooltips: {
          clear: 'Reset all fields to default values',
          saveAndNew: 'Save this product and stay here to add another one',
          save: 'Finalize and save this product to inventory',
        },
        actions: {
          save: 'Save Product',
          saveAndNew: 'Save & New',
          clear: 'Clear Form',
          cancel: 'Cancel',
          generateCode: 'Auto-Generate',
        },
        messages: {
          success: 'Product added successfully!',
          error: 'Failed to add product. Please check all fields.',
          cleared: 'Form cleared',
        },
      },
    },
    stockAdjustment: {
      title: 'Stock Adjustment',
      subtitle: 'Correct inventory levels',
      searchPlaceholder: 'Search product to adjust...',
      add: 'Add to Adjustment',
      newStock: 'New Stock',
      difference: 'Difference',
      reason: 'Reason',
      notes: 'Notes',
      save: 'Save Adjustment',
      clear: 'Clear',
      history: 'Adjustment History',
      table: {
        product: 'Name',
        current: 'Current Stock',
        new: 'New Stock',
        diff: 'Difference',
        valueImpact: 'Value Impact',
        reason: 'Reason',
        notes: 'Notes',
        action: 'Action',
      },
      reasons: {
        damaged: 'Damaged',
        expired: 'Expired',
        theft: 'Theft / Loss',
        inventory_count: 'Inventory Count',
        correction: 'Data Correction',
        other: 'Other',
      },
      success: 'Stock adjustment saved successfully!',
      empty: 'No items to adjust. Search and add products.',
      noHistory: 'No history records found for the selected period.',
      noRecent: 'No recent transaction to print. Perform an adjustment first.',
      historyLog: 'History Log',
      adjustStock: 'Adjust Stock',
    },
    drugInteractions: {
      title: 'Drug Interactions',
      subtitle: 'Analyze safety warnings and contraindications from FDA data',
      searchPlaceholder: 'Search drug by name or @active_ingredient...',
      interactions: 'Interactions',
      warnings: 'Warnings',
      contraindications: 'Contraindications',
      noFdaData: 'No FDA data available for this drug',
      notApproved: 'This drug may not be FDA-registered (e.g., generic local version)',
      activeIngredients: 'Active Ingredients',
      source: 'Source: OpenFDA',
      loading: 'Fetching FDA clinical data...',
      retry: 'Retry Request',
      selectDrug: 'Select a drug from results to view data',
      noInteractions: 'No interactions found in FDA database',
      combinationDrug: 'Combination Product',
      ingredientCount: 'active ingredients',
      usage: 'Usage',
      noResults: 'No products found',
      safetyStatus: 'Safety Status',
      selectMultipleDrugs: 'Select multiple drugs to analyze interactions',
      searchAndAddDrugs: 'Search and add drugs from the sidebar, then explore FDA clinical data',
      clearCocktail: 'Clear Cocktail',
      interactionAnalysis: 'Interaction Analysis',
      drugsSelected: '{count} Drug{s} Selected',
      sidebarPlaceholder: 'Start typing to search drugs...',
    },
    barcodePrinter: {
      title: 'Barcode Printer',
      subtitle: 'Queue and print product labels',
      searchPlaceholder: 'Search product to print...',
      addToQueue: 'Add to Queue',
      printLabels: 'Print Labels',
      totalLabels: 'Total Labels',
      clearQueue: 'Clear Queue',
      tableHeaders: {
        item: 'Item',
        expiry: 'Expiry',
        qty: 'Qty',
        actions: 'Actions',
      },
      alerts: {
        queueEmpty: 'Print queue is empty. Add items to print.',
        printing: 'Generating labels...',
        printSuccess: 'Labels sent to printer',
      },
      settings: {
        store: 'Pharmacy Name',
        hotline: 'Hotline',
        name: 'Drug Name',
        publicPrice: 'Price',
        expiry: 'Expiry Date',
        barcode: 'Barcode',
      },
    },
    suppliers: {
      title: 'Suppliers',
      subtitle: 'Manage distributors and pharmaceutical companies',
      addSupplier: 'Add Supplier',
      searchPlaceholder: 'Search supplier name, contact...',
      suppliersList: 'Suppliers List',
      addNewSupplier: 'Add New Supplier',
      allSuppliers: 'All Suppliers',
      manageSuppliers: 'Manage your suppliers',
      createNewRecord: 'Create a new supplier record',
      noSuppliersFound: 'No suppliers found matching your search',
      noSuppliersAvailable: 'No suppliers available',
      headers: {
        id: 'ID',
        name: 'Name',
        contactPerson: 'Contact Person',
        phone: 'Phone',
        email: 'Email',
        address: 'Address',
        action: 'Action',
        companyName: 'Company Name',
        contact: 'Contact Person',
        details: 'Contact Details',
        actions: 'Actions',
        governorate: 'Governorate',
        city: 'City',
        area: 'Area',
      },
      form: {
        companyInfo: 'Company Information',
        contactInfo: 'Contact Information',
        id: 'ID',
        companyName: 'Company Name',
        enterCompanyName: 'Enter company name',
        address: 'Address',
        enterAddress: 'Enter company address',
        contactPerson: 'Contact Person',
        enterContactPerson: 'Enter contact person name',
        phone: 'Phone',
        phonePlaceholder: '+1234567890',
        email: 'Email',
        emailPlaceholder: 'email@example.com',
        paymentTerms: 'Payment Terms',
        openingBalance: 'Opening Balance',
        paymentType: 'Default Payment Type',
        creditDays: 'Credit Period (days)',
        creditLimit: 'Credit Limit',
        cash: 'Cash',
        credit: 'Credit',
      },
      modal: {
        add: 'Add New Supplier',
        edit: 'Edit Supplier',
        editSubtitle: 'Update supplier information',
        details: 'Supplier Details',
        detailsSubtitle: 'View supplier information',
        delete: 'Delete Supplier',
        deleteSubtitle: 'This action cannot be undone',
        confirmDelete: 'Are you sure you want to delete',
        name: 'Company Name',
        contact: 'Contact Person',
        phone: 'Phone Number',
        email: 'Email Address',
        address: 'Physical Address',
        cancel: 'Cancel',
        save: 'Save Supplier',
        saveChanges: 'Save Changes',
        deleteBtn: 'Delete',
        addSupplier: 'Add Supplier',
        close: 'Close',
      },
      contextMenu: {
        viewDetails: 'View Details',
        edit: 'Edit',
        delete: 'Delete',
        copyName: 'Copy Name',
        copyPhone: 'Copy Phone',
        copyEmail: 'Copy Email',
        showHideColumns: 'Show/Hide Columns',
      },
      errors: {
        invalidPhone: 'Invalid phone number',
        invalidEmail: 'Invalid email address',
      },
      required: 'required',
      fillRequired: 'Please fill in all required fields',
    },
    supplierAccounts: {
      paymentsTitle: 'Supplier Payments',
      paymentsSubtitle: 'Record and manage payments to suppliers',
      statementTitle: 'Supplier Statement',
      statementSubtitle: 'Chronological ledger with running balance',
      agingTitle: 'Supplier Aging',
      agingSubtitle: 'Outstanding balances by due period',
      supplier: 'Supplier',
      date: 'Date',
      amount: 'Amount',
      paymentMethod: 'Payment Method',
      reference: 'Reference',
      notes: 'Notes',
      currentBalance: 'Current Balance',
      allocations: 'Allocations',
      invoice: 'Invoice',
      dueDate: 'Due Date',
      openAmount: 'Open Amount',
      applyToOldest: 'will be applied to your oldest invoices automatically',
      totalAllocated: 'Total Allocated',
      remaining: 'Remaining',
      recordPayment: 'Record Payment',
      voidPayment: 'Void',
      voidReason: 'Void reason',
      paymentHistory: 'Payment History',
      noPayments: 'No payments recorded yet',
      noPayables: 'No open invoices for this supplier',
      openPayables: 'Open Payables',
      cash: 'Cash',
      bank: 'Bank',
      visa: 'Card',
      entryDate: 'Date',
      type: 'Type',
      sourceRef: 'Source Ref',
      debit: 'Debit (owed)',
      credit: 'Credit (paid)',
      runningBalance: 'Running Balance',
      exportCSV: 'Export CSV',
      asOfDate: 'As of date',
      searchSupplier: 'Search by supplier name...',
      searchTransactions: 'Search by reference, date, or type...',
      current: 'Current',
      days130: '1–30',
      days3160: '31–60',
      days6190: '61–90',
      days90Plus: '90+',
      total: 'Total',
      serialId: 'Serial',
      voided: 'Voided',
      actions: 'Actions',
      dateFrom: 'From',
      dateTo: 'To',
      noTransactions: 'No transactions found',
      noSuppliersFound: 'No suppliers found for this branch',
      print: 'Print',
      selectSupplier: 'Select a supplier',
      filters: 'Filters',
      apply: 'Apply',
      clear: 'Clear',
      paymentType: 'Type',
      purchase: 'Purchase',
      payment: 'Payment',
      creditNote: 'Credit Note',
      openingBalance: 'Opening Balance',
      reversal: 'Reversal',
      confirmVoid: 'Void this payment? This cannot be undone.',
      invalidAmount: 'Enter a valid amount greater than zero',
      allocationsExceed: 'Allocated amount cannot exceed payment amount',
    },
    pendingApproval: {
      title: 'Pending Approvals',
      subtitle:
        'Review and approve incoming purchase orders. Orders must be approved before inventory is updated.',
      allCaughtUp: 'All caught up!',
      noPendingOrders: 'No pending purchase orders requiring approval.',
      pendingReview: 'Pending Review',
      invCode: 'Invoice',
      supplier: 'Supplier',
      date: 'Date',
      totalCost: 'Total Cost',
      items: 'Items',
      reject: 'Reject',
      approve: 'Approve',
      approveOrder: 'Approve Order',
      orderDetails: 'Purchase Order Details',
      itemsList: 'Items List',
      approvedBy: 'Approved By:',
      enterName: 'Enter Name',
      enterApproverName: "Please enter the approver's name.",
      rejectOrder: {
        title: 'Reject Purchase Order?',
        confirm: 'Are you sure you want to reject this order? This action cannot be undone.',
        reason: 'Reason (Optional)',
        reasonPlaceholder: 'E.g., Incorrect pricing, wrong items...',
        cancel: 'Cancel',
        reject: 'Reject',
      },
      tableHeaders: {
        item: 'Item',
        expiry: 'Expiry',
        qty: 'Qty',
        cost: 'Cost',
        discount: 'Disc %',
        publicPrice: 'Sale Price',
        total: 'Total',
      },
      info: {
        supplier: 'Supplier',
        invId: 'Invoice #',
        payment: 'Payment',
        totalCost: 'Total Cost',
        cash: 'Cash',
        credit: 'Credit',
      },
    },
    drugApproval: {
      title: 'Drug Approval Queue',
      subtitle:
        'Review new catalog drugs and decide whether to approve them into your pharmacy inventory.',
      statusPending: 'Pending',
      statusApproved: 'Approved',
      statusSuspended: 'Suspended',
      approveSelected: 'Approve Selected',
      suspendSelected: 'Suspend Selected',
      reactivateSelected: 'Reactivate Selected',
      noPendingDrugs: 'No pending global catalog additions requiring review.',
      tableHeaders: {
        drug: 'Drug Name',
        barcode: 'Barcode',
        dosageForm: 'Dosage Form',
        category: 'Category',
        price: 'Public Price',
        manufacturer: 'Manufacturer',
        status: 'Status',
        actions: 'Actions',
      },
      confirmApproveTitle: 'Approve {{count}} Drug(s)?',
      confirmApproveDesc:
        'This will instantly add the selected drug(s) to all branches in your pharmacy network with 0 initial stock and cost price.',
      confirmSuspendTitle: 'Suspend {{count}} Drug(s)?',
      confirmSuspendDesc:
        'This will mark the selected drug(s) as suspended. You can reactivate them later.',
      successApprove: 'Selected drug(s) approved successfully!',
      successSuspend: 'Selected drug(s) suspended successfully!',
      successReactivate: 'Selected drug(s) reactivated successfully!',
      filterAll: 'All',
      searchPlaceholder: 'Search by name, active substance or barcode...',
    },
    purchases: {
      title: 'New Purchase',
      subtitle: 'Restock inventory from suppliers',
      selectSupplier: 'Select Supplier',
      searchDrug: 'Search drug to buy...',
      emptyCart: 'No items in order',
      tabs: {
        newTab: 'New Purchase',
        closeTab: 'Close Tab',
        closeOthers: 'Close Others',
        duplicateTab: 'Duplicate Tab',
        pin: 'Pin Tab',
        unpin: 'Unpin Tab',
        rename: 'Rename Tab',
      },
      // Stock filter options
      filters: {
        all: 'All',
        inStock: 'In Stock',
        outOfStock: 'Out Stock',
      },
      // Status filter options
      status: {
        title: 'Status',
        all: 'All Status',
        pending: 'Pending',
        approved: 'Approved',
        completed: 'Completed',
        returned: 'Returned',
        rejected: 'Rejected',
      },
      // Placeholders
      placeholders: {
        searchHistory: 'Search ID, Supplier...',
        searchSupplier: 'Search and select supplier...',
        enterId: 'Enter ID',
        searchDrug: 'Search products...',
      },
      pack: 'Pack',
      unit: 'Unit',
      // Table headers
      tableHeaders: {
        orderId: 'Order #',
        invId: 'Invoice #',
        barcode: 'Barcode',
        date: 'Date',
        supplier: 'Supplier',
        payment: 'Payment',
        items: 'Items',
        discount: 'Discount',
        total: 'Total',
        action: 'Action',
      },
      // Status tooltips
      tooltips: {
        pending: 'Pending Approval',
        approved: 'Approved',
        rejected: 'Rejected',
        returned: 'Returned',
        completed: 'Completed',
      },
      // Context menu actions
      contextMenu: {
        viewDetails: 'View Details',
        copyInvoice: 'Copy Invoice',
        copySupplier: 'Copy Supplier',
        addToOrder: 'Add to Order',
        copyName: 'Copy Name',
        editQuantity: 'Edit Quantity',
        removeItem: 'Remove Item',
        copy: 'Copy',
        paste: 'Paste',
        clear: 'Clear',
        showHideColumns: 'Show/Hide Columns',
      },
      // Alert messages
      alerts: {
        enterInvoice: 'Please enter the Invoice Number (Inv #) from the supplier.',
        duplicateInvoice: 'This Invoice ID already exists. Please enter a unique Invoice ID.',
        enterQuantity: 'Please enter a valid quantity for',
        enterCost: 'Please enter a valid Cost Price for',
        enterSale: 'Please enter a valid Sale Price for',
        enterExpiry: 'Please enter an Expiry Date for',
        incompleteExpiry: 'Please enter a complete expiry date (4 digits: MMYY)',
        promptQuantity: 'Enter quantity:',
        confirmCloseTab:
          'This tab contains items. Are you sure you want to close it and move it to history?',
        couldNotCreateInvoice: 'Could not create the invoice, please try again.',
      },
      // Other labels
      unknown: 'Unknown',
      cash: 'Cash',
      credit: 'Credit',
      partial: 'Partial',
      paidNow: 'Paid Now',
      payNow: 'Pay Now',
      paymentMethod: 'Payment Method',
      pending: 'Save as Pending',
      startSearching: 'Start searching for products to add to purchase order',
      noResults: 'No results found',
      tryDifferentKeywords: 'Try searching with different keywords',
      fromDate: 'From',
      toDate: 'To',
      time: {
        am: 'AM',
        pm: 'PM',
      },
      headers: {
        item: 'Item',
        cost: 'Cost',
        qty: 'Qty',
        total: 'Total',
        name: 'Name',
        codes: 'Codes',
        expiry: 'Expiry',
        stock: 'Stock',
        profit: 'Profit',
        product: 'Product',
        subtotal: 'Subtotal',
        totalPlusTax: 'Total+Tax',
        supplier: 'Supplier',
        internalCode: 'Internal Code',
      },
      cartFields: {
        qty: 'Qty',
        expiry: 'Expiry',
        cost: 'Cost',
        discount: 'Disc %',
        sale: 'Sale',
        tax: 'Tax %',
        subtotal: 'Subtotal',
        totalWithTax: 'Total+Tax',
        batchNumber: 'Batch #',
        unitCost: 'U. Cost',
        unitSale: 'U. Sale',
      },
      summary: {
        totalItems: 'Total Items',
        totalCost: 'Total Cost',
        confirm: 'Confirm',
        discount: 'Discount',
        tax: 'Tax',
      },
      startedAt: 'Started at',
      taxMode: 'Tax Mode',
      taxExclusive: 'Excl.',
      taxInclusive: 'Incl.',
      emptyCartHint: 'Search and add products to the invoice',
      suspendInvoice: 'Suspend Invoice',
      confirming: 'Confirming...',
      confirmMergeTitle: 'Confirm Merge Items',
      confirmMove: 'Confirm Move',
      cancel: 'Cancel',
      cartQr: {
        title: 'Cart QR',
        generate: 'Generate',
        scan: 'Scan',
        page: 'Page',
        invalidQr: 'Unrecognized QR. Please scan the cart QR.',
        scanMore: 'Scanning... Awaiting more pages',
        done: 'items scanned.',
        notFound: 'not found',
        previewTitle: 'Review before adding',
        addToCart: 'Add to cart',
        discard: 'Discard',
        emptyPreview: 'No readable items found in the scanned QR.',
        copied: 'Copied!',
        copyFailed: 'Copy failed',
        copy: 'Copy payload',
        download: 'Download',
        items: 'items',
        generateHint: 'Scan the QR in Purchases on the receiving pharmacy to load the items.',
        generating: 'Generating...',
      },
      mergeInvoiceHas: 'The invoice',
      mergeAlreadyContains: 'already contains',
      mergeItemsCount: 'items',
      mergeQuestion: 'Are you sure you want to merge the selected items into it?',
      historyTitle: 'Purchase History',
      viewHistory: 'History',
      newPurchase: 'New Purchase',
      actions: {
        viewDetails: 'View Drug Details',
        editQty: 'Edit Quantity',
      },
      // Purchase Details Modal
      detailsModal: {
        title: 'Purchase Order Details',
        supplier: 'Supplier',
        invNumber: 'Inv #',
        payment: 'Payment',
        totalCost: 'Total Cost',
        approvedOn: 'Approved On',
        approvedBy: 'Approved By',
        status: 'Status',
        pendingApproval: 'Pending Approval',
        // Table headers
        item: 'Item',
        expiry: 'Expiry',
        qty: 'Qty',
        returned: 'Returned',
        cost: 'Cost',
        total: 'Total',
        // Return labels
        fullReturn: 'Full',
        partialReturn: 'Partial',
        returnedLabel: 'returned',
      },
      // Empty states
      noHistory: 'No purchase history found',
      markAsReceived: 'Mark as Received',
      autoBatchCreated: 'Inventory batches created automatically',
    },
    purchaseReturns: {
      createReturn: 'Create Return',
      returnHistory: 'Return History',
      createSubtitle: 'Return items to supplier',
      historySubtitle: 'View all purchase returns',
      selectPurchase: 'Select Purchase Order',
      selectPlaceholder: 'Select a purchase order...',
      returnAll: 'Return All Items from This Purchase',
      itemsToReturn: 'Items to Return',
      availableItems: 'Available Items from Purchase Order',
      additionalNotes: 'Additional Notes',
      notesPlaceholder: 'Add any additional notes about this return...',
      clear: 'Clear',
      submit: 'Submit Return',
      totalRefund: 'Total Refund',
      refundMethod: 'Refund Method',
      cashRefund: 'Cash',
      creditNote: 'Credit Note',
      creditNoteHint: 'This will be added to the supplier account and reduce their balance.',
      currentBalance: 'Current balance',
      quantity: 'Quantity',
      reason: 'Reason',
      condition: 'Condition',
      addToReturn: 'Add to Return',
      returnDetails: 'Return Details',
      returnInfo: 'Return Information',
      returnedItems: 'Returned Items',
      notes: 'Notes',
      close: 'Close',
      tableHeaders: {
        id: 'Return ID',
        date: 'Date',
        purchaseId: 'Purchase ID',
        supplier: 'Supplier',
        refund: 'Total Refund',
        status: 'Status',
        action: 'Action',
      },
      searchPlaceholder: 'Search returns...',
      reasons: {
        damaged: 'Damaged',
        expired: 'Expired',
        wrong_item: 'Wrong Item',
        defective: 'Defective',
        overage: 'Overage - Entered by Mistake',
        other: 'Other',
      },
      conditions: {
        damaged: 'Damaged',
        expired: 'Expired',
        other: 'Other',
      },
      status: {
        pending: 'Pending',
        approved: 'Approved',
        completed: 'Completed',
        rejected: 'Rejected',
      },
      messages: {
        selectPurchaseAlert: 'Please select a purchase and add items to return',
        noReturnsFound: 'No returns found matching your search',
        noReturnsYet: 'No purchase returns yet',
      },
      packs: 'packs',
      pack: 'pack',
      inReturnList: 'In Return List',
      available: 'Available',
    },
    pos: {
      closedTabsTitle: 'Recently Closed Tabs',
      viewClosedTabs: 'View Closed Tabs History',
      closedTabsEmpty: 'No recently closed tabs.',
      restoreTab: 'Restore Tab',
      searchPlaceholder: 'Scan barcode, name, desc...',
      categories: {
        all: 'All',
        medicine: 'Medicine',
        cosmetics: 'Cosmetics',
        general: 'General',
      },
      cartTitle: 'Current Sale',
      emptyCart: 'Cart is empty',
      customerInfo: 'Customer Information',
      customerName: 'Customer Name',
      customerCode: 'Customer Code',
      paymentMethod: 'Payment Method',
      cash: 'Cash',
      visa: 'Card',
      discount: 'Disc %',
      subtotal: 'Subtotal',
      deliveryFee: 'Delivery Fee',
      orderDiscount: 'Order Disc %',
      total: 'Total',
      completeOrder: 'Complete Order',
      itemLeft: 'left',
      viewCart: 'View Cart',
      backToProducts: 'Back to Products',
      items: 'items',
      drugTypes: 'types',
      totalQty: 'pcs',
      selectDriverPlaceholder: 'Select Driver (Optional)',
      feePlaceholder: 'Fee',
      netProfit: 'Net Profit',
      expMargin: 'Exp. Margin:',
      netProfitDesc: 'Net gain after cost and discounts.',
      pack: 'Pack',
      unit: 'Unit',
      perUnit: '/unit',
      actions: {
        checkStock: 'Check Stock',
        showSimilar: 'Show Similar',
        discount: 'Apply Discount',
        changeQty: 'Change Quantity',
        searchInTable: 'Search in Table',
      },
      change: 'Change',
      allStock: 'All Stock',
      inStock: 'In Stock',
      outOfStock: 'Out of Stock',
      posTitle: 'Point of Sale',
      name: 'Name',
      code: 'Code',
      category: 'Category',
      publicPrice: 'Price',
      stock: 'Stock',
      batches: 'Batches',
      inCart: 'In Cart',
      address: 'Address',
      changeCustomer: 'Change Customer',
      customerSearchPlaceholder: 'Search by name, phone, code...',
      noStock: 'No Stock',
      productDetails: 'Product Details',
      close: 'Close',
      copy: 'Copy',
      paste: 'Paste',
      clear: 'Clear',
      deliveryOrder: 'Delivery Order',
      deliveryOrders: 'Delivery Orders',
      deliveryCalculator: {
        title: 'Distance Pricing',
        reset: 'Reset to Default',
        customDistance: 'Custom Distance...',
        km: 'km',
        perKm: '/ km',
        egp: 'EGP',
      },
      noOpenShift: 'Open a shift before completing sales',
      switchToPack: 'Switch to Pack',
      switchToUnit: 'Switch to Unit',
      removeItem: 'Remove Item',
      viewDetails: 'View Details',
      copyName: 'Copy Name',
      addToCart: 'Add to Cart',
      icon: 'Icon',
      pendingValueDetails: 'Pending Value Details',
      timeDistribution: 'Time Distribution',
      today: 'Today',
      older: 'Older',
      todaysOrders: "Today's Orders",
      last24Hours: 'Last 24 Hours',
      olderOrders: 'Older Orders',
      pendingValueInsightFooter: 'Analyzing pending values helps improve delivery speed.',
      orderPlural: 'Orders',
      orderSingular: 'Order',
      cancelOrderConfirm:
        'Are you sure you want to cancel this order? This cannot be undone and all items will be returned to stock.',
      keepOrder: 'No, Keep it',
      modal: {
        stock: 'Stock',
        publicPrice: 'Price',
        category: 'Category',
        expiry: 'Expiry Date',
        location: 'Location',
        description: 'Description',
        noDescription: 'No description available.',
        shelf: 'Shelf',
        history: 'Customer History',
      },
      invoice: 'Invoice',
      invoiceDetails: 'Invoice Details',
      favorites: 'Favorites',
      phone: 'Phone',
      itemAdded: 'Item added to cart',
      customerHistory: {
        title: 'Customer History',
      },
      startSearching: 'Start searching...',
      noResults: 'No drugs found matching your search.',
      resultsLabel: 'RESULT',
      tryDifferentKeywords: 'Try different names or codes.',
      tabs: {
        newTab: 'New Tab',
        closeTab: 'Close Tab',
        closeOthers: 'Close Others',
        duplicateTab: 'Duplicate Tab',
        pin: 'Pin',
        unpin: 'Unpin',
        rename: 'Rename',
      },
      selectDriver: 'Select Driver',
      assignDriver: 'Assign Driver',
      all: 'All',
      pending: 'Pending',
      active: 'Active',

      startDelivery: 'Start Delivery',
      onWay: 'On Way',
      completed: 'Completed',
      cancelled: 'Cancelled',
      start: 'Start',
      complete: 'Complete',
      deliveryMan: 'Delivery Man',
      searchOrder: 'Search orders...',
      noOrders: 'No delivery orders found',
      totalPending: 'Total Pending Value',
      edit: 'Edit',
      editOrder: 'Edit Order',
      printReceipt: 'Print Receipt',
      saveChanges: 'Save Changes',
      discard: 'Discard',
      editGuestInfo: 'Edit Guest Info',
      tempCustomerName: 'Temporary Name',
      tempCustomerAddress: 'Temporary Address',
      addressPlaceholder: 'Enter full delivery address...',
      history: 'History',
      orderItems: 'Order Items',
      loginToEdit: 'Login to edit',
      restore: 'Restore',
      delete: 'Delete',
      deleted: 'Deleted',
      by: 'By',
      time: 'Time',
      yesterday: 'Yesterday',
      am: 'AM',
      pm: 'PM',
      confirmDelete: 'Are you sure you want to delete this item?',
      remainder: 'Change',
      justNow: 'Just now',
      ago: 'ago',
      modification: 'Change',
      modifications: 'Changes',
      customer: 'Customer',
      guestCustomer: 'Guest Customer',
      stockLimitReached: 'Stock limit reached',
      units: 'Units',
      packs: 'Packs',
    },
    cashRegister: {
      title: 'Cash Register',
      subtitle: 'Manage daily cash shifts and transactions',
      status: {
        open: 'Shift Open',
        closed: 'Shift Closed',
        details: 'Shift Details',
      },
      actions: {
        openShift: 'Open Shift',
        closeShift: 'Close Shift',
        addCash: 'Add Cash (In)',
        removeCash: 'Remove Cash (Out)',
        printReport: 'Print Report',
      },
      summary: {
        openingBalance: 'Opening Balance',
        cashSales: 'Cash Sales',
        cashIn: 'Cash In',
        cashOut: 'Cash Out',
        closingBalance: 'Closing Balance',
        expectedBalance: 'Expected Balance',
        variance: 'Variance',
        cardSales: 'Card Sales',
        returns: 'Returns',
        cashPurchases: 'Cash Purchases',
        cashPurchaseReturns: 'Cash Purch. Returns',
      },
      transactions: {
        title: 'Shift Transactions',
        time: 'Time',
        type: 'Type',
        amount: 'Amount',
        reason: 'Reason/Notes',
        user: 'User',
      },
      modal: {
        openTitle: 'Open New Shift',
        closeTitle: 'Close Shift',
        amount: 'Amount',
        notes: 'Notes',
        cancel: 'Cancel',
        confirm: 'Confirm',
      },
      types: {
        opening_balance: 'Opening Balance',
        sale: 'Cash Sale',
        card_sale: 'Card Sale',
        in: 'Cash In',
        out: 'Cash Out',
        closing_balance: 'Closing Balance',
        return: 'Return',
        expense: 'Expense',
        purchase: 'Purchase',
        purchase_return: 'Purchase Return',
        supplier_payment: 'Supplier Payment',
      },
      messages: {
        loading: 'Loading...',
        noShift: 'Open a shift to see live balance details',
        noTransactions: 'No transactions yet',
        started: 'Started',
        by: 'By',
        id: 'ID',
        expected: 'Expected',
        countedCash: 'Counted Cash Amount',
        optionalNotes: 'Optional notes...',
      },
      validation: {
        amountRequired: 'Please enter an amount',
        negativeAmount: 'Amount cannot be negative',
        positiveAmount: 'Amount must be greater than zero',
        insufficientBalance: 'Cannot withdraw more than current balance',
        reasonRequired: 'Reason is required for cash withdrawal',
        protectedBalance:
          'Cannot withdraw from opening balance. Only sales and deposits can be withdrawn.',
      },
      filters: {
        all: 'All',
        sales: 'Sales',
        returns: 'Returns',
        purchases: 'Purchases',
        operations: 'Ops',
      },
      help: '@@import(./cashRegisterHelp.json)',
    },
    shiftHistory: {
      title: 'Shift History',
      subtitle: 'View past cash register shifts',
      searchPlaceholder: 'Search by shift ID or user...',
      activeNow: 'Active Now',
      dateFrom: 'From',
      dateTo: 'To',
      exportCSV: 'Export CSV',
      noResults: 'No shifts found',
      durationLabels: {
        hourSingular: 'hour',
        hourDual: 'hours',
        hours3to10: 'hours',
        hours11plus: 'hours',
        minuteSingular: 'minute',
        minuteDual: 'minutes',
        minutes3to10: 'minutes',
        minutes11plus: 'minutes',
        and: ' and ',
      },
      headers: {
        shiftId: 'Shift ID',
        shiftNumber: 'Shift #',
        systemId: 'System ID',
        openTime: 'Opened',
        closeTime: 'Closed',
        duration: 'Duration',
        openingBalance: 'Opening',
        closingBalance: 'Closing',
        variance: 'Variance',
        openedBy: 'Opened By',
        closedBy: 'Closed By',
      },
      summary: {
        totalShifts: 'Total Shifts',
        totalRevenue: 'Total Revenue',
        avgPerShift: 'Avg/Shift',
      },
      details: {
        title: 'Shift Details',
        openedBy: 'Opened By',
        cashSales: 'Cash Sales',
        cardSales: 'Card Sales',
        transactions: 'Transactions',
        transactionLog: 'Transaction Log',
        moreTransactions: 'more transactions',
        cashPurchases: 'Cash Purchases',
        noTransactions: 'No transactions recorded',
      },
    },
    salesHistory: {
      title: 'Sales History',
      subtitle: 'View and manage past transactions',
      status: 'Status',
      completed: 'Completed',
      cancelled: 'Cancelled',
      searchPlaceholder: 'Search customer, ID, or item name...',
      dateFrom: 'From',
      dateTo: 'To',
      sortBy: 'Sort by',
      sortNewest: 'Newest First',
      sortOldest: 'Oldest First',
      sortHighTotal: 'Highest Amount',
      sortLowTotal: 'Lowest Amount',
      exportCSV: 'Export CSV',
      totalRevenue: 'Total Revenue',
      pending: 'Pending',
      with_delivery: 'With Delivery',
      on_way: 'On Way',
      returned: 'Returned',
      fullReturn: 'Full Return',
      partialReturn: 'Return',
      headers: {
        date: 'Date',
        customer: 'Customer',
        payment: 'Payment',
        items: 'Items Count',
        total: 'Total',
        actions: 'Actions',
        soldBy: 'Sold By',
        delivery: 'Delivery',
      },
      noResults: 'No sales records found.',
      visa: 'Card',
      cash: 'Cash',
      modal: {
        title: 'Sale Details',
        id: 'Transaction ID',
        date: 'Date',
        customer: 'Customer',
        payment: 'Payment Method',
        items: 'Items Purchased',
        subtotal: 'Subtotal',
        discount: 'Global Discount',
        total: 'Final Total',
        close: 'Close',
        print: 'Print Receipt',
        qty: 'Qty',
        publicPrice: 'Price',
        unitType: 'Type',
        receipt: 'Receipt',
        return: 'Return',
        history: 'Modification History',
        modificationHistory: 'Modification History',
        noHistory: 'No modifications found',
      },
      invoice: 'Invoice',
      invoiceDetails: 'Invoice Details',
      customerHistory: {
        title: 'Customer History',
      },
      customer: 'Customer',
      favorites: 'Favorites',
      phone: 'Phone',
      history: 'History',
      items: 'Items',
      discount: 'Discount',
      globalDiscount: 'Global Discount',
      total: 'Total',
      favoritesEmpty: 'No favorites yet',
      order: 'Order',
      orders: 'Orders',
      addToCart: 'Buy',
      noSalesFound: 'No sales history found',
      returns: {
        processReturn: 'Process Return',
        returnType: 'Return Type',
        fullReturn: 'Full Return',
        partialReturn: 'Return',
        unitReturn: 'Unit Return',
        selectItems: 'Select Items to Return',
        returnQuantity: 'Return Quantity',
        returnReason: 'Return Reason',
        reasons: {
          customer_request: 'Customer Request',
          wrong_item: 'Wrong Item',
          damaged: 'Damaged',
          expired: 'Expired',
          defective: 'Defective',
          other: 'Other',
        },
        notes: 'Additional Notes',
        refundAmount: 'Refund Amount',
        confirmReturn: 'Confirm Return',
        cancelReturn: 'Cancel',
        returnSuccess: 'Return processed successfully',
        inventoryRestored: 'Inventory restored',
        returned: 'Returned',
        partiallyReturned: 'Partially Returned',
        netTotal: 'Net Total',
        originalTotal: 'Original Total',
        returnedAmount: 'Returned Amount',
        viewReturns: 'View Returns',
        returnHistory: 'Return History',
        noReturns: 'No returns for this sale',
        step1: 'Step 1: Select Return Type',
        step2: 'Step 2: Select Items',
        step3: 'Step 3: Reason & Notes',
        step4: 'Step 4: Review & Confirm',
        maxQuantity: 'Max',
        itemsToReturn: 'Items to Return',
        reviewReturn: 'Review Return',
        willBeRestored: 'will be restored to inventory',
        selectReturnType: 'Select how you want to process this return',
        fullReturnDesc: 'Return all items from this sale',
        partialReturnDesc: 'Return specific items',
        unitReturnDesc: 'Return individual units from packets',
        next: 'Next',
        back: 'Back',
        noItemsSelected: 'Please select at least one item',
        enterReason: 'Please select a return reason',
        processing: 'Processing return...',
        totalItems: 'Total Items',
        itemsSelected: 'items selected',
        selectAll: 'Select All',
        deselectAll: 'Deselect All',
        noItemsAvailable: 'All items have been returned',
        validation: {
          noOpenShift: 'Cannot process return - no open shift',
          insufficientBalance: 'Return amount exceeds available sales balance',
        },
        item: 'item',
        items: 'items',
        pack: 'pack',
        packs: 'packs',
        unit: 'unit',
        units: 'units',
        returnByPack: 'Return by Pack',
        returnByUnit: 'Return by Unit',
        switchToUnit: 'Switch to Unit',
        switchToPack: 'Switch to Pack',
        perUnit: 'per unit',
      },
    },
    dashboard: {
      title: 'Dashboard Overview',
      revenue: 'Total Revenue',
      expenses: 'Total Expenses',
      profit: 'Net Profit',
      inStock: 'Products In Stock',
      lowStock: 'Low Stock Alerts',
      trend: 'Revenue Trend',
      attention: 'Low Stock Attention',
      restock: 'Restock',
      allGood: 'All stock levels normal.',
      expiringSoon: 'Expiring Soon',
      mostProfitable: 'Most Profitable',
      topSelling: 'Top Selling Products',
      recentSales: 'Recent Transactions',
      byRevenue: 'Revenue',
      byQuantity: 'Quantity',
      sold: 'sold',
      days: 'days left',
      topProducts: 'Top Products',
      dailyRevenue: 'Daily Revenue',
      analytics: 'Advanced Analytics',
      customerCode: 'Code',
      paymentMethod: 'Payment',
      cash: 'Cash',
      visa: 'Card',
      modal: {
        title: 'Restock Product',
        qty: 'Quantity to Add',
        cancel: 'Cancel',
        confirm: 'Add Stock',
      },
      expand: {
        expand: 'Expand',
        collapse: 'Collapse',
        close: 'Close',
        export: 'Export',
        exportCSV: 'Export to CSV',
        exportImage: 'Export as Image',
        filter: 'Filter',
        sort: 'Sort By',
        dateRange: 'Date Range',
        category: 'Category',
        allCategories: 'All Categories',
        from: 'From',
        to: 'To',
        apply: 'Apply',
        reset: 'Reset',
        search: 'Search',
        showAll: 'Show All',
        viewMode: 'View Mode',
        daily: 'Daily',
        weekly: 'Weekly',
        monthly: 'Monthly',
        comparison: 'Compare with Previous Period',
        breakdown: 'Breakdown',
        metrics: 'Metrics',
        quantity: 'Quantity',
        amount: 'Amount',
        profitMargin: 'Profit Margin',
        threshold: 'Stock Threshold',
        bulkRestock: 'Bulk Restock',
        details: 'Details',
        createPurchaseOrder: 'Create Purchase Order',
        timeRange: 'Time Range',
        thisWeek: 'This Week',
        thisMonth: 'This Month',
        next3Months: 'Next 3 Months',
        groupBy: 'Group By',
        expiryPeriod: 'Expiry Period',
        bulkActions: 'Bulk Actions',
        markForDiscount: 'Mark for Discount',
        removeFromInventory: 'Remove from Inventory',
        timeline: 'Timeline View',
        detailedView: 'Detailed View',
        printReceipt: 'Print Receipt',
        transactionDetails: 'Transaction Details',
        historicalTrend: 'Historical Trend (30 Days)',
        previousPeriod: 'vs Previous Period',
        noData: 'No data available',
        loading: 'Loading...',
        top20: 'Top 20 Products',
        allItems: 'All Items',
        configureThreshold: 'Configure Threshold',
        supplierInfo: 'Supplier Info',
        date: 'Date',
        target: 'Target',
        revenue: 'Revenue',
        achievement: 'Achievement',
        upcoming: 'Upcoming',
      },
    },
    barcodeStudio: {
      title: 'Barcode Studio',
      selectProduct: 'Select Product',
      settings: 'Label Settings',
      preview: 'Live Preview',
      printMode: 'Print Mode',
      single: 'Single Label (Thermal)',
      sheet: 'A4 Sheet (30 Labels)',
      print: 'Print Labels',
      saveTemplate: 'Save Template',
      templateSaved: 'Template Saved!',
      templates: 'Templates',
      gallery: {
        title: 'Template Gallery',
        subtitle: 'Browse and test templates visually before applying',
        apply: 'Apply Template',
        double: 'Double',
        myTemplates: 'My Templates',
        store: 'Template Store',
      },
      loadTemplate: 'Load Template',
      templateName: 'Template Name',
      saveAsNew: 'Save as New Template',
      deleteTemplate: 'Delete',
      templatePlaceholder: 'My Custom Template',
      createNew: 'Create Design',
      undo: 'Undo',
      redo: 'Redo',
      searchPlaceholder: 'Search product to label...',
      noProductSelected: 'Select a product to start designing',
      labelSize: 'Label Size',
      width: 'Width (mm)',
      height: 'Height (mm)',
      custom: 'Custom Size',
      barcodeSource: 'Barcode Source',
      sources: { global: 'Global Barcode', internal: 'Internal Code' },
      borderStyle: 'Border',
      borders: { none: 'None', solid: 'Solid', dashed: 'Dashed' },
      modal: {
        cancel: 'Cancel',
        save: 'Save',
      },
      inspector: {
        noSelection: 'Global Settings',
        properties: 'Element Properties',
        x: 'X (mm)',
        y: 'Y (mm)',
        width: 'W (mm)',
        height: 'H (mm)',
        fontSize: 'Font Size',
        fontWeight: 'Font Weight',
        align: 'Alignment',
        content: 'Content',
        remove: 'Remove Element',
        elements: 'Elements',
        duplicate: 'Duplicate Element',
        lock: 'Lock Position',
        unlock: 'Unlock Position',
        bringToFront: 'Bring to Front',
        sendToBack: 'Send to Back',
        defaultSet: 'Set as Default Template',
        rotate90: 'Rotate 90°',
      },
      elements: {
        storeName: 'Store Name',
        drugName: 'Drug Name',
        category: 'Category',
        publicPrice: 'Price',
        barcode: 'Barcode',
        qrcode: 'QR Code',
        internal: 'Internal Code',
        expiry: 'Expiry Date',
        text: 'Custom Text',
        image: 'Image/Logo',
        hotline: 'Hotline',
        genericName: 'Generic Name',
      },
      mode: {
        edit: 'Edit',
        truePreview: 'True Preview',
      },
      toolbar: {
        addText: 'Add Text',
        addImage: 'Add Image',
        resetLayout: 'Reset Layout',
      },
      printSettings: {
        pairedPreview: 'Show paired label preview',
        pairedPreviewDesc: 'Two labels will always be printed together to fit the dual roll.',
        showBorders: 'Show borders when printing',
        showBordersDesc: 'To verify exact dimensions during testing.',
        printCalibration: 'Print Calibration',
        horizontalOffset: 'Horizontal (Left/Right)',
        verticalOffset: 'Vertical (Up/Down)',
        resetCalibration: 'Reset Calibration',
        labelGap: 'Label Gap',
        labelGapDesc: 'Vertical spacing between labels on the same page.',
        currency: 'Currency Symbol',
        currencyEGP: 'EGP (ج.م)',
        currencyUSD: 'USD ($)',
      },
      calibration: {
        title: 'Screen Calibration',
        subtitle: 'Adjust scale to match physical size',
        button: 'Calibrate',
        reset: 'Reset Default',
      },
      tooltips: {
        doubleClickRename: 'Double-click to rename',
      },
    },
    returnHistory: {
      title: 'Return History',
      subtitle: 'View and manage processed returns',
      searchPlaceholder: 'Search returns, Sale ID, or Customer...',
      dateFrom: 'From',
      dateTo: 'To',
      headers: {
        returnId: 'Return ID',
        date: 'Date',
        saleId: 'Invoice Number',
        customer: 'Customer',
        refundAmount: 'Refund Amount',
        reason: 'Reason',
        actions: 'Actions',
        notes: 'Notes',
        totalRefund: 'Total Refund',
        items: 'Items',
      },
      noResults: 'No return records found.',
      reasons: {
        customer_request: 'Customer Request',
        wrong_item: 'Wrong Item',
        damaged: 'Damaged',
        expired: 'Expired',
        defective: 'Defective',
        other: 'Other',
      },
      actions: {
        viewDetails: 'View Details',
      },
      modal: {
        title: 'Return Details',
        returnInfo: 'Return Information',
        itemsReturned: 'Items Returned',
        item: 'Item',
        qty: 'Qty',
        refund: 'Refund',
        unit: 'Unit',
        pack: 'Pack',
      },
    },
    customers: {
      title: 'Customer Management',
      subtitle: 'Manage your customer database, loyalty points, and history',
      globalView: 'Global Directory',
      showSummary: 'Show Statistics',
      hideSummary: 'Hide Statistics',
      summary: {
        total: 'Total Customers',
        newThisMonth: 'New This Month',
        newToday: 'New Today',
        activeTotal: 'Active (Yearly)',
        activeRecently: 'Active (30 Days)',
      },
      addCustomer: 'Add Customer',
      searchPlaceholder: 'Search by name, phone, or email...',
      headers: {
        name: 'Name',
        contact: 'Contact Info',
        purchases: 'Total Purchases',
        points: 'Points',
        lastVisit: 'Last Visit',
        status: 'Status',
        actions: 'Actions',
        branch: 'Branch',
      },
      customerOverview: 'Customer Overview',
      allCustomers: 'All Customers',
      customerAddedSuccess: 'Customer added successfully!',
      addCustomerSubtitle: 'Register a new customer',
      basicInfo: 'Basic Information',
      status: {
        active: 'Active',
        inactive: 'Inactive',
      },
      conditions: {
        diabetes: 'Diabetes',
        hypertension: 'Hypertension',
        asthma: 'Asthma',
        allergies: 'Allergies',
        heartDisease: 'Heart Disease',
        arthritis: 'Arthritis',
      },
      contactOptions: {
        phone: 'Phone Call',
        sms: 'SMS / WhatsApp',
        email: 'Email',
      },
      modal: {
        add: 'Add New Customer',
        edit: 'Edit Customer',
        delete: 'Delete',
        code: 'Customer Code',
        name: 'Full Name',
        phone: 'Phone Number',
        email: 'Email Address',
        address: 'Address',
        insurance: 'Insurance Provider',
        policy: 'Policy Number',
        location: 'Preferred Pharmacy Location',
        contact: 'Preferred Contact Method',
        conditions: 'Chronic Conditions',
        streetAddress: 'Street Address',

        notes: 'Notes',
        placeholders: {
          code: 'C-001',
          downtownBranch: 'e.g. Downtown Branch',
          streetAddress: '123 Main St, Building 4, Apt 5...',
          johnDoe: 'John Doe',
          phone: '+1 234 567 890',
          email: 'john@example.com',
          insurance: 'e.g. Bupa, AXA',
          policy: 'e.g. 123456789',
          notes: 'Any additional information...',
        },
        cancel: 'Cancel',
        save: 'Save Customer',
        kioskMode: 'Patient Entry Mode',
        generateCode: 'Generate Code',
        preferences: 'Preferences & Insurance',
        insuranceDetails: 'Insurance Details',
        medicalInfo: 'Medical Information (Optional)',
        register: 'Register',
        editProfile: 'Edit Profile',
        close: 'Close',
        copied: 'Code copied to clipboard',
        kioskDesc: 'Please fill in your details to register with us.',
        history: 'Customer History',
        basicInfo: 'Basic Info',
        additionalInfo: 'Additional Info',
      },
      contextMenu: {
        showProfile: 'Show Profile Info',
        viewHistory: 'View History',
      },
      noResults: 'No customers found.',
      invoice: 'Invoice',
      invoiceDetails: 'Invoice Details',
      customerHistory: {
        title: 'Customer History',
        subtitle: 'View detailed sales and returns history',
        searchPlaceholder: 'Search by name, code, phone, or address...',
        noCustomerSelected: 'Select a customer to view history',
        stats: {
          totalSpent: 'Total Spent',
          totalReturns: 'Total Returns',
          points: 'Loyalty Points',
          visitCount: 'Total Visits',
        },
        tabs: {
          invoices: 'Invoices',
          returns: 'Returns',
        },
        table: {
          date: 'Date',
          invoiceNo: 'Invoice #',
          returnNo: 'Return #',
          items: 'Items',
          total: 'Total',
          refundAmount: 'Refund',
          status: 'Status',
          reason: 'Reason',
        },
        statuses: {
          completed: 'Completed',
          pending: 'Pending',
          cancelled: 'Cancelled',
          with_delivery: 'With Delivery',
          on_way: 'On Way',
          returned: 'Returned',
        },
      },
    },
    customerOverview: {
      title: 'Customer Overview',
      subtitle: 'Comprehensive customer analytics and insights',
      totalCustomers: 'Total Customers',
      active: 'Active',
      inactive: 'Inactive',
      lifetimeValue: 'Avg. Lifetime Value',
      perCustomer: 'Per Customer',
      newCustomers: 'New Customers',
      thisWeek: 'This Week',
      thisMonth: 'This Month',
      loyaltyPoints: 'Loyalty Points',
      avgPoints: 'Avg',
      locationDistribution: 'Location Distribution',
      noData: 'No data available',
      topCustomers: 'Top Customers',
      noCustomers: 'No customer data',
      healthInsights: 'Health Insights',
      withInsurance: 'With Insurance',
      withoutInsurance: 'Without Insurance',
      chronicConditions: 'Chronic Conditions',
      tracked: 'Types Tracked',
      patients: 'patients',
      engagement: 'Customer Engagement',
      activeRate: 'Active Rate',
      avgDays: 'Avg Days Since Visit',
      inactive90: 'Inactive 90d+',
      segmentation: 'Customer Segmentation',
      vip: 'VIP',
      regular: 'Regular',
      basic: 'Basic',
      atRisk: 'At Risk',
      daysInactive: 'days inactive',
      contactPrefs: 'Contact Preferences',
      phone: 'Phone',
      email: 'Email',
      sms: 'SMS/WhatsApp',
      recentActivity: 'Recent Customer Activity',
      satisfaction: 'Satisfaction Metrics',
      repeatRate: 'Repeat Rate',
      avgOrder: 'Avg Order',
      returnRate: 'Return Rate',
      lastVisit: 'Last Visit',
      expand: 'Expand',
      exportCSV: 'Export',
    },
    loyalty: {
      overview: 'Loyalty Program Overview',
      lookup: 'Customer Loyalty Lookup',
      activeCustomers: 'Active Customers',
      totalPoints: 'Total Points Issued',
      avgPoints: 'Avg Points/Customer',
      topTier: 'Top Tier (>1000)',
      distribution: 'Points Distribution',
      trend: 'Points Issued (30 Days)',
      recentActivity: 'Recent Points Activity',
      topCustomers: 'Top Loyalty Customers',
      searchPlaceholder: 'Search by name, code, or phone...',
      startSearch: 'Search for a customer to view loyalty details',
      currentPoints: 'Current Points',
      totalPurchases: 'Total Purchases',
      totalOrders: 'Total Orders',
      avgOrder: 'Avg Order',
      pointsHistory: 'Points History',
      noHistory: 'No purchase history',
      orderPoints: 'Order Pts',
      itemPoints: 'Item Pts',
      totalEarned: 'Total Earned',
      balance: 'Balance',
      notFound: 'Customer not found',
      tryAgain: 'Please check the code and try again',
      order: 'Order',
      pts: 'pts',
      totalSuffix: 'total',
    },
    realTimeSales: {
      title: 'Real-time Sales Monitor',
      todayRevenue: "Today's Revenue",
      totalTransactions: 'Total Transactions',
      itemsSold: 'Items Sold',
      activeCounters: 'Active Counters',
      avgTransaction: 'Avg/Sale',
      vsYesterday: 'vs Yesterday',
      recentSales: 'Recent Transactions',
      salesByHour: 'Sales by Hour',
      topProducts: 'Top Products Today',
      paymentMethods: 'Payment Methods',
      avgValue: 'Average Transaction Value',
      salesVelocity: 'Sales Velocity',
      customerTypes: 'Customer Types',
      registered: 'Registered',
      walkIn: 'Walk-in',
      guest: 'Guest',
      live: 'LIVE',
      avg: 'avg',
      hold: 'hold',
      recentTransactions: 'Recent Transactions',
      filterAll: 'All',
      filterVip: 'VIP',
      filterHighValue: 'High Value',
      vipBadge: 'VIP',
      highValueBadge: 'High Value',
      tableTime: 'Time',
      tableId: 'ID',
      tableItems: 'Items',
      tableTotal: 'Total',
      tableMethod: 'Method',
      tableStatus: 'Status',
      noTransactions: 'No transactions yet today.',
      salesRate: 'Sales Rate',
      invoices: 'Invoices',
      newCust: 'New Cust.',
      perHour: 'hr',
      orders: 'Orders',
      delivery: 'Delivery',
      customers: 'Customers',
      reg: 'Reg.',
      anon: 'Anon.',
      hourlyTrend: 'Hourly Trend',
      byQty: 'by Qty',
      noData: 'No data yet',
      salesByCategory: 'Sales by Category',
      medicine: 'Medicine',
      cosmetic: 'Cosmetic',
      general: 'General',
      noSalesData: 'No sales data yet',
      returnActivity: 'Return Activity',
      returnsToday: 'Today',
      returnsProcessed: 'Returns Processed',
      returnValue: 'Value',
      returnRate: 'Rate',
      revenueBreakdown: 'Revenue Analysis',
      transactionDetails: "Today's Transactions",
      itemAnalysis: 'Inventory Analysis',
      topPerforming: 'Top Performing Products',
      byQuantity: 'By Quantity',
      byRevenue: 'By Revenue',
      tableRank: 'Rank',
      tableName: 'Product Name',
      tableCategory: 'Category',
      tableQtySold: 'Qty Sold',
      tableRevenue: 'Revenue',
      counterPerformance: 'Counter Status',
      counter: 'Counter',
      mainHall: 'Main Hall',
      online: 'ONLINE',
      offline: 'OFFLINE',
      operator: 'Operator',
      unmanned: 'Unmanned',
      todaySales: "Today's Sales",
      cash: 'Cash',
      card: 'Card',
      visa: 'Visa',
      insurance: 'Insurance',
      credit: 'Credit',
      items: 'items',
      justNow: 'Just now',
      minsAgo: 'mins ago',
      hoursAgo: 'hours ago',
      noSales: 'No sales yet today',
      loadMore: 'Load More',
      viewDetails: 'View Details',
      hideDetails: 'Hide Details',
      peakHour: 'Peak Hour',
      slowPeriod: 'Slow Period',
      salesPerHour: 'sales/hour',
    },

    receiptDesigner: {
      title: 'Receipt Designer',
      subtitle: 'Customize templates for your sales receipts',
      activeTemplate: 'Active Template',
      newTemplate: 'New Template',
      newTemplatePlaceholder: 'New Template Name...',
      saveTemplate: 'Save Template',
      deleteTemplate: 'Delete Template',
      setDefault: 'Set as Default',
      printTest: 'Print Test',
      renameTemplate: 'Rename Template',
      printingPreferences: 'Printing Preferences',
      autoPrintDelivery: 'Delivery Only',
      autoPrintAll: 'All Orders',
      autoPrintNone: 'None',
      options: {
        storeName: 'Store Name',
        storeSubtitle: 'Subtitle / Tagline',
        hotline: 'Hotline / Phone',
        address: 'Address Line 1',
        area: 'Area / City',
        footer: 'Footer message',
        inquiry: 'Inquiry / Contact info',
        terms: 'Terms & Conditions',
        font: 'Font Style',
        logo: 'Store Logo (Base64)',
        addressBox: 'Show Address Box',
        delivery: 'Delivery Preview',
        returns: 'Returns Preview',
        useBranchInfo: 'Use Branch Info',
      },
      preview: {
        memorySize: 'Real Printing Memory Size (Actual)',
        payloadSize: 'Raw Data Transfer Size (Payload)',
        saleReceipt: 'Sale Receipt',
        shiftReceipt: 'Shift Receipt',
        previewAsDuplicate: 'Preview as Duplicate',
      },
      storage: {
        full: 'Storage Full!',
        quotaExceeded: 'Storage Quota Exceeded',
        pngSvg: 'PNG / SVG',
        fileTooLarge: 'File too large',
      },
      gallery: {
        title: 'Template Marketplace',
        subtitle: 'Choose a receipt layout that fits your pharmacy.',
        apply: 'Apply Template',
        premium: 'Premium',
        unlock: 'Unlock / Buy',
        free: 'Free',
        layouts: {
          'layout-1': 'Standard (Default)',
          'layout-2': 'Modern Dark',
          'layout-3': 'Compact Minimal',
        },
      },
    },
    printerSettings: {
      title: 'Printer Settings',
      subtitle: 'Configure silent printing with QZ Tray',
      connection: 'Connection',
      enabled: 'QZ Tray Enabled',
      status: {
        connected: 'Connected',
        disconnected: 'Disconnected',
        notInstalled: 'Not Installed',
        connecting: 'Connecting...',
      },
      reconnect: 'Reconnect',
      download: 'Download QZ Tray',
      labelPrinter: 'Label Printer',
      receiptPrinter: 'Receipt Printer',
      selectPrinter: 'Select Printer...',
      noPrinters: 'No printers found',
      refreshList: 'Refresh Printer List',
      testPrintLabel: 'Test',
      testPrintReceipt: 'Test',
      silentMode: 'Silent Mode',
      silentModes: {
        on: 'ON',
        off: 'OFF',
        fallback: 'FALLBACK',
      },
      silentHints: {
        on: 'Always print silently (requires QZ Tray)',
        off: 'Always show print dialog',
        fallback: 'Silent if QZ available, dialog otherwise',
      },
      alerts: {
        connected: 'QZ Tray connected successfully',
        disconnected: 'QZ Tray disconnected',
        printSuccess: 'Printed successfully',
        printFailed: 'Print failed',
        notInstalled: 'QZ Tray is not installed. Please download from qz.io',
      },
    },
    desktop: {
      title: 'Desktop Settings',
      subtitle: 'Manage desktop-specific features and integration',
      printer: 'Thermal Printer',
      autoUpdate: 'Auto Update',
      checkUpdates: 'Check for Updates',
      version: 'App Version',
      systemInfo: 'System Information',
      os: 'Operating System',
      arch: 'Architecture',
      memory: 'Memory',
      upToDate: 'App is up to date',
      updateAvailable: 'Update available',
      downloading: 'Downloading update...',
      noUpdates: 'No updates available',
      errorChecking: 'Error checking for updates',
      installNow: 'Install and Restart',
      preferredInterface: 'Preferred Printer Interface',
      interfaceAuto: 'Auto — Try Tauri first, fall back to QZ Tray',
      interfaceTauri: 'Tauri Native — Use system thermal printer API only',
      interfaceQz: 'QZ Tray — Network-based silent printing',
      receiptPrinterHelper: 'Standard 80mm thermal receipts. Works with Tauri Native and QZ Tray.',
      labelPrinterHelper: 'Small adhesive labels (e.g. 38×12mm). QZ Tray recommended for custom sizes.',
      linuxWarning: 'Thermal printers via Tauri Native may have limited support on Linux. QZ Tray is recommended for best stability.',
      windowsInfo: 'Windows is fully supported. QZ Tray recommended for silent label printing.',
      osVersion: 'OS Version',
    },
    employeeList: {
      title: 'Employee List',
      subtitle: 'Manage pharmacy staff',
      globalView: 'Global View',
      addEmployee: 'Add Employee',
      editEmployee: 'Edit Employee',
      viewDetails: 'Employee Details',
      employeeProfile: 'Employee Profile',
      stats: {
        totalSales: 'Total Sales',
        netProfit: 'Net Profit',
        itemsSold: 'Units Sold',
        transactions: 'Transactions',
        topProduct: 'Top Product',
        performanceInsight: 'Performance Insight',
        salesAnalytics: 'Sales Analytics',
        dispensingAccuracy: 'Dispensing Accuracy',
        dispensingAccuracySub: 'Patient safety & error prevention',
        chronicCare: 'Chronic Patient Care',
        chronicCareSub: 'Loyalty & counseling rate',
        counselingSpeed: 'Patient Service Speed',
        counselingSpeedSub: 'Average customer processing time',
      },
      deleteConfirm: 'Delete this employee?',
      name: 'Full Name',
      phone: 'Phone Number',
      email: 'Email Address',
      position: 'Position',
      department: 'Department',
      branch: 'Branch',
      unassigned: 'Unassigned',
      role: 'System Role',
      status: 'Status',
      salary: 'Salary',
      notes: 'Notes',
      emptyBio: 'No Bio',
      username: 'Username',
      usernamePlaceholder: 'Login Username',
      password: 'Password',
      passwordPlaceholder: 'Login Password',
      table: {
        code: 'Code',
        name: 'Name',
        position: 'Position',
        department: 'Department',
        phone: 'Phone',
        status: 'Status',
        branch: 'Branch',
        actions: 'Actions',
      },
      departments: {
        sales: 'Sales',
        pharmacy: 'Pharmacy',
        marketing: 'Marketing',
        hr: 'Human Resources',
        it: 'IT',
        logistics: 'Logistics',
      },
      roles: {
        admin: 'System Admin',
        pharmacist_owner: 'Pharmacy Owner',
        pharmacist_manager: 'Pharmacy Manager',
        pharmacist: 'Pharmacist',
        assistant: 'Pharmacy Assistant',
        inventory_officer: 'Inventory Officer',
        hr_manager: 'HR Manager',
        senior_cashier: 'Senior Cashier',
        cashier: 'Cashier',
        manager: 'Branch Manager',
        delivery: 'Delivery Courier',
        delivery_pharmacist: 'Medical Delivery',
        officeboy: 'Support Staff',
      },
      statusOptions: {
        all: 'All',
        active: 'Active',
        inactive: 'Inactive',
        holiday: 'On Holiday',
        pending: 'Pending',
      },
      modal: {
        save: 'Save Employee',
        cancel: 'Cancel',
        deleteTitle: 'Delete Employee',
        successAdd: 'Employee added successfully',
        successUpdate: 'Employee updated successfully',
        successDelete: 'Employee deleted successfully',
        forgotPassword: 'Forgot Password?',
        sendResetLink: 'Send Reset Link',
        generateTempPassword: 'Generate Temporary Password',
        tempPasswordGenerated: 'Temporary password generated and saved!',
        resetLinkSent: 'Reset link sent to employee email!',
        confirmReset: 'Are you sure you want to reset the password for this employee?',
        tempPasswordLabel: 'Temporary Password:',
      },
      tabs: {
        general: 'General',
        credentials: 'Credentials',
        documents: 'Documents',
        achievements: 'Achievements',
        experience: 'Work Experience',
        cv: 'CV',
      },
      changePhoto: 'Change Photo',
      removePhoto: 'Remove Image',
      newEmployee: 'New Employee',
      nameArabicLabel: 'Full Name (Arabic)',
      nameArabicPlaceholder: 'Full Name in Arabic...',
      notesPlaceholder: 'Additional notes...',
      passkey: 'Passkey',
      confirmPasswordPrompt: 'Please enter password to confirm:',
      incorrectPassword: 'Incorrect password',
      deletePasskeyConfirm: 'Delete current passkey for this employee?',
      passkeyUnsupported:
        'Browser does not support Passkeys. Ensure you are on HTTPS or Localhost.',
      passkeySet: 'Passkey Set',
      setupPasskey: 'Setup Passkey',
      changePassword: 'Change Password',
      verified: 'Verified',
      verifyIdentity: 'Enter current password to verify your identity',
      currentPassword: 'Current Password',
      enterCurrentPassword: 'Enter current password',
      enterPassword: 'Enter password',
      verify: 'Verify',
      emailRequiredForReset: 'Employee must have an email address first to reset',
      resetViaEmail: 'Reset via Email?',
      verifiedSuccess: 'Verified! You can now enter the new password',
      newPassword: 'New Password',
      enterNewPassword: 'Enter new password',
      noPasswordWarning: 'This employee has no password. Add a new password above.',
      officialDocuments: 'Official Documents',
      nationalIdCard: 'National ID Card',
      uploadFront: 'Upload Front',
      sub: 'Sub',
      syndicateCards: 'Syndicate Cards',
      mainSyndicateCard: 'Main Syndicate Card',
      subSyndicate: 'Sub Syndicate',
      frontFace: 'Front Face',
      backFace: 'Back Face',
      noIdCardImages: 'No ID card images uploaded',
      noSyndicateImages: 'No syndicate cards uploaded',
      fingerprintEnabled: 'Fingerprint Enabled',
      registeredCredentials: 'Registered Login Credentials',
      usernameTaken:
        'This username is already taken by another employee. Please choose a different one.',
      searchPlaceholder: 'Search employees...',
      emptyMessage: 'No employees found',
      generalInfo: 'General Info',
      contactInfo: 'Contact Information',
      joined: 'Joined',
      shift: 'Shift',
      license: 'License',
      address: 'Address',
      workingHours: 'Working Hours',
      attendance: 'Attendance',
      rating: 'Rating',
      totalSalesAmount: 'Total Sales Amount',
      sales: 'Sales',
      employeeId: 'Employee ID',
      workHistory: {
        present: 'Present',
        empty: 'No work history registered',
        currentBranch: 'Current Branch',
        branchOfOrg: '{branchName} - {orgName}',
        branchOfOrgCurrent: '{branchName} - {orgName} (Current Branch)',
        pharmacy: 'Pharmacy {orgName}',
        pharmacyCurrent: 'Pharmacy {orgName} (Current Pharmacy)',
        branchCurrent: '{branchName} (Current Branch)',
      },
      navTabs: {
        overview: 'Overview',
        employees: 'Employees',
        profile: 'Profile',
      },
    },
    intelligence: {
      dashboard: {
        title: 'Intelligence',
        subtitle: 'Advanced inventory and sales analysis',
        tabs: {
          procurement: 'Procurement',
          financials: 'Financials',
          risk: 'Risk',
          audit: 'Audit',
        },
        loading: 'Loading data...',
      },
      common: {
        pagination: {
          showing: 'Showing',
          of: 'of',
          prev: 'Previous',
          next: 'Next',
        },
      },
      audit: {
        title: 'Audit Log',
        searchPlaceholder: 'Search transactions...',
        exportCSV: 'Export CSV',
        emptyMessage: 'No transactions recorded yet',
        columns: {
          timestamp: 'Time',
          type: 'Type',
          product: 'Product',
          quantity: 'Quantity',
          amount: 'Amount',
          employee: 'Employee',
          notes: 'Notes',
        },
        grid: {
          columns: {
            date: 'Date',
            type: 'Type',
            product: 'Product',
            qty: 'Qty',
            amount: 'Amount',
            user: 'User',
          },
        },
        details: {
          title: 'Transaction Details',
          dateTime: 'Date & Time',
          totalAmount: 'Total Amount',
          additionalDetails: 'Additional Details',
          anomalyWarning: 'Warning',
          anomalyDetected: 'Anomaly detected in this transaction',
        },
        types: {
          sale: 'Sale',
          return: 'Return',
          void: 'Void',
          adjustment: 'Adjustment',
          edit: 'Edit',
        },
        alert: 'Alert',
      },
      financials: {
        filters: {
          period: 'Period',
          select: 'Select',
          export: 'Export Report',
          searchPlaceholder: 'Search products or categories...',
          periods: {
            this_month: 'This Month',
            last_month: 'Last Month',
            last_3_months: 'Last 3 Months',
            this_year: 'This Year',
          },
        },
        kpis: {
          revenue: 'Revenue',
          grossProfit: 'Gross Profit',
          margin: 'Margin %',
          unitsSold: 'Units Sold',
          compare: 'vs previous period',
          points: 'points',
        },
        sections: {
          productProfitability: 'Profit Details',
          categoryBreakdown: 'Profit Analysis',
          noData: 'No sales data for this period',
        },
        productGrid: {
          columns: {
            product: 'Product',
            quantitySold: 'Qty Sold',
            revenue: 'Revenue',
            cost: 'Cost',
            profit: 'Profit',
            margin: 'Margin %',
          },
        },
        categoryGrid: {
          columns: {
            category: 'Category',
            products: 'Products',
            revenue: 'Revenue',
            profit: 'Profit',
            margin: 'Margin',
            abcDistribution: 'ABC Dist.',
          },
        },
        profitLoss: {
          title: 'Profit & Loss Report',
          netRevenue: 'Net Revenue',
          grossProfit: 'Gross Profit',
          netCogs: 'Net COGS',
          margin: 'Margin %',
          statement: 'Financial Statement',
          grossSales: 'Gross Sales',
          returns: 'Returns',
          netSales: 'Net Sales',
          costOfSales: 'Cost of Sales (COGS)',
          operatingExpenses: 'Operating Expenses',
          noExpenses: '* No expenses recorded for this period',
          estimatedNetProfit: 'Estimated Net Profit',
          dailyPerformance: 'Daily Revenue Performance',
          salesVsReturns: 'Sales vs Returns',
          sales: 'Sales',
          refunds: 'Refunds',
          profitabilityByCategory: 'Profitability by Category',
          category: 'Category',
          revenue: 'Revenue',
          profit: 'Profit',
          cogs: 'COGS',
          unclassified: 'Unclassified',
        },
      },
      procurement: {
        filters: {
          supplier: 'Supplier',
          category: 'Category',
          select: 'Select',
          all: 'All',
        },
        kpis: {
          needingOrder: 'Needing Order',
          outOfStock: 'Out Of Stock',
          lostSales: 'Lost Sales (Est)',
          pendingPO: 'Pending PO',
          items: 'items',
          vsLastWeek: 'vs last week',
          improved: 'improved',
        },
        actions: {
          generatePO: 'Generate PO',
        },
        empty: {
          title: 'No Products',
          subtitle: 'No products match the selected filters',
        },
        grid: {
          columns: {
            product: 'Product',
            stockStatus: 'Status',
            available: 'Available',
            days: 'days',
            dailySales: 'Daily Sales',
            suggested: 'Suggested',
            confidence: 'Confidence',
            perDay: '/day',
          },
          skipped: 'Skipped',
        },
      },
      risk: {
        kpis: {
          valueAtRisk: 'Value at Risk',
          batchesAtRisk: 'Batches at Risk',
          total: 'Total',
          batch: 'batch',
        },
        actions: {
          createReturn: 'Create Return List',
          applyDiscount: 'Apply Discount Policy',
        },
        empty: {
          title: 'No At-Risk Batches',
          subtitle: 'All inventory products are safe from expiry',
        },
        grid: {
          columns: {
            product: 'Product / Batch',
            daysLeft: 'Days Left',
            quantity: 'Quantity',
            valueAtRisk: 'Value at Risk',
            riskCategory: 'Risk Category',
            recovery: 'Est. Recovery',
            action: 'Action',
          },
          actions: {
            discount50: '50% Off',
            discount25: '25% Off',
            return: 'Return',
            action: 'Action',
          },
          day: 'day',
        },
      },
    },
    stockMovement: {
      title: 'Stock Movement',
      summary: 'Summary',
      showAll: 'Show All',
      todayOnly: 'Specific Drug',
      allProductsReport: 'All Products Report',
      allMovements: 'All Movements',
      details: 'Details',
      stockIn: 'Stock In',
      stockOut: 'Stock Out',
      returns: 'Returns',
      netMovement: 'Net Movement',
      currentStock: 'Current Stock',
      branch: 'Branch',
      allBranches: 'All Branches',
      export: 'Export CSV',
      noMovements: 'No movements found for this drug.',
      searchDrug: 'Search for a drug to view its movements...',
      loading: 'Loading movements...',
      stats: {
        totalIn: 'Total Stock In',
        totalOut: 'Total Stock Out',
        netChange: 'Net Inventory Change',
      },
      table: {
        date: 'Date',
        type: 'Type',
        qty: 'Qty',
        batch: 'Batch',
        reason: 'Reason',
        user: 'User',
        ref: 'Reference',
        prev: 'Prev',
        next: 'Next',
      },
      types: {
        sale: 'Sale',
        purchase: 'Purchase',
        return_customer: 'Sales Return',
        return_supplier: 'Purchase Return',
        adjustment: 'Adjustment',
        damage: 'Damage',
        transfer_in: 'Transfer In',
        transfer_out: 'Transfer Out',
        correction: 'Correction',
        initial: 'Initial Stock',
      },
      filters: {
        dateRange: 'Date Range',
        allTypes: 'All Types',
      },
    },
    expiryManagement: {
      invalidQuantity: 'Invalid quantity',
      actionSuccess: 'Successfully updated stock',
      actions: 'Actions',
      returnToSupplier: 'Return to Supplier',
      damageStock: 'Damage Stock',
      potentialLoss: 'Potential Loss',
      status: {
        valid: 'Valid',
        expired: 'Expired',
      },
      time: {
        days: 'Days',
      },
      details: {
        packs: 'Packs',
      },
      title: 'Expiry Management',
      subtitle: 'Track and manage batch expiration dates alerts',
      expiredItems: 'Expired Items',
      nearExpiry30: 'Expiring < 30 Days',
      nearExpiry90: 'Expiring < 90 Days',
      all: 'All',
      expired: 'Expired',
      searchPlaceholder: 'Search batches...',
      noRecords: 'No batches found matching criteria',
      batch: 'Batch',
      available: 'Available',
      quantity: 'Quantity',
      notes: 'Notes',
      optional: 'Optional',
      addNotes: 'Add any reason or details...',
      cancel: 'Cancel',
      confirm: 'Confirm',
      table: {
        codeOrBatch: 'Code / Batch',
        name: 'Name',
        remainingQty: 'Remaining Qty',
        costPrice: 'Cost Price',
        potentialLoss: 'Potential Loss',
        expiryDate: 'Expiry Date',
      },
    },
    // ═══════════════════════════════════════════
    // Attendance Module
    // ═══════════════════════════════════════════
    attendance: {
      title: 'Attendance Terminal',
      subtitle: 'Biometric attendance tracking',
      reportsTitle: 'Attendance Reports',
      reportsSubtitle: 'Monitor staff presence and punctuality',
      showPresentOnly: 'Show Present Only',
      profileSubtitle: 'Review monthly presence and metrics',
      // Terminal States
      terminalLocked: 'Terminal Locked',
      enterToken: 'Enter Terminal Token',
      tokenPlaceholder: 'Paste terminal token here...',
      activateTerminal: 'Activate Terminal',
      invalidToken: 'Invalid terminal token. Please check with the administrator.',
      terminalNotActivated: 'Attendance terminal is not activated.',
      contactAdmin: 'Contact the administrator to activate it.',
      accessDenied: 'You do not have permission to access the attendance terminal.',
      // Clock Actions
      clockIn: 'Clock In',
      clockOut: 'Clock Out',
      selectEmployee: 'Select Employee',
      searchEmployee: 'Search employee...',
      // Timeline
      todayTimeline: "Today's Timeline",
      noEventsToday: 'No attendance events recorded today.',
      eventIn: 'Clock In',
      eventOut: 'Clock Out',
      // Status
      currentlyIn: 'Currently Clocked In',
      currentlyOut: 'Currently Clocked Out',
      notClockedIn: 'Not Clocked In Yet',
      // Biometric
      touchSensor: 'Touch the fingerprint sensor',
      biometricVerified: 'Biometric verified',
      biometricFailed: 'Biometric verification failed',
      noBiometric: 'No biometric registered for this employee',
      // Branch Settings
      terminalToken: 'Attendance Terminal Token',
      generateToken: 'Generate New Token',
      regenerateToken: 'Regenerate Token',
      tokenGenerated: 'New terminal token generated successfully',
      tokenWarning: 'Generating a new token will immediately invalidate the previous one.',
      copyToken: 'Copy Token',
      tokenCopied: 'Token copied to clipboard',
      shiftStartDesc: 'Define work start time to calculate lateness',
      saveTime: 'Save Time',
      terminalTokenDesc: 'Terminal activation token for this branch',
      generateTokenHint:
        'Generate a token, copy it, and paste it on the attendance terminal device to activate it',
      // Success/Error
      clockInSuccess: 'Clocked in successfully',
      clockOutSuccess: 'Clocked out successfully',
      clockError: 'Failed to record attendance',
      // PIN Fallback
      enterPin: 'Enter PIN',
      pinPlaceholder: '4-digit PIN',
      invalidPin: 'Invalid PIN',
      setPin: 'Set Attendance PIN',
      unverifiedClock: 'Unverified (no biometric or PIN)',
      pinOrBiometric: 'Use biometric or enter PIN',
      noPinNoBiometric: 'No biometric or PIN registered',
      fingerprintOnly: 'Biometric verification is required for this employee',
      // Working Hours
      totalHours: 'Total Hours',
      ongoing: 'Ongoing',
      sessionDuration: 'Session Duration',
      // Reports & Lateness
      shiftStartTime: 'Shift Start Time',
      graceMinutes: 'Grace Period (minutes)',
      attendanceReports: 'Attendance Reports',
      present: 'Present',
      absent: 'Absent',
      late: 'Late',
      avgHours: 'Average Hours',
      firstIn: 'First In',
      lastOut: 'Last Out',
      lateStatus: 'Status',
      lateMinutes: 'Late (min)',
      noReportData: 'No attendance data for this date.',
      employeeCode: 'Code',
      employeeName: 'Employee',
      lateStatusOk: 'On Time',
      lateStatusLate: 'Late',
      // Employee Attendance Profile
      attendanceProfile: 'Attendance Profile',
      attendanceRate: 'Attendance Rate',
      totalHoursMonth: 'Total Hours',
      lateDays: 'Late Days',
      absentDays: 'Absent Days',
      day: 'Day',
      date: 'Date',
      noDataFound: 'No Data Found',
      employeeNotFound: 'Employee not found',
      quickAttendance: 'Quick Attendance',
      usernamePlaceholder: 'Username...',
      authFailed: 'Auth Failed',
      hourSymbol: 'hrs',
      minuteSymbol: 'm',
      backToReports: 'Back to Reports',
      avgDailyHours: 'Avg Daily',
      totalLateMinutes: 'Total Late',
      nextDay: '+1',
    },
    shortages: {
      title: 'Shortages & Predictive Alerts',
      subtitle: 'AI-driven inventory optimization and low-stock predictions',
      statsTotal: 'Monitored Products',
      statsCritical: 'Out of Stock',
      statsPredictive: 'Predictive Warnings',
      statsHealthy: 'Healthy Stock',
      searchPlaceholder: 'Search by drug name, active ingredient or barcode...',
      filterCategory: 'All Categories',
      filterAlert: 'All Alert Types',
      tableProduct: 'Name',
      tableStock: 'Current Stock',
      tableMinStock: 'Manual Limit',
      tableVelocity: 'Avg Daily Sales',
      tableStockDays: 'Days Left',
      tableSuggestedQty: 'Suggested Order',
      tableAbcClass: 'ABC Priority',
      tableAlertStatus: 'Alert Status',
      tableActions: 'Actions',
      reorderBtn: 'Create Purchase Order',
      statusOutOfStockSold: 'Sold Recently',
      statusManualMinimumReached: 'Below Limit',
      statusPredictiveShortage: 'Predictive Shortage',
      statusOutOfStockDefault: 'Inactive',
      statusNormal: 'Normal',
      abcDescription:
        'ABC Analysis ranks products by sales velocity: A = Top 80% contribution, B = Mid 15%, C = Low 5%',
      predictedRunoutText: 'Predicted runout in {days} days',
      suggestedOrderTooltip: 'Calculated as: 14 Days Velocity × 1.5 Safety Margin - Current Stock',
      noShortagesFound: 'No shortages or alerts found for this branch. Stock levels are healthy!',
      warningNoDataToExportTitle: 'No data to export!',
      warningNoDataToExportDesc: 'Please select drugs or ensure there are shortages.',
      exportSuccess: 'Shortages list exported successfully!',
      exportSuccessDesc: 'Downloaded CSV containing {count} products.',
      warningNoItemsSelectedTitle: 'No items selected!',
      warningNoItemsSelectedDesc: 'Please select the drugs you want to reorder first.',
      navigatingToPurchases: 'Navigating to Purchases...',
      navigationError: 'Navigation error',
      navigationErrorDesc:
        'Sorry, we could not resolve the navigation controller for purchases page.',
      preparedOrderDesc: 'Prepared purchase order draft containing {count} items.',
      exportButtonLabel: 'Export Shortages',
      exportSelectedButtonLabel: 'Export Selected',
      units: 'units',
      daysLeft: 'days left',
      itemsSelectedText: '{count} items selected',
      clearSelection: 'Clear Selection',
      focusModeToggle: 'Focus Mode (Selected Only)',
      allGoodText: 'Stock levels are perfect!',
      exportCsvFilename: 'shortages_report',
      csvHeaderProduct: 'Product Name',
      csvHeaderGeneric: 'Generic Name',
      csvHeaderCategory: 'Category',
      generalCategory: 'General',
      weekAbbreviation: 'wk',
    },
    newTests: {
      title: 'Scrollbar Lab',
      subtitle: 'Premium testing ground for custom scrollbars and UI patterns',
      description:
        'An interactive playground built for developers to test and copy advanced custom scrollbar styles.',
      welcome: 'Welcome to the Scrollbar Lab',
      getStarted: 'Ready to write your next feature?',
      designAesthetic: 'Premium Design Language Active',
    },
    a5InvoiceDesigner: {
      title: 'A5 Purchase Invoice Designer',
      subtitle: 'Design, preview, and test print high-fidelity A5 pharmaceutical invoices',
      description:
        'An interactive design lab to customize A5 templates for major Egyptian drug suppliers and print them with perfect page scaling.',
    },
    expenses: {
      title: 'Expense Tracker',
      recordExpense: 'Record Expense',
      filters: {
        today: 'Today',
        thisWeek: 'This Week',
        thisMonth: 'This Month',
        custom: 'Custom Range',
      },
      summary: {
        total: 'Total Expenses',
        cashTotal: 'Cash Expenses',
        count: 'Transactions',
        largestCategory: 'Largest Category',
      },
      categories: {
        utilities: 'Utilities',
        rent: 'Rent',
        maintenance: 'Maintenance',
        supplies: 'Supplies',
        petty_cash: 'Petty Cash',
        transportation: 'Transportation',
        salaries: 'Salaries',
        misc: 'Miscellaneous',
      },
      modal: {
        title: 'Record New Expense',
        amount: 'Amount',
        category: 'Category',
        description: 'Description',
        paymentMethod: 'Payment Method',
        cash: 'Cash',
        bank_transfer: 'Bank Transfer',
        card: 'Card',
        confirm: 'Record Expense',
        cancel: 'Cancel',
        shiftBalance: 'Current Shift Cash',
        noShiftWarning:
          "No active shift found. Expense will be logged but won't affect the cash register.",
      },
      validation: {
        amountRequired: 'Amount is required',
        positiveAmount: 'Amount must be greater than 0',
        descriptionRequired: 'Description is required',
        insufficientBalance: 'Expense amount exceeds available cash above shift base balance',
      },
      table: {
        time: 'Time',
        id: 'Ref',
        category: 'Category',
        description: 'Description',
        paymentMethod: 'Payment Method',
        amount: 'Amount',
        employee: 'Recorded By',
      },
      empty: 'No expenses recorded for this period',
    },
    dashboardExperiments: {
      title: 'Dashboard Card Experiments',
      card1: 'Sales Trend (Line Chart)',
      card2: 'Revenue by Category (Bar Chart)',
      card3: 'Customer Segments (Pie Chart)',
      card4: 'Stock Status (Donut Chart)',
      card5: 'Daily Traffic (Area Chart)',
      card6: 'Performance Goals (Radial Bar)',
      card7: 'Task Completion (Progress Bars)',
      card8: 'Quick Stat (with Sparkline)',
      card9: 'Weekly Activity (Heatmap)',
      card10: 'Price vs Sales (Scatter Plot)',
      card11: 'Monthly Sales (Stacked Bar)',
      card12: 'Sales Funnel',
      card13: 'Target Gauge',
      card14: 'Top Products (Horizontal Bar)',
      card15: 'Revenue vs Orders (Composed)',
      totalRevenue: 'Total Revenue',
      vsLastMonth: 'vs last month',
      morning: 'Morning',
      afternoon: 'Afternoon',
      evening: 'Evening',
      targetAchieved: 'Target Achieved',
      salesTarget: 'Sales Target',
      customerSatisfaction: 'Customer Satisfaction',
      inventoryTurnover: 'Inventory Turnover',
    },
    employeeProfile: {
      profile: 'Profile',
      workHistory: 'History',
      documents: 'Documents',
      sessions: 'Sessions',
      independentEmployee: 'Independent Employee',
      personalInformation: 'Personal Information',
      cancel: 'Cancel',
      save: 'Save',
      edit: 'Edit',
      fullName: 'Full Name',
      nameArabic: 'Name (Arabic)',
      loginAndFingerprint: 'Login & Fingerprint Credentials',
      workspacesAndCredentials: 'Workspaces & Credentials',
      duration: 'Duration',
      achievementsAndNotes: 'Achievements & Notes',
      changePassword: 'Change Password',
      newPassword: 'New Password',
      setupPasskey: 'Setup Passkey',
      days: 'days',
      months: 'months',
      years: 'years',
      and: '&',
      unknownBranch: 'Unknown Branch',
      noFingerprint: 'No Fingerprint',
      fingerprintEnabled: 'Fingerprint Enabled',
      username: 'Username',
      email: 'Email',
      phone: 'Phone',
      licenseNo: 'License No.',
      memberSince: 'Member Since',
      overview: 'Overview',
      employers: 'Employers',
      pending: 'Pending',
      totalRequests: 'Total Requests',
      officialDocuments: 'Official Documents',
      nationalIdFront: 'National ID (Front)',
      nationalIdBack: 'National ID (Back)',
      syndicateCard: 'Syndicate Card',
      subSyndicateCard: 'Sub Syndicate Card',
      noWorkHistory: 'No Work History Yet',
      pendingRequests: 'Requests',
      noFile: 'No file',
      operationFailed: 'Operation failed',
      sentBy: 'Sent by:',
      rejecting: 'Rejecting...',
      accepting: 'Accepting...',
      darkMode: 'Dark Mode',
      signOut: 'Sign Out',
      language: 'Language',
      menu: 'Menu',
      prescriptionPricing: 'Pricing',
      copied: 'Copied ✓',
      noPendingRequestsDesc:
        'When a pharmacy sends you an employment request, it will appear here. Accept it to gain access to their management system.',
      noWorkHistoryDesc:
        'When you accept an employment request from a pharmacy, it will appear here in your work history.',
      fileTooLarge: 'File too large (max {{size}}KB)',
    },
    employeeSetup: {
      expired: 'Expired',
      invitationRejected: 'Invitation was rejected by the user',
      pleaseEnterUsername: 'Please enter Email or Username',
      userNotFound: 'User not found. They must create an account first.',
      errorOccurred: 'An error occurred',
      failedToCancel: 'Failed to cancel, it might have been accepted already',
      fillRequiredFields: 'Please fill all required fields',
      passwordsDoNotMatch: 'Passwords do not match',
      passwordLength: 'Password must be at least 4 characters',
      goBack: 'Go Back',
      accountSetup: 'Account Setup',
      inviteAdminInstructions:
        'Send an invitation to the pharmacy administrator or manager to complete the setup and access the system.',
      globalEmailOrUsername: 'Global Email or Username',
      emailExample: 'e.g. ahmed@email.com',
      autoAcceptNote: 'If you enter your current email, the invite will auto-accept.',
      role: 'Role',
      expirationTime: 'Expiration Time',
      oneHour: '1 Hour',
      oneDay: '24 Hours',
      noLimit: 'No Limit',
      sendInvitation: 'Send Invitation',
      invitationExpiresIn: 'Invitation expires in:',
      cancelInvitation: 'Cancel Invitation',
      accepted: 'Accepted',
      localUsernameAuto: 'Local Username (Auto)',
      quickLoginNote: 'This is the unified quick login code for the branch. It cannot be changed.',
      localPassword: 'Local Password',
      confirmPassword: 'Confirm',
      finishAndSave: 'Finish & Save',
      step3Admin: 'Step 3: Invite the first admin for this branch to start using the system.',
      cannotInviteSelf:
        'Cannot invite the admin account. Please invite a separate employee account.',
      waitingForAcceptance: 'Waiting for acceptance from',
    },
    secureGate: {
      identityVerification: 'Identity Verification',
      tooManyAttempts: 'Too Many Attempts',
      passwordRequired: 'Password Verification',
      biometricRequired: 'Biometric Verification',
      exceededAttempts: 'You have exceeded the maximum attempts.',
      enterPassword: 'Please enter your password to continue',
      incorrectPassword: 'Incorrect password',
      unlockArea: 'Unlock Area',
      protectedArea: 'Protected Area',
      requiresPassword: 'This area requires authentication to access its contents',
      unlockNow: 'Unlock Now',
      authorizedTitle: 'Access Authorized',
      authorizedDesc: 'You have the necessary permissions to access this area.',
      unauthorizedTitle: 'Access Denied',
      unauthorizedDesc:
        'You do not have permission to access this area. Access is restricted to managers and owners.',
      confirmUnlock: 'Confirm Identity',
      lockoutTimer: 'Locked out.. retry in {{timeLeft}}s',
      usernamePlaceholder: 'Username',
      passwordPlaceholder: 'Password',
      verifying: 'Verifying...',
      confirmUnlockBtn: 'Confirm Unlock',
      useBiometrics: 'Use Biometrics instead',
      scanBiometric: 'Scan Fingerprint to Unlock',
      usePassword: 'Use Password instead',
      usernameRequiredError: 'Please enter username',
      passwordRequiredError: 'Please enter password',
      usernameNotFoundError: 'Username not found',
      unauthorizedErrorMessage: 'You do not have permission to access this area',
      verificationFailedError: 'Identity verification failed. Please try again.',
      biometricFailedError: 'Biometric verification failed. Please try again.',
    },
    updater: {
      updateAvailableTitle: 'New Update Available',
      updateAvailableMsg: 'A new update (v{{version}}) is ready to install.',
      restartNow: 'Restart Now',
      updateNow: 'Update Now',
      later: 'Later',
      downloading: 'Downloading update in background...',
      appVersion: 'App Version',
      error: 'Failed to update application.',
    },
  },
  AR: {
    common: {
      loading: 'جاري التحميل...',
      edit: 'تعديل',
      save: 'حفظ',
      cancel: 'إلغاء',
      delete: 'حذف',
      selectMonth: 'اختر الشهر',
      close: 'إغلاق',
      actions: 'إجراءات',
      am: 'ص',
      pm: 'م',
      add: 'إضافة',
      reset: 'إعادة تعيين',
      confirmReset: 'تأكيد إعادة التعيين',
      search: 'بحث',
    },
    employeeList: {
      title: 'قائمة الموظفين',
      subtitle: 'إدارة طاقم الصيدلية',
      globalView: 'العرض العام',
      addEmployee: 'إضافة موظف',
      editEmployee: 'تعديل موظف',
      viewDetails: 'تفاصيل الموظف',
      employeeProfile: 'ملف الموظف',
      stats: {
        totalSales: 'إجمالي المبيعات',
        netProfit: 'صافي الربح',
        itemsSold: 'القطع المباعة',
        transactions: 'عدد الفواتير',
        topProduct: 'المنتج الأكثر مبيعاً',
        performanceInsight: 'تحليل الأداء',
        salesAnalytics: 'تحليل المبيعات',
        dispensingAccuracy: 'دقة صرف الأدوية',
        dispensingAccuracySub: 'سلامة المرضى والحد من الأخطاء',
        chronicCare: 'رعاية الحالات المزمنة',
        chronicCareSub: 'إرشاد ومتابعة مرضى الضغط والسكري',
        counselingSpeed: 'سرعة خدمة المرضى',
        counselingSpeedSub: 'متوسط زمن معالجة وتجهيز الطلب',
      },
      deleteConfirm: 'حذف هذا الموظف؟',
      name: 'الاسم بالكامل',
      phone: 'رقم الهاتف',
      email: 'البريد الإلكتروني',
      position: 'المسمى الوظيفي',
      department: 'القسم',
      branch: 'الفرع',
      unassigned: 'غير محدد',
      role: 'الدور في النظام',
      status: 'الحالة',
      salary: 'الراتب',
      notes: 'ملاحظات',
      emptyBio: 'تيمبليت فاضي',
      username: 'اسم المستخدم',
      usernamePlaceholder: 'اسم المستخدم للدخول',
      password: 'كلمة المرور',
      passwordPlaceholder: 'كلمة المرور للدخول',
      table: {
        code: 'الكود',
        name: 'الاسم',
        position: 'الوظيفة',
        department: 'القسم',
        phone: 'الهاتف',
        status: 'الحالة',
        branch: 'الفرع',
        actions: 'إجراءات',
      },
      departments: {
        sales: 'المبيعات',
        pharmacy: 'الصيدلية',
        marketing: 'التسويق',
        hr: 'الموارد البشرية',
        it: 'تقنية المعلومات',
        logistics: 'اللوجستيات / التوصيل',
      },
      roles: {
        admin: 'مدير النظام (Admin)',
        pharmacist_owner: 'مالك الصيدلية',
        pharmacist_manager: 'مدير الصيدلية',
        pharmacist: 'صيدلي',
        assistant: 'مساعد صيدلي',
        inventory_officer: 'مسؤول مخزون',
        hr_manager: 'مدير موارد بشرية',
        senior_cashier: 'كاشير أول',
        cashier: 'كاشير',
        manager: 'مدير فرع',
        delivery: 'مندوب توصيل',
        delivery_pharmacist: 'صيدلي توصيل (رعاية منزلية)',
        officeboy: 'عامل دعم',
      },
      statusOptions: {
        all: 'الكل',
        active: 'نشط',
        inactive: 'غير نشط',
        holiday: 'في إجازة',
        pending: 'معلق',
      },
      modal: {
        save: 'حفظ الموظف',
        cancel: 'إلغاء',
        deleteTitle: 'حذف الموظف',
        successAdd: 'تم إضافة الموظف بنجاح',
        successUpdate: 'تم تحديث بيانات الموظف بنجاح',
        successDelete: 'تم حذف الموظف بنجاح',
        forgotPassword: 'نسيت كلمة المرور؟',
        sendResetLink: 'إرسال رابط إعادة التعيين',
        generateTempPassword: 'إنشاء كلمة مرور مؤقتة',
        tempPasswordGenerated: 'تم إنشاء وحفظ كلمة مرور مؤقتة!',
        resetLinkSent: 'تم إرسال رابط إعادة التعيين لبريد الموظف!',
        confirmReset: 'هل أنت متأكد من رغبتك في إعادة تعيين كلمة مرور هذا الموظف؟',
        tempPasswordLabel: 'كلمة المرور المؤقتة:',
      },
      tabs: {
        general: 'معلومات عامة',
        credentials: 'بيانات الدخول',
        documents: 'المستندات',
        achievements: 'الإنجازات',
        experience: 'الخبرة المهنية',
        cv: 'السيرة الذاتية (CV)',
      },
      changePhoto: 'تغيير الصورة',
      removePhoto: 'إزالة الصورة',
      newEmployee: 'موظف جديد',
      nameArabicLabel: 'الاسم بالكامل (عربي)',
      nameArabicPlaceholder: 'الاسم بالكامل باللغة العربية...',
      notesPlaceholder: 'ملاحظات إضافية...',
      passkey: 'مفتاح المرور (Passkey)',
      confirmPasswordPrompt: 'يرجى إدخال كلمة المرور للتأكيد:',
      incorrectPassword: 'كلمة المرور غير صحيحة',
      deletePasskeyConfirm: 'هل تريد حذف مفتاح المرور الحالي لهذا الموظف؟',
      passkeyUnsupported:
        'هذا المتصفح لا يدعم مفاتيح المرور (Passkeys). تأكد من استخدام HTTPS أو Localhost.',
      passkeySet: 'تم ضبط المفتاح',
      setupPasskey: 'إعداد مفتاح المرور',
      changePassword: 'تغيير كلمة المرور',
      verified: 'تم التحقق',
      verifyIdentity: 'أدخل كلمة المرور الحالية للتحقق من هويتك',
      currentPassword: 'كلمة المرور الحالية',
      enterCurrentPassword: 'أدخل كلمة المرور الحالية',
      enterPassword: 'أدخل كلمة المرور',
      verify: 'تحقق',
      emailRequiredForReset: 'يجب إضافة بريد إلكتروني للموظف أولاً للاستعادة',
      resetViaEmail: 'استعادة بالإيميل؟',
      verifiedSuccess: 'تم التحقق! يمكنك الآن إدخال كلمة المرور الجديدة',
      newPassword: 'كلمة المرور الجديدة',
      enterNewPassword: 'أدخل كلمة المرور الجديدة',
      noPasswordWarning: 'هذا الموظف ليس لديه كلمة مرور. أضف كلمة مرور جديدة من الأعلى.',
      officialDocuments: 'المستندات الرسمية',
      nationalIdCard: 'البطاقة الشخصية',
      uploadFront: 'اضغط لرفع الوجه الأمامي',
      sub: 'الفرعية',
      syndicateCards: 'كارنيهات النقابة',
      mainSyndicateCard: 'كارنية النقابة الرئيسية',
      subSyndicate: 'النقابة الفرعية',
      frontFace: 'الوجه الأمامي',
      backFace: 'الوجه الخلفي',
      noIdCardImages: 'لا يوجد صور للبطاقة',
      noSyndicateImages: 'لا يوجد صور للكارنيهات',
      fingerprintEnabled: 'البصمة مفعلة',
      registeredCredentials: 'بيانات الدخول المسجلة',
      usernameTaken: 'اسم المستخدم هذا مسجل مسبقاً لموظف آخر. يرجى اختيار اسم مختلف.',
      searchPlaceholder: 'بحث عن موظف...',
      emptyMessage: 'لم يتم العثور على موظفين',
      generalInfo: 'معلومات عامة',
      contactInfo: 'بيانات الاتصال',
      joined: 'تاريخ الانضمام',
      shift: 'الوردية',
      license: 'الترخيص',
      address: 'العنوان',
      workingHours: 'ساعات العمل',
      attendance: 'نسبة الحضور',
      rating: 'التقييم',
      totalSalesAmount: 'إجمالي قيمة المبيعات',
      sales: 'المبيعات',
      employeeId: 'رقم الموظف',
      workHistory: {
        present: 'حتى الآن',
        empty: 'لا يوجد سجل خبرة مسجل',
        currentBranch: 'الفرع الحالي',
        branchOfOrg: '{branchName} - {orgName}',
        branchOfOrgCurrent: '{branchName} - {orgName} (الفرع الحالي)',
        pharmacy: 'صيدلية {orgName}',
        pharmacyCurrent: 'صيدلية {orgName} (الصيدلية الحالية)',
        branchCurrent: '{branchName} (الفرع الحالي)',
      },
      navTabs: {
        overview: 'نظرة عامة',
        employees: 'قائمة الموظفين',
        profile: 'ملف الموظف',
      },
    },
    intelligence: {
      dashboard: {
        title: 'ذكاء الأعمال (Intelligence)',
        subtitle: 'تحليل متقدم للمخزون والمبيعات لاتخاذ قرارات أذكى',
        tabs: {
          procurement: 'المشتريات',
          financials: 'المالية',
          risk: 'المخاطر',
          audit: 'التدقيق',
        },
        loading: 'جاري تحميل البيانات...',
      },
      common: {
        pagination: {
          showing: 'عرض',
          of: 'من',
          prev: 'السابق',
          next: 'التالي',
        },
      },
      audit: {
        title: 'سجل تدقيق العمليات (Audit Log)',
        searchPlaceholder: 'بحث في العمليات...',
        exportCSV: 'تصدير CSV',
        emptyMessage: 'لا توجد عمليات مسجلة بعد',
        columns: {
          timestamp: 'التوقيت',
          type: 'نوع العملية',
          product: 'المنتج',
          quantity: 'الكمية',
          amount: 'القيمة',
          employee: 'الموظف',
          notes: 'ملاحظات',
        },
        grid: {
          columns: {
            date: 'التاريخ',
            type: 'النوع',
            product: 'المنتج',
            qty: 'الكمية',
            amount: 'القيمة',
            user: 'المستخدم',
          },
        },
        details: {
          title: 'تفاصيل العملية',
          dateTime: 'التاريخ والوقت',
          totalAmount: 'القيمة الإجمالية',
          additionalDetails: 'تفاصيل إضافية',
          anomalyWarning: 'تنبيه',
          anomalyDetected: 'تم اكتشاف حالة غير طبيعية في هذه العملية',
        },
        types: {
          sale: 'بيع',
          return: 'إرجاع',
          void: 'إلغاء',
          adjustment: 'تعديل',
          edit: 'تعديل',
        },
        alert: 'تنبيه',
      },
      financials: {
        filters: {
          period: 'الفترة',
          select: 'اختر',
          export: 'تصدير تقرير',
          searchPlaceholder: 'بحث في المنتجات أو التصنيفات...',
          periods: {
            this_month: 'هذا الشهر',
            last_month: 'الشهر الماضي',
            last_3_months: 'آخر 3 شهور',
            this_year: 'هذا العام',
          },
        },
        kpis: {
          revenue: 'الإيرادات',
          grossProfit: 'مجمل الربح',
          margin: 'نسبة الهامش',
          unitsSold: 'الوحدات المباعة',
          compare: 'مقارنة بالفترة السابقة',
          points: 'نقاط',
        },
        sections: {
          productProfitability: 'تفاصيل الربح',
          categoryBreakdown: 'تحليل الربح',
          noData: 'لا توجد بيانات مبيعات لهذه الفترة',
        },
        productGrid: {
          columns: {
            product: 'المنتج',
            quantitySold: 'الكمية المباعة',
            revenue: 'الإيرادات',
            cost: 'التكلفة',
            profit: 'الربح',
            margin: 'الهامش %',
          },
        },
        categoryGrid: {
          columns: {
            category: 'التصنيف',
            products: 'المنتجات',
            revenue: 'الإيرادات',
            profit: 'الربح',
            margin: 'الهامش',
            abcDistribution: 'ABC توزيع',
          },
        },
        profitLoss: {
          title: 'تقرير الأرباح والخسائر',
          netRevenue: 'صافي الإيرادات',
          grossProfit: 'إجمالي الربح',
          netCogs: 'تكلفة البضاعة',
          margin: 'نسبة الهامش',
          statement: 'القائمة المالية',
          grossSales: 'إجمالي المبيعات',
          returns: 'المرتجعات',
          netSales: 'صافي المبيعات',
          costOfSales: 'تكلفة المبيعات',
          operatingExpenses: 'مصاريف تشغيلية',
          noExpenses: '* لم يتم تسجيل مصاريف لهذه الفترة',
          estimatedNetProfit: 'صافي الربح التقديري',
          dailyPerformance: 'أداء الإيرادات اليومي',
          salesVsReturns: 'مقارنة المبيعات بالمرتجات',
          sales: 'المبيعات',
          refunds: 'المرتجعات',
          profitabilityByCategory: 'الربحية حسب التصنيف',
          category: 'التصنيف',
          revenue: 'الإيرادات',
          profit: 'الربح',
          cogs: 'التكلفة',
          unclassified: 'غير مصنف',
        },
      },
      procurement: {
        filters: {
          supplier: 'المورد',
          category: 'التصنيف',
          select: 'اختر',
          all: 'الكل',
        },
        kpis: {
          needingOrder: 'أصناف بحاجة للطلب',
          outOfStock: 'نواقص (Out of Stock)',
          lostSales: 'مبيعات ضائعة (متوقع)',
          pendingPO: 'طلبات معلقة (Pending)',
          items: 'صنف',
          vsLastWeek: 'مقارنة بالأسبوع الماضي',
          improved: 'تحسن',
        },
        actions: {
          generatePO: 'إنشاء طلب شراء',
        },
        empty: {
          title: 'لا توجد منتجات',
          subtitle: 'لا توجد منتجات تطابق الفلاتر المحددة',
        },
        grid: {
          columns: {
            product: 'المنتج',
            stockStatus: 'حالة المخزون',
            available: 'المتاح',
            days: 'أيام',
            dailySales: 'معدل البيع',
            suggested: 'اقتراح الطلب',
            confidence: 'الثقة',
            perDay: '/يوم',
          },
          skipped: 'تم التخطي',
        },
      },
      risk: {
        kpis: {
          valueAtRisk: 'قيمة معرضة للخطر',
          batchesAtRisk: 'باتشات تنتهي قريباً',
          total: 'إجمالي',
          batch: 'باتش',
        },
        actions: {
          createReturn: 'إنشاء قائمة إرجاع',
          applyDiscount: 'تطبيق سياسة التخفيضات',
        },
        empty: {
          title: 'لا توجد باتشات معرضة للخطر',
          subtitle: 'جميع المنتجات في المخزون بعيدة عن تاريخ انتهاء الصلاحية',
        },
        grid: {
          columns: {
            product: 'المنتج / الباتش',
            daysLeft: 'الأيام المتبقية',
            quantity: 'الكمية',
            valueAtRisk: 'قيمة المخاطرة',
            riskCategory: 'درجة الخطورة',
            recovery: 'استرداد متوقع',
            action: 'إجراء مقترح',
          },
          actions: {
            discount50: 'تخفيض 50%',
            discount25: 'تخفيض 25%',
            return: 'إرجاع للمورد',
            action: 'إجراء',
          },
          day: 'يوم',
        },
      },
    },
    drugInteractions: {
      title: 'تفاعلات الأدوية',
      subtitle: 'تحليل تحذيرات السلامة وموانع الاستعمال من بيانات FDA',
      searchPlaceholder: 'ابحث عن دواء بالاسم أو @المادة_الفعالة...',
      interactions: 'التفاعلات',
      warnings: 'التحذيرات',
      contraindications: 'موانع الاستعمال',
      noFdaData: 'لا تتوفر بيانات FDA لهذا الدواء',
      notApproved: 'هذا الدواء قد لا يكون مسجلاً في FDA (مثلاً: إصدار محلي)',
      activeIngredients: 'المواد الفعالة',
      source: 'المصدر: OpenFDA',
      loading: 'جاري جلب البيانات السريرية من FDA...',
      retry: 'إعادة المحاولة',
      selectDrug: 'اختر دواءً من النتائج لعرض البيانات',
      noInteractions: 'لم يتم العثور على تفاعلات في قاعدة بيانات FDA',
      combinationDrug: 'منتج مركب',
      ingredientCount: 'مواد فعالة',
      usage: 'دواعي الاستعمال',
      noResults: 'لم يتم العثور على منتجات',
      safetyStatus: 'حالة السلامة',
      selectMultipleDrugs: 'اختر عدة أدوية لتحليل التفاعلات',
      searchAndAddDrugs: 'ابحث عن الأدوية وأضفها من الشريط الجانبي، ثم استكشف بيانات FDA السريرية',
      clearCocktail: 'مسح المجموعة',
      interactionAnalysis: 'تحليل التفاعلات',
      drugsSelected: 'تم اختيار {count} أدوية',
    },
    appTitle: 'Zinc',
    zincAcronym: {
      Z: {
        title: 'Zero Errors (دقة بلا أخطاء)',
        desc: 'يضمن أعلى معايير الدقة في صرف الوصفات وإدارة المخزون لضمان سلامة المرضى.',
      },
      I: {
        title: 'Integrated Management (إدارة متكاملة)',
        desc: 'نظام موحد يربط المبيعات والمشتريات وشركات التأمين في منصة تقنية واحدة.',
      },
      N: {
        title: 'Next-Gen Tech (تقنيات الجيل القادم)',
        desc: 'واجهة ذكية تعتمد على الأتمتة لتقليل الجهد اليدوي وتسريع وتيرة العمل اليومي.',
      },
      C: {
        title: 'Constant Control (رقابة مستمرة)',
        desc: 'تحكم كامل وشامل في الأرباح، النواقص، وحركة الأصناف من خلال تقارير لحظية دقيقة.',
      },
    },
    date: 'التاريخ',
    orderId: 'رقم الطلب',
    orderTotal: 'إجمالي الطلب',
    nav: {
      dashboard: 'لوحة التحكم',
      inventory: 'المخزون',
      pos: 'نقطة البيع',
      sales: 'المبيعات',
      purchase: 'المشتريات',
      salesHistory: 'سجل المبيعات',
      suppliers: 'الموردين',
      purchases: 'المشتريات / المخزون',
      barcodeStudio: 'ستوديو الباركود',
      barcodePrinter: 'طباعة الباركود',
      stockAdjustment: 'جرد المخزون',
      stockMovement: 'حركة الصنف',
    },
    menu: {
      cashRegister: 'سجل النقدية',
      modules: 'الوحدات',
      sections: 'أقسام',
      totalItems: 'إجمالي العناصر',
      searchIn: 'بحث في',
      pack: 'علبة',
      unit: 'وحدة',
    },
    settings: {
      tools: 'أدوات',
      currencyConverter: {
        title: 'تحويل العملات',
        amount: 'المبلغ',
        from: 'مــــن',
        to: 'إلــــى',
        result: 'النتيجة',
        loading: 'جاري تحميل الأسعار...',
        error: 'فشل في جلب الأسعار',
      },
      calculator: {
        title: 'آلة حاسبة',
      },
      holidays: {
        title: 'الإجازات والمناسبات',
        all: 'الكل',
        upcoming: 'القادمة',
        past: 'السابقة',
        shifted: 'مُرَحَّلَة',
        originalDate: 'التاريخ الأصلي',
        actualDate: 'التاريخ الفعلي',
        daysLeft: 'يوم متبقي',
        daysAgo: 'يوم مضى',
        today: 'اليوم',
        tomorrow: 'غداً',
        yesterday: 'أمس',
        ramadanSpecial: 'موسم رمضان',
        shiftedToThursday: 'مُرَحَّلَة للخميس',
        nationalType: 'إجازة رسمية',
        religiousType: 'مناسبة دينية',
        observanceType: 'مواسم ومناسبات',
        rescheduleRule: 'قاعدة ترحيل الخميس',
      },
      settings: 'الإعدادات',
      theme: 'السمة',
      language: 'اللغة',
      darkMode: 'الوضع الداكن',
      lightMode: 'الوضع الفاتح',
      appearance: 'المظهر واللغة',
      fullscreen: 'ملء الشاشة',
      reload: 'إعادة تحميل',
      textTransform: 'أسماء بأحرف كبيرة',
      focusMode: 'وضع التركيز',
      navigationLayout: 'تخطيط التنقل',
      sidebarSettings: 'الشريط الجانبي',
      navbarMenuLayout: 'القائمة العلوية',
      navbarMenuSingle: 'عمود واحد',
      navbarMenuMulti: 'أعمدة متعددة',
      designStyle: 'نمط التصميم',
      designStyleFull: 'كامل',
      designStyleNavbar: 'علوي',
      developerMode: 'وضع المطور',
      themesMenu: 'الثيم',
      accent: 'لون التميز',
      statusBarSettings: 'إعدادات شريط الحالة',
      showNotificationBell: 'زر الإشعارات',
      showNotificationToast: 'تنبيه الإشعارات',
      quickStatuses: 'الحالات السريعة',
      showSales: 'المبيعات',
      showInventory: 'المخزون',
      showCustomers: 'العملاء الجدد',
      showTopSeller: 'أفضل بائع',
      comingSoon: 'قريباً',
      typography: 'التنسيق والخطوط',
      fontEN: 'خط اللغة الإنجليزية',
      fontAR: 'خط اللغة العربية',
      graphicStyle: 'نمط العناوين الجرافيكي',
      borderRadius: 'نصف قطر الحواف',
      radiusDefault: 'عصري',
      radiusSharp: 'الحاد',
      radiusFull: 'ناعم',
      dataManagement: 'إدارة البيانات',
      exportBackup: 'تصدير نسخة احتياطية',
      importBackup: 'استيراد نسخة احتياطية',
      importWarning:
        'تحذير: استيراد نسخة احتياطية سيؤدي إلى مسح جميع البيانات الحالية. هل تريد الاستمرار؟',
      storageQuota: {
        warningTitle: 'تحذير من مساحة التخزين',
        warningMessage:
          'مساحة التخزين للمتصفح المستخدمة مرتفعة (تم استخدام {{percentage}}% من أصل {{limit}} ميجابايت). يرجى مسح الفروع القديمة أو كاش المتصفح لتجنب فقدان البيانات.',
        criticalTitle: 'مساحة التخزين ممتلئة',
        criticalMessage:
          'فشل في حفظ البيانات. تم تجاوز الحد الأقصى للتخزين المحلي. يرجى توفير مساحة فوراً.',
      },
      branchManagement: 'إدارة الفروع',
      addBranch: 'إضافة فرع جديد',
      branchName: 'اسم الفرع',
      branchCode: 'كود الفرع',
      branchAddress: 'عنوان الفرع',
      branchPhone: 'هاتف الفرع',
      branchStatus: 'الحالة',
      assignEmployees: 'تعيين الموظفين',
      saveBranch: 'حفظ الفرع',
      deleteBranch: 'حذف الفرع',
      deleteBranchConfirm:
        'هل أنت متأكد من حذف هذا الفرع؟ سيؤثر هذا على جميع البيانات المرتبطة به.',
      sidebarStyle: 'شكل القائمة الجانبية',
      sidebarFixed: 'ثابت',
      sidebarMini: 'منكمش',
      borderStyle: 'شكل البطاقة',
      customCss: 'كود CSS للبطاقات',
      numeralSystem: 'نظام الأرقام',
      numeralArabic: 'عربي (١٢٣)',
      numeralLatin: 'إنجليزي (123)',
      cardStyle: 'شكل البطاقة',
      cardThick: 'سميك',
      cardThin: 'نحيف',
      cardNone: 'بدون',
      customCardCss: 'كود CSS للبطاقات',
      badgeStyle: 'شكل البادج',
      badgeStyleDefault: 'مستدير',
      badgeStyleSharp: 'حاد',
      badgeStylePill: 'بيضاوي',
      badgeStyleSlim: 'مدمج',
      modalPresentationMode: 'عرض النوافذ',
      modalPresentationModeModal: 'نافذة منبثقة',
      modalPresentationModeSidebar: 'سايدبار جانبي',
      modalSettings: 'إعدادات النوافذ',
      modalPresentationStyle: 'طريقة العرض',
      sidebarModalWidth: 'عرض السايدبار',
      sidebarModalWidthNarrow: 'SM',
      sidebarModalWidthStandard: 'MD',
      sidebarModalWidthWide: 'LG',
      sidebarModalWidthExtraWide: 'XL',
      reducedMotion: 'إيقاف الحركة',
      disableCSSTransitions: 'إيقاف الانتقالات',
      backgroundPattern: 'خلفية منقوشة',
      patternNone: 'بدون',
      patternDots: 'نقاط',
      patternGrid: 'شبكة',
      patternThickGrid: 'شبكة سميكة',
      patternBlueprint: 'مخطط هندسي',
      patternRings: 'حلقات',
      patternMesh: 'شبكي',
      patternNoise: 'ضوضاء',
      patternMandala: 'ماندالا',
      patternDiamond: 'ماسي',
      patternCorners: 'زوايا',
      patternCross: 'تقاطع',
      patternStars: 'نجوم',
      patternGlowingStars: 'نجوم مضيئة',
      patternHearts: 'قلوب',
      patternBricks: 'طوب',
      patternAbstract: 'جمجمه',
      patternCircuit: 'دائرة كهربائية',
      patternOrnate: 'مزخرف',
      patternDiagonalStripes: 'خطوط مائلة',
      patternOpacity: 'شفافية النقش',
      vividBg: 'مزيج الخلفية',
      posSettings: 'إعدادات البيع',
      defaultDeliveryFee: 'مصاريف التوصيل الافتراضية',
      egp: 'ج.م',
      active: 'نشط',
      inactive: 'غير نشط',
      noStreetAddress: 'لم يتم تحديد عنوان تفصيلي',
      noLocationSet: 'لم يتم تحديد موقع',
      cannotDeactivateActive: 'لا يمكن الغاء تفعيل الفرع الحالي النشط',
      saveSuccess: 'تم حفظ بيانات الفرع بنجاح',
      deactivated: 'تم إلغاء تفعيل الفرع',
      streetAddress: 'العنوان بالتفصيل',
      streetAddressPlaceholder: 'رقم المبنى، اسم الشارع...',
      all: 'الكل',
      selected: 'المختارة',
      noSelectedEmployees: 'لا يوجد موظفين مختارين',
      noResults: 'لا توجد نتائج',
      saveFirstForToken: 'احفظ الفرع أولاً لتتمكن من توليد رمز الحضور',
      current: 'الحالي',
      noBranchesYet: 'لا توجد فروع بعد',
      startAddingFirst: 'ابدأ بإضافة فرعك الأول للمنظمة',
      tabGeneral: 'البيانات',
      tabEmployees: 'الموظفين',
      tabAttendance: 'الحضور',
    },
    orgManagement: {
      title: 'إدارة المنظمة',
      subtitle: 'تحكم مركزي لجميع الفروع والأعضاء',
      permissionsTitle: 'صلاحيات المدراء',
      permissionsSubtitle: 'إدارة الوصول والأدوار عبر جميع الفروع',
      roleOwner: 'مالك',
      roleAdmin: 'مدير',
      roleMember: 'عضو',
      primaryBranch: 'الفرع الأساسي',
      globalRole: 'الدور العام',
      member: 'الموظف',
      email: 'البريد الإلكتروني',
      status: 'الحالة',
      active: 'نشط',
      inactive: 'غير نشط',
      saveSuccess: 'تم تحديث الصلاحيات بنجاح',
      saveError: 'فشل تحديث الصلاحيات',
      multiBranchAccessNote: 'الأدوار (مالك/مدير) تمنح وصولاً كاملاً لجميع الفروع',
      searchPlaceholder: 'بحث عن موظف...',
      lastOwnerWarning: 'لا يمكن تغيير الدور: هذا هو المالك الوحيد للمنظمة.',
      noAccountNote: 'هذا الموظف ليس لديه حساب نظام ولا يمكن تعيين دور عام له.',
      fetchError: 'فشل جلب البيانات',
      organization: 'إدارة المنظمة',
      branches: 'إدارة الفروع',
      allStaff: 'كافة الموظفين',
      allStaffSubtitle: 'قائمة الموظفين المسجلين في كافة الفروع',
      managers: 'المدراء',
      staff: 'الموظفين',
    },
    organization: {
      inviteMember: 'دعوة عضو',
      invites: {
        title: 'دعوة عضو جديد',
        subtitle: 'إرسال رابط دعوة لموظف جديد',
        email: 'البريد الإلكتروني',
        role: 'الدور',
        send: 'إرسال الدعوة',
        success: 'تم إنشاء رابط الدعوة!',
        copy: 'نسخ الرابط',
        copied: 'تم النسخ!',
        close: 'إغلاق',
      },
    },

    profile: {
      role: 'صيدلي',
      changePhoto: 'تغيير الصورة',
      reset: 'إعادة تعيين',
      signOut: 'تسجيل الخروج',
      updateProfile: 'تحديث الملف الشخصي',
      avatar: 'الصورة الشخصية',
      contactInfo: 'معلومات الاتصال',
      adjustBanner: 'تعديل موضع الغلاف',
      dragToPan: 'اسحب لضبط موضع الخلفية',
      bannerZoom: 'التكبير',
      resetPosition: 'إعادة تعيين',
      done: 'تم',
    },

    global: {
      actions: {
        theme: 'تغيير السمة',
        fullscreen: 'ملء الشاشة',
        reload: 'إعادة تحميل',
        help: 'المساعدة والدعم',
        close: 'إغلاق',
        asc: 'تصاعدي',
        desc: 'تنازلي',
        cancel: 'إلغاء',
        save: 'حفظ',
        delete: 'حذف',
        edit: 'تعديل',
        view: 'عرض',
        search: 'بحث',
        filter: 'تصفية',
        export: 'تصدير',
        date: 'التاريخ',
        orderId: 'رقم الطلب',
        orderTotal: 'إجمالي الطلب',
        actions: 'إجراءات',
        all: 'الكل',
        import: 'استيراد',
        print: 'طباعة',
        openInWindow: 'فتح في نافذة',
        showStats: 'عرض الإحصائيات',
        hideStats: 'إخفاء الإحصائيات',
        // DataPort (Export/Import)
        exportCSV: 'تصدير CSV',
        exportExcel: 'تصدير Excel',
        exportPDF: 'تصدير PDF',
        importCSV: 'استيراد CSV/Excel',
        exportData: 'تصدير البيانات',
        importData: 'استيراد البيانات',
        importSuccess: 'تم الاستيراد بنجاح',
        importError: 'فشل الاستيراد',
        rowsImported: 'صف تم استيراده',
      },
      datePicker: {
        cancel: 'إلغاء',
        ok: 'موافق',
        hour: 'ساعة',
        minute: 'دقيقة',
      },
      currency: 'ج.م',
      loading: 'جاري التحميل...',
      checkingAuth: 'جاري التحقق من تسجيل الدخول...',
      pageNotFound: 'الصفحة غير موجودة',
      table: {
        columns: 'الأعمدة',
        alignment: 'المحاذاة',
        noColumnsVisible: 'لا توجد أعمدة مرئية',
        manageColumnsHint: 'انقر بزر الماوس الأيمن في أي مكان لإدارة الأعمدة',
        loadingData: 'جاري تحميل البيانات...',
        page: 'صفحة',
        of: 'من',
        showAll: 'عرض الكل',
        filters: 'الفلاتر',
        addFilter: 'إضافة فلتر',
        clearAllFilters: 'مسح كل الفلاتر',
        clearFilter: 'مسح الفلتر',
        selectedCount: 'محدد',
        showing: 'عرض',
        showingAll: 'عرض كل العناصر',
        total: 'الإجمالي',
      },
    },
    login: {
      title: 'مرحباً بعودتك!',
      subtitle: 'أدخل بيانات الدخول للوصول إلى حسابك.',
      username: 'اسم المستخدم',
      usernamePlaceholder: 'أدخل اسم المستخدم',
      password: 'كلمة المرور',
      passwordPlaceholder: '••••••••',
      rememberMe: 'تذكرني لمدة 30 يوم',
      forgotPassword: 'نسيت كلمة المرور؟',
      submit: 'متابعة',
      signingIn: 'جاري تسجيل الدخول...',
      successTitle: 'تم تسجيل الدخول بنجاح!',
      redirecting: 'جاري التوجيه للوحة التحكم...',
      errorInvalidCredentials: 'اسم المستخدم أو كلمة المرور غير صحيحة',
      errorGeneric: 'حدث خطأ غير متوقع. حاول مرة أخرى.',
      validationUsernameRequired: 'اسم المستخدم مطلوب',
      validationPasswordRequired: 'كلمة المرور مطلوبة',
      validationPasswordLength: 'كلمة المرور يجب أن تكون 4 أحرف على الأقل',
      authorizedUserNotice: 'بالمتابعة، أنت تؤكد أنك مستخدم مُصرح له.',
      email: 'البريد الإلكتروني',
      emailPlaceholder: 'you@example.com',
      fullName: 'الاسم الكامل',
      fullNamePlaceholder: 'أحمد محمود',
      confirmPassword: 'تأكيد كلمة المرور',
      signUpTitle: 'إنشاء حساب جديد',
      signUpSubtitle: 'انضم إلينا للوصول إلى النظام.',
      signUpSubmit: 'إنشاء حساب',
      dontHaveAccount: 'ليس لديك حساب؟',
      signUpLink: 'سجل الآن',
      haveAccount: 'لديك حساب بالفعل؟',
      loginLink: 'تسجيل الدخول',
      resetTitle: 'استعادة كلمة المرور',
      resetSubtitle: 'أدخل بريدك الإلكتروني لاستلام رابط الاستعادة.',
      resetSubmit: 'إرسال رابط الاستعادة',
      backToLogin: 'العودة لتسجيل الدخول',
      passwordsDoNotMatch: 'كلمات المرور غير متطابقة',
      resetLinkSent: 'تم إرسال رابط الاستعادة إلى بريدك الإلكتروني',
      employeePortal: 'بوابة الموظفين',
      manageEmployment: 'إدارة توظيفك عبر منظمات ZINC',
      pendingRequests: 'الطلبات',
      noPendingRequests: 'لا توجد طلبات معلقة',
      accept: 'قبول',
      reject: 'رفض',
      selectWorkspace: 'اختر مساحة العمل',
      selectWorkspaceDesc: 'لديك وصول إلى مؤسسات صيدلانية متعددة. اختر أيهما تريد الدخول.',
      confirmReject: 'هل أنت متأكد من رفض طلب التوظيف هذا؟',
      errorRejectingRequest: 'فشل في رفض الطلب',
      errorAcceptingRequest: 'فشل في قبول الطلب',
      clockSkewTitle: 'ساعة الجهاز غير متزامنة',
      clockSkewMessage:
        'وقت جهازك غير صحيح، مما يمنع الاتصال الآمن بخوادمنا. يرجى تحديث التاريخ والوقت والمنطقة الزمنية لجهازك، ثم تحديث الصفحة.',
    },
    statusBar: {
      ready: 'جاهز',
      online: 'متصل',
      offline: 'غير متصل',
      notifications: 'الإشعارات',
      noNotifications: 'لا توجد إشعارات',
      clearAll: 'مسح الكل',
      dismiss: 'تجاهل',
      shiftOpen: 'المناوبة مفتوحة',
      shiftClosed: 'المناوبة مغلقة',
      shiftSince: 'منذ',
      userLabel: 'المستخدم',
      quickLogin: {
        login: 'تسجيل دخول الموظف',
        username: 'اسم المستخدم',
        password: 'كلمــــة المـرور',
        newPassword: 'كلمة المرور الجديدة',
        forgotPassword: 'نسيت كلمة المرور؟',
        notFound: 'غير موجود',
        resetSent: 'تم إرسال رابط إعادة التعيين لبريدك',
        passwordUpdated: 'تم تحديث كلمة المرور بنجاح',
        changeSuccess: 'تم تغيير كلمة المرور بنجاح',
        biometricUnsupported:
          'هذا المتصفح لا يدعم مفاتيح المرور (Passkeys). تأكد من استخدام HTTPS.',
        contactManager: 'تواصل مع المدير لإعادة تعيين كلمة المرور',
        passkeyTooltip: 'تسجيل الدخول بمفتاح مرور',
      },
      messages: {
        outOfStock: 'نفذت الكمية: {{name}} {{form}}',
        saleComplete: 'تمت عملية بيع: {{total}}',
      },
      ticker: {
        todaySales: 'اليوم',
        invoices: 'فواتير',
        completed: 'مكتمل',
        pending: 'معلق',
        lowStock: 'مخزون منخفض',
        shortages: 'نواقص',
        newCustomers: 'عملاء جدد',
        topSeller: 'أفضل بائع',
      },
    },
    loginAudit: {
      title: 'سجل عمليات الدخول',
      subtitle: 'تتبع محاولات تسجيل الدخول والخروج والتبديل بين الحسابات',
      globalView: 'العرض العام',
      searchPlaceholder: 'بحث باسم المستخدم أو ID...',
      emptyMessage: 'لا توجد سجلات عمليات حالياً',
      showAllBranches: 'كل الفروع',
      yesterday: 'أمس',
      today: 'اليوم',
      am: 'ص',
      pm: 'م',
      actions: {
        login: 'دخول موظف',
        logout: 'خروج',
        switch: 'تبديل',
        system_login: 'دخول النظام',
        system_logout: 'خروج النظام',
        switchBranch: 'تبديل الفرع',
        forceLogout: 'إنهاء قسري',
        switchOrg: 'تبديل المنشأة',
        employeeLogout: 'خروج موظف',
      },
      headers: {
        id: 'ID',
        timestamp: 'الوقت والتاريخ',
        user: 'المستخدم',
        code: 'الكود',
        action: 'العملية',
        branch: 'الفرع',
        details: 'التفاصيل',
      },
      detailPatterns: {
        switchedFrom: 'تحول من {{name}}',
        switchedBranch: 'تحول من {{from}} إلى {{to}}',
        account: 'حساب: {{name}}',
        employeeSignedOut: 'قام الموظف بتسجيل الخروج',
        accountLogout: 'تسجيل خروج الحساب',
        systemSessionStarted: 'بداية جلسة النظام',
        employeeSessionStarted: 'بداية جلسة الموظف',
        terminatedBy: 'الجلسة تم إنهاؤها بواسطة {{name}}',
        employeeLoggedOutBy: 'الموظف {{name}} تم تسجيل خروجه بواسطة {{admin}}',
        remotelyBadge: 'عن بُعد',
      },
    },
    activeSessions: {
      title: 'الأجهزة المتصلة',
      errorLoading: 'فشل تحميل الجلسات',
      confirmEndAll: 'هل أنت متأكد من إنهاء جميع الجلسات الأخرى؟',
      sessionsCount: 'جلسة',
      activeCount: 'نشط',
      online: 'متصل',
      offline: 'غير متصل',
      stale: 'قديم',
      current: 'الحالي',
      thisDevice: 'هذا الجهاز',
      yourCurrentDevice: 'جهازك الحالي',
      runningMoreThan24h: 'نشط لأكثر من 24 ساعة',
      considerEndingStale: 'يُفضل إنهاء الجلسات القديمة',
      unassigned: 'غير محدد',
      searchPlaceholder: 'بحث بالاسم أو الجهاز أو IP...',
      refresh: 'تحديث',
      endAllOthers: 'إنهاء الكل',
      noSearchResults: 'لا توجد نتائج تطابق البحث',
      noActiveSessions: 'لا توجد جلسات نشطة',
      onlineNow: 'متصل الآن',
      action: 'إجراء',
      terminate: 'إنهاء الجلسة',
      end: 'إنهاء',
      sessionInfo: 'معلومات الجلسة',
      loggedIn: 'دخل',
      previously: 'السابق',
      startedLabel: 'البداية:',
      durationLabel: 'المدة:',
      seenLabel: 'آخر ظهور:',
      sort: {
        user: 'المستخدم',
        device: 'الجهاز',
        browser: 'المتصفح',
        started: 'البداية',
        duration: 'المدة',
        ip: 'IP',
        lastSeen: 'آخر ظهور',
      },
    },
    pendingBranch: {
      title: 'في انتظار التعيين لفرع',
      subtitle: 'الملف الشخصي نشط، ولكن لم يتم تعيينك لأي فرع بعد.',
      employeeMessage:
        'يرجى التواصل مع مسؤول النظام أو مالك المؤسسة لتعيين ملفك الشخصي لفرع في إعدادات الموظفين (HR/Staff).',
      switchProfile: 'تبديل الحساب',
      signOut: 'تسجيل الخروج',
      adminTitle: 'تفعيل أحد الفروع',
      adminSubtitle:
        'بصفتك مسؤولاً، يمكنك اختيار أحد الفروع لإدارتها أو إنشاء فرع جديد إذا لم تكن هناك فروع.',
      selectBranch: 'اختر الفرع',
      selectBranchPlaceholder: 'اختر فرعاً لتفعيله...',
      noBranches: 'لم يتم إنشاء أي فروع بعد.',
      createBranchBtn: 'انتقل لتهيئة الفروع',
      loadingBranches: 'جاري تحميل فروع المؤسسة...',
      errorLoading: 'فشل تحميل الفروع',
      activateBtn: 'تفعيل الفرع',
    },
    inventory: {
      tabs: {
        inventory: 'المخزون',
        addProduct: 'إضافة منتج',
        stockMovement: 'حركة المخزون',
        shortages: 'النواقص',
      },
      title: 'المخـــزون',
      subtitle: 'إدارة المخزون الدوائي',
      outOfStock: 'الخالص / نواقص',
      available: 'متوفر',
      shortage: 'ناقص',
      all: 'الكل',
      active: 'نشط',
      inactive: 'غير نشط',
      discontinued: 'ملغى',
      summary: {
        totalItems: 'إجمالي الأصناف',
        totalCost: 'تكلفة المخزون',
        saleValue: 'إجمالي قيمة البيع',
        restock: 'نواقص (رصيد صفر)',
        nearReorder: 'أوشكت على النفاذ',
        discontinued: 'أصناف ملغاة',
      },
      addDrug: 'إضافة دواء',
      allProducts: 'جميع المنتجات',
      addNewProduct: 'إضافة منتج جديد',
      addProductSubtitle: 'إضافة عنصر جديد إلى المخزون',
      productAddedSuccess: 'تمت إضافة المنتج بنجاح!',
      basicInfo: 'المعلومات الأساسية',
      productCodes: 'أكواد المنتج',
      stockDetails: 'تفاصيل المخزون',
      // Stock Alerts Items (submenu)
      pricing: 'التسعير',
      additionalInfo: 'معلومات إضافية',
      autoGenerate: 'توليد تلقائي',
      clearForm: 'مسح النموذج',
      saveAndAddAnother: 'حفظ وإضافة آخر',
      searchPlaceholder: 'بحث بالاسم، الكود، الفئة أو الوصف...',
      headers: {
        name: 'الاسم',
        codes: 'الأكواد',
        category: 'الفئة',
        // Warehouse Management Items
        status: 'الحالة',
        stock: 'المخزون',
        publicPrice: 'سعر البيع',
        cost: 'سعر التكلفة',
        expiry: 'الصلاحية',
        actions: 'إجراءات',
      },
      fields: {
        status: 'حالة المنتج',
        origin: 'منشأ المنتج',
        originLocal: 'محلي',
        originImported: 'مستورد',
      },
      actionsMenu: {
        edit: 'تعديل الدواء',
        delete: 'حذف الدواء',
        view: 'عرض التفاصيل',
        printBarcode: 'طباعة الباركود',
        duplicate: 'نسخ الدواء',
        adjustStock: 'تعديل سريع للمخزون',
        printQtyPrompt: 'كم عدد الملصقات للطباعة؟',
      },
      noResults: 'لم يتم العثور على أدوية مطابقة لبحثك.',
      modal: {
        edit: 'تعديل دواء',
        add: 'إضافة دواء جديد',
        brand: 'اسم الصنف',
        nameArabic: 'الاسم بالعربي',
        generic: 'الاسم العلمي',
        category: 'الفئة',
        expiry: 'تاريخ الانتهاء',
        publicPrice: 'سعر البيع ($)',
        cost: 'سعر الشراء ($)',
        stock: 'كمية المخزون',
        desc: 'الوصف',
        // Returns & Refunds Items
        barcode: 'الباركود الدولي',
        internalCode: 'الكود الداخلي',
        unitsPerPack: 'وحدات لكل عبوة',
        cancel: 'إلغاء',
        save: 'حفظ الدواء',
        tabGeneral: 'معلومات عامة',
        tabInventoryPricing: 'المخزون والتسعير',
      },
      details: {
        stockLevel: 'مستوى المخزون',
        packs: 'علبة',
        sellingPrice: 'سعر البيع',
        costPrice: 'سعر التكلفة',
        unitsPerPack: 'وحدات/عبوة',
        barcode: 'الباركود',
        internalCode: 'الكود الداخلي',
        description: 'الوصف',
        noDescription: 'لا يوجد وصف متاح.',
      },
      expiryStatus: 'حالة الصلاحية',
      allExpiry: 'عرض الكل',
      nearExpired: 'اقترب الانتهاء (6 شهور)',
      expired: 'منتهي الصلاحية',
      addProduct: {
        title: 'إضافة منتج جديد',
        subtitle: 'إضافة صنف جديد إلى المخزون',
        active: 'نشط',
        inactive: 'غير نشط',
        discontinued: 'ملغى',
        sections: {
          identification: 'التعريف والمعلومات الأساسية',
          classification: 'التصنيف والتصنيع',
          usage: 'تفاصيل إضافية والاستخدام',
          inventory: 'المخزون والتخزين',
          financial: 'التفاصيل المالية',
        },
        fields: {
          brandName: 'اسم المنتج (العلامة التجارية)',
          nameArabic: 'الاسم باللغة العربية',
          genericName: 'الاسم العلمي / العام',
          category: 'التصنيف الرئيسي',
          dosageForm: 'الشكل الصيدلاني / النوع',
          barcode: 'الباركود الرئيسي',
          internalCode: 'كود النظام الداخلي',
          manufacturer: 'الشركة المنتجة',
          status: 'حالة المنتج',
          origin: 'منشأ المنتج',
          description: 'الوصف',
          stock: 'المخزون الابتدائي (عبوات)',
          unitsPerPack: 'الوحدات / العبوة',
          minStock: 'تنبيه الحد الأدنى للمخزون',
          expiryDate: 'تاريخ الانتهاء',
          publicPrice: 'سعر البيع للجمهور (علبة)',
          tax: 'الضريبة (%)',
          costPrice: 'سعر التكلفة (شراء)',
          liveProfit: 'الربح المباشر',
          margin: 'الهامش',
          currency: 'ج.م',
          gross: 'إجمالي',
          originLocal: 'محلي',
          originImported: 'مستورد',
        },
        placeholders: {
          brandName: 'مثال: أوجمنتين ١ جرام أقراص',
          genericName: 'مثال: أملوديبين، فالسارتان',
          barcode: 'امسح للإضافة...',
          internalCode: 'توليد تلقائي أو يدوي',
          category: 'اختر التصنيف',
          dosageForm: 'اختر الشكل',
          manufacturer: 'مثال: فايزر، نوفارتيس',
          description: 'أدخل دواعي الاستعمال، التحذيرات، أو معلومات إضافية هنا...',
          langDetect: 'جارٍ اكتشاف اللغة...',
          nameArabic: 'مثال: أوجمنتين ١ جرام',
        },
        tooltips: {
          clear: 'إعادة ضبط جميع الحقول إلى القيم الافتراضية',
          saveAndNew: 'حفظ هذا المنتج والبقاء هنا لإضافة منتج آخر',
          save: 'إتمام وحفظ هذا المنتج في المخزون',
        },
        actions: {
          save: 'حفظ المنتج',
          saveAndNew: 'حفظ وجديد',
          clear: 'مسح النموذج',
          cancel: 'إلغاء',
          generateCode: 'توليد تلقائي',
        },
        messages: {
          success: 'تم إضافة المنتج بنجاح!',
          error: 'فشل في إضافة المنتج. يرجى التحقق من جميع الحقول.',
          cleared: 'تم مسح النموذج',
        },
      },
    },
    stockAdjustment: {
      title: 'جـــــرد المخزون',
      subtitle: 'تصحح مستويات المخزون',
      searchPlaceholder: 'بحث عن منتج للتعديل...',
      add: 'إضافة للتعديل',
      newStock: 'المخزون الجديد',
      difference: 'الفارق',
      reason: 'السبب',
      notes: 'ملاحظات',
      save: 'حفظ التعديل',
      clear: 'مسح',
      history: 'سجل التعديلات',
      table: {
        product: 'الاسم',
        current: 'المخزون الحالي',
        new: 'المخزون الجديد',
        diff: 'الفارق',
        valueImpact: 'الأثر المالي',
        reason: 'السبب',
        notes: 'ملاحظات',
        action: 'إجراء',
      },
      reasons: {
        damaged: 'تالف',
        expired: 'منتهي الصلاحية',
        theft: 'سرقة / فقدان',
        inventory_count: 'جرد دوري',
        correction: 'تصحيح بيانات',
        other: 'أخرى',
      },
      success: 'تم حفظ تعديل المخزون بنجاح!',
      empty: 'لا توجد عناصر للتعديل. ابحث وأضف منتجات.',
      noHistory: 'لا توجد سجلات تعديل للفترة المختارة.',
      noRecent: 'لا توجد عملية حديثة للطباعة. قم بإجراء تعديل أولاً.',
      historyLog: 'سجل التعديلات',
      adjustStock: 'تعديل المخزون',
    },

    barcodePrinter: {
      title: 'طباعــة الباركود',
      subtitle: 'قائمة انتظار وطباعة ملصقات المنتجات',
      searchPlaceholder: 'بحث عن منتج للطباعة...',
      addToQueue: 'إضافة للقائمة',
      printLabels: 'طباعة الملصقات',
      totalLabels: 'إجمالي الملصقات',
      clearQueue: 'مسح القائمة',
      tableHeaders: {
        item: 'المنتج',
        expiry: 'الصلاحية',
        qty: 'الكمية',
        actions: 'إجراءات',
      },
      alerts: {
        queueEmpty: 'قائمة الطباعة فارغة. أضف عناصر للطباعة.',
        printing: 'جاري إنشاء الملصقات...',
        printSuccess: 'تم إرسال الملصقات للطابعة',
      },
      settings: {
        store: 'اسم الصيدلية',
        hotline: 'الخط الساخن',
        name: 'اسم الدواء',
        publicPrice: 'السعر',
        expiry: 'تاريخ الانتهاء',
        barcode: 'الباركود',
      },
    },
    suppliers: {
      title: 'الموردين',
      subtitle: 'إدارة الموزعين وشركات الأدوية',
      addSupplier: 'إضافة مورد',
      searchPlaceholder: 'بحث باسم المورد، جهة الاتصال...',
      suppliersList: 'قائمة الموردين',
      addNewSupplier: 'إضافة مورد جديد',
      allSuppliers: 'جميع الموردين',
      manageSuppliers: 'إدارة الموردين',
      createNewRecord: 'إنشاء سجل مورد جديد',
      noSuppliersFound: 'لا يوجد موردين مطابقين للبحث',
      noSuppliersAvailable: 'لا يوجد موردين',
      headers: {
        id: 'ID',
        name: 'الاسم',
        contactPerson: 'الشخص المسؤول',
        phone: 'الهاتف',
        email: 'البريد',
        address: 'العنوان',
        action: 'إجراء',
        companyName: 'اسم الشركة',
        contact: 'الشخص المسؤول',
        details: 'بيانات الاتصال',
        actions: 'إجراءات',
        governorate: 'المحافظة',
        city: 'المدينة',
        area: 'المنطقة',
      },
      form: {
        companyInfo: 'معلومات الشركة',
        contactInfo: 'معلومات الاتصال',
        id: 'ID',
        companyName: 'اسم الشركة',
        enterCompanyName: 'أدخل اسم الشركة',
        address: 'العنوان',
        enterAddress: 'أدخل عنوان الشركة',
        contactPerson: 'الشخص المسؤول',
        enterContactPerson: 'أدخل اسم الشخص المسؤول',
        phone: 'الهاتف',
        phonePlaceholder: '+966500000000',
        email: 'البريد الإلكتروني',
        emailPlaceholder: 'email@example.com',
        paymentTerms: 'شروط الدفع',
        openingBalance: 'رصيد افتتاحي',
        paymentType: 'طريقة الدفع الافتراضية',
        creditDays: 'فترة الائتمان (أيام)',
        creditLimit: 'الحد الائتماني',
        cash: 'نقدي',
        credit: 'آجل',
      },
      modal: {
        add: 'إضافة مورد جديد',
        edit: 'تعديل بيانات المورد',
        editSubtitle: 'تحديث بيانات المورد',
        details: 'تفاصيل المورد',
        detailsSubtitle: 'عرض بيانات المورد',
        delete: 'حذف المورد',
        deleteSubtitle: 'لا يمكن التراجع عن هذا الإجراء',
        confirmDelete: 'هل أنت متأكد من حذف',
        name: 'اسم الشركة',
        contact: 'الشخص المسؤول',
        phone: 'رقم الهاتف',
        email: 'البريد الإلكتروني',
        address: 'العنوان',
        cancel: 'إلغاء',
        save: 'حفظ المورد',
        saveChanges: 'حفظ التغييرات',
        deleteBtn: 'حذف',
        addSupplier: 'إضافة المورد',
        close: 'إغلاق',
      },
      contextMenu: {
        viewDetails: 'عرض التفاصيل',
        edit: 'تعديل',
        delete: 'حذف',
        copyName: 'نسخ الاسم',
        copyPhone: 'نسخ الهاتف',
        copyEmail: 'نسخ البريد',
        showHideColumns: 'إظهار/إخفاء الأعمدة',
      },
      errors: {
        invalidPhone: 'رقم الهاتف غير صالح',
        invalidEmail: 'البريد الإلكتروني غير صالح',
      },
      required: 'مطلوب',
      fillRequired: 'الرجاء ملء جميع الحقول المطلوبة',
    },
    supplierAccounts: {
      paymentsTitle: 'مدفوعات الموردين',
      paymentsSubtitle: 'تسجيل وإدارة المدفوعات للموردين',
      statementTitle: 'كشف حساب المورد',
      statementSubtitle: 'دفتر أستاذ زمني مع الرصيد الجاري',
      agingTitle: 'أعمار الديون',
      agingSubtitle: 'الأرصدة المستحقة حسب فترة الاستحقاق',
      supplier: 'المورد',
      date: 'التاريخ',
      amount: 'المبلغ',
      paymentMethod: 'طريقة الدفع',
      reference: 'المرجع',
      notes: 'ملاحظات',
      currentBalance: 'الرصيد الحالي',
      allocations: 'التوزيع',
      invoice: 'الفاتورة',
      dueDate: 'تاريخ الاستحقاق',
      openAmount: 'المبلغ المفتوح',
      applyToOldest: 'سيتم تطبيقه على أقدم فواتيرك تلقائياً',
      totalAllocated: 'إجمالي الموزع',
      remaining: 'المتبقي',
      recordPayment: 'تسجيل دفعة',
      voidPayment: 'إلغاء',
      voidReason: 'سبب الإلغاء',
      paymentHistory: 'سجل المدفوعات',
      noPayments: 'لا توجد مدفوعات مسجلة بعد',
      noPayables: 'لا توجد فواتير مفتوحة لهذا المورد',
      openPayables: 'فواتير مفتوحة',
      cash: 'نقدي',
      bank: 'بنك',
      visa: 'بطاقة',
      entryDate: 'التاريخ',
      type: 'النوع',
      sourceRef: 'مرجع المصدر',
      debit: 'مدين (مستحق)',
      credit: 'دائن (مدفوع)',
      runningBalance: 'الرصيد الجاري',
      exportCSV: 'تصدير CSV',
      asOfDate: 'حتى تاريخ',
      searchSupplier: 'ابحث باسم المورد...',
      searchTransactions: 'ابحث بالمرجع، التاريخ، أو النوع...',
      current: 'حالي',
      days130: '1–30',
      days3160: '31–60',
      days6190: '61–90',
      days90Plus: '90+',
      total: 'الإجمالي',
      serialId: 'الرقم التسلسلي',
      voided: 'ملغاة',
      actions: 'إجراءات',
      dateFrom: 'من',
      dateTo: 'إلى',
      noTransactions: 'لا توجد حركات',
      noSuppliersFound: 'لا يوجد موردون لهذا الفرع',
      print: 'طباعة',
      selectSupplier: 'اختر مورد',
      filters: 'الفلاتر',
      apply: 'تطبيق',
      clear: 'مسح',
      paymentType: 'النوع',
      purchase: 'شراء',
      payment: 'دفعة',
      creditNote: 'إشعار دائن',
      openingBalance: 'رصيد افتتاحي',
      reversal: 'إلغاء',
      confirmVoid: 'إلغاء هذه الدفعة؟ لا يمكن التراجع عن هذا الإجراء.',
      invalidAmount: 'أدخل مبلغاً صحيحاً أكبر من صفر',
      allocationsExceed: 'المبلغ الموزع لا يمكن أن يتجاوز مبلغ الدفعة',
    },
    pendingApproval: {
      title: 'الموافقات المعلقة',
      subtitle: 'مراجعة واعتماد أوامر الشراء. يجب الموافقة على الطلبات قبل تحديث المخزون.',
      allCaughtUp: 'لا يوجد طلبات!',
      noPendingOrders: 'لا توجد أوامر شراء معلقة تتطلب الموافقة.',
      pendingReview: 'في الانتظار',
      invCode: 'فاتورة',
      supplier: 'المورد',
      date: 'التاريخ',
      totalCost: 'إجمالي التكلفة',
      items: 'العناصر',
      reject: 'رفض',
      approve: 'موافقة',
      approveOrder: 'اعتماد الطلب',
      orderDetails: 'تفاصيل أمر الشراء',
      itemsList: 'قائمة العناصر',
      approvedBy: 'المعتمد:',
      enterName: 'أدخل الاسم',
      enterApproverName: 'الرجاء إدخال اسم المعتمد.',
      rejectOrder: {
        title: 'رفض أمر الشراء؟',
        confirm: 'هل أنت متأكد من رفض هذا الطلب؟ لا يمكن التراجع عن هذا الإجراء.',
        reason: 'السبب (اختياري)',
        reasonPlaceholder: 'مثال: تسعير خاطئ، عناصر غير صحيحة...',
        cancel: 'إلغاء',
        reject: 'رفض',
      },
      tableHeaders: {
        item: 'الصنف',
        expiry: 'الصلاحية',
        qty: 'الكمية',
        cost: 'التكلفة',
        discount: 'الخصم %',
        publicPrice: 'سعر البيع',
        total: 'الإجمالي',
      },
      info: {
        supplier: 'المورد',
        invId: 'رقم الفاتورة',
        payment: 'الدفع',
        totalCost: 'إجمالي التكلفة',
        cash: 'نقدي',
        credit: 'آجل',
      },
    },
    drugApproval: {
      title: 'موافقات الأدوية الجديدة',
      subtitle:
        'مراجعة الأدوية الجديدة المضافة للكتالوج العام واتخاذ القرار بإضافتها للمخزن المحلي.',
      statusPending: 'قيد الانتظار',
      statusApproved: 'مقبول',
      statusSuspended: 'معلق',
      approveSelected: 'الموافقة على المحدد',
      suspendSelected: 'تعليق المحدد',
      reactivateSelected: 'إعادة تنشيط المحدد',
      noPendingDrugs: 'لا توجد أدوية جديدة مضافة للكتالوج العام بانتظار المراجعة.',
      tableHeaders: {
        drug: 'اسم الدواء',
        barcode: 'الباركود',
        dosageForm: 'الشكل الدوائي',
        category: 'التصنيف',
        price: 'سعر الجمهور',
        manufacturer: 'الشركة المصنعة',
        status: 'الحالة',
        actions: 'الإجراءات',
      },
      confirmApproveTitle: 'الموافقة على {{count}} دواء؟',
      confirmApproveDesc:
        'سيتم إضافة الأدوية المحددة فوراً لجميع فروع صيدليتك برصيد ابتدائي صفر وسعر تكلفة صفر.',
      confirmSuspendTitle: 'تعليق {{count}} دواء؟',
      confirmSuspendDesc: 'سيتم تعليم الأدوية المحددة كمعلقة. يمكنك إعادة تنشيطها في أي وقت لاحق.',
      successApprove: 'تمت الموافقة على الأدوية المحددة بنجاح!',
      successSuspend: 'تم تعليق الأدوية المحددة بنجاح!',
      successReactivate: 'تم إعادة تنشيط الأدوية المحددة بنجاح!',
      filterAll: 'الكل',
      searchPlaceholder: 'بحث بالاسم، المادة الفعالة أو الباركود...',
    },
    purchases: {
      title: 'شـــــراء جديد',
      subtitle: 'إعادة تعبئة المخزون من الموردين',
      selectSupplier: 'اختر المورد',
      searchDrug: 'بحث عن دواء للشراء...',
      emptyCart: 'لا توجد عناصر',
      tabs: {
        newTab: 'شراء جديد',
        closeTab: 'إغلاق التبويب',
        closeOthers: 'إغلاق الآخرين',
        duplicateTab: 'تكرار التبويب',
        pin: 'تثبيت التبويب',
        unpin: 'إلغاء التثبيت',
        rename: 'إعادة التسمية',
      },
      // Stock filter options
      filters: {
        all: 'الكل',
        inStock: 'متوفر',
        outOfStock: 'غير متوفر',
      },
      // Status filter options
      status: {
        title: 'الحالة',
        all: 'كل الحالات',
        pending: 'معلق',
        approved: 'مقبول',
        completed: 'مكتمل',
        returned: 'مرتجع',
        rejected: 'مرفوض',
      },
      // Placeholders
      placeholders: {
        searchHistory: 'بحث برقم، مورد...',
        searchSupplier: 'بحث واختيار المورد...',
        enterId: 'أدخـل الـ ID',
        searchDrug: 'بحث عن دواء...',
      },
      pack: 'علبة',
      unit: 'وحدة',
      // Table headers
      tableHeaders: {
        orderId: 'رقم الاوردر',
        invId: 'رقم الفاتورة',
        barcode: 'الباركود',
        date: 'التاريخ',
        supplier: 'المورد',
        payment: 'الدفع',
        items: 'العناصر',
        discount: 'الخصم',
        total: 'الإجمالي',
        action: 'إجراء',
      },
      // Status tooltips
      tooltips: {
        pending: 'في انتظار الموافقة',
        approved: 'مقبول',
        rejected: 'مرفوض',
        returned: 'مرتجع',
        completed: 'مكتمل',
      },
      // Context menu actions
      contextMenu: {
        viewDetails: 'عرض التفاصيل',
        copyInvoice: 'نسخ الفاتورة',
        copySupplier: 'نسخ المورد',
        addToOrder: 'إضافة للطلب',
        copyName: 'نسخ الاسم',
        editQuantity: 'تعديل الكمية',
        removeItem: 'حذف الصنف',
        copy: 'نسخ',
        paste: 'لصق',
        clear: 'مسح',
        showHideColumns: 'إظهار/إخفاء الأعمدة',
      },
      // Alert messages
      alerts: {
        enterInvoice: 'الرجاء إدخال رقم الفاتورة من المورد.',
        duplicateInvoice: 'رقم الفاتورة موجود بالفعل. الرجاء إدخال رقم فاتورة فريد.',
        enterQuantity: 'الرجاء إدخال كمية صالحة لـ',
        enterCost: 'الرجاء إدخال سعر تكلفة صالح لـ',
        enterSale: 'الرجاء إدخال سعر بيع صالح لـ',
        enterExpiry: 'الرجاء إدخال تاريخ صلاحية لـ',
        incompleteExpiry: 'الرجاء إدخال تاريخ صلاحية كامل (4 أرقام: شهر سنة)',
        promptQuantity: 'أدخل الكمية:',
        confirmCloseTab:
          'هذا التبويب يحتوي على أصناف. هل أنت متأكد من رغبتك في إغلاقه ونقله إلى السجل؟',
        couldNotCreateInvoice: 'تعذر إنشاء الفاتورة، حاول تاني',
      },
      // Other labels
      unknown: 'غير معروف',
      cash: 'نقدي',
      credit: 'آجل',
      partial: 'جزئي',
      paidNow: 'المدفوع الآن',
      payNow: 'ادفع الآن',
      paymentMethod: 'طريقة الدفع',
      pending: 'حفظ كمعلق',
      startSearching: 'ابدأ البحث عن منتجات لإضافتها لأمر الشراء',
      noResults: 'لا توجد نتائج',
      tryDifferentKeywords: 'جرب البحث بكلمات مختلفة',
      fromDate: 'من',
      toDate: 'إلى',
      time: {
        am: 'ص',
        pm: 'م',
      },
      headers: {
        item: 'الصنف',
        cost: 'التكلفة',
        qty: 'الكمية',
        total: 'الإجمالي',
        name: 'الاسم',
        codes: 'الأكواد',
        expiry: 'الصلاحية',
        stock: 'المخزون',
        profit: 'الربح',
        product: 'المنتج',
        subtotal: 'الإجمالي',
        totalPlusTax: 'الإجمالي+ض',
        supplier: 'المورد',
        internalCode: 'كود داخلي',
      },
      cartFields: {
        qty: 'الكمية',
        expiry: 'الصلاحية',
        cost: 'التكلفة',
        discount: 'الخصم %',
        sale: 'البيع',
        tax: 'ض %',
        subtotal: 'الإجمالي',
        totalWithTax: 'الإجمالي+ض',
        batchNumber: 'الدفعة #',
        unitCost: 'ت. وحدة',
        unitSale: 'س. وحدة',
      },
      summary: {
        totalItems: 'إجمالي العناصر',
        totalCost: 'إجمالي التكلفة',
        confirm: 'تأكيد واستلام المخزون',
        discount: 'الخصم',
        tax: 'الضريبة',
      },
      startedAt: 'وقت البدء',
      taxMode: 'نظام الضريبة',
      taxExclusive: 'غير شامل',
      taxInclusive: 'شامل',
      emptyCartHint: 'قم بالبحث وإضافة منتجات إلى الفاتورة',
      suspendInvoice: 'تعليق الفاتورة',
      confirming: 'جاري التأكيد...',
      confirmMergeTitle: 'تأكيد دمج الأصناف',
      confirmMove: 'تأكيد النقل',
      cancel: 'إلغاء',
      cartQr: {
        title: 'رمز الكارت',
        generate: 'توليد',
        scan: 'مسح',
        page: 'صفحة',
        invalidQr: 'رمز QR غير معروف. يرجى مسح رمز الكارت.',
        scanMore: 'جاري المسح... بانتظار المزيد من الصفحات',
        done: 'أصناف تم مسحها.',
        notFound: 'غير موجود',
        previewTitle: 'مراجعة قبل الإضافة',
        addToCart: 'أضف إلى الكارت',
        discard: 'تجاهل',
        emptyPreview: 'لا توجد أصناف قابلة للقراءة في رمز QR الممسوح.',
        copied: 'تم النسخ!',
        copyFailed: 'فشل النسخ',
        copy: 'نسخ المحتوى',
        download: 'تنزيل',
        items: 'أصناف',
        generateHint: 'امسح الرمز في صفحة المشتريات بالصيدلية المستقبلة لتحميل الأصناف.',
        generating: 'جاري التوليد...',
      },
      mergeInvoiceHas: 'الفاتورة',
      mergeAlreadyContains: 'تحتوي بالفعل على',
      mergeItemsCount: 'أصناف',
      mergeQuestion: 'هل أنت متأكد من دمج الأصناف المحددة معها؟',
      historyTitle: 'سجل المشتريات',
      viewHistory: 'عرض السجل',
      newPurchase: 'شراء جديد',
      actions: {
        viewDetails: 'عرض تفاصيل الدواء',
        editQty: 'تعديل الكمية',
      },
      // Purchase Details Modal
      detailsModal: {
        title: 'تفاصيل طلب الشراء',
        supplier: 'المورد',
        invNumber: 'فاتورة #',
        payment: 'الدفع',
        totalCost: 'إجمالي التكلفة',
        approvedOn: 'تاريخ الموافقة',
        approvedBy: 'تمت الموافقة بواسطة',
        status: 'الحالة',
        pendingApproval: 'قيد الموافقة',
        // Table headers
        item: 'الصنف',
        expiry: 'الصلاحية',
        qty: 'الكمية',
        returned: 'المرتجع',
        cost: 'التكلفة',
        total: 'الإجمالي',
        // Return labels
        fullReturn: 'كلي',
        partialReturn: 'جزئي',
        returnedLabel: 'مرتجع',
      },
      // Empty states
      noHistory: 'لا يوجد سجل مشتريات',
      markAsReceived: 'تعليم كمستلم',
      autoBatchCreated: 'تم إنشاء دفعات المخزون تلقائياً',
    },
    purchaseReturns: {
      createReturn: 'إنشاء إرجاع',
      returnHistory: 'سجل الإرجاع',
      createSubtitle: 'إرجاع الأصناف للمورد',
      historySubtitle: 'عرض جميع مردودات المشتريات',
      selectPurchase: 'اختر طلب الشراء',
      selectPlaceholder: 'اختر طلب شراء...',
      returnAll: 'إرجاع جميع عناصر هذا الشراء',
      itemsToReturn: 'العناصر المراد إرجاعها',
      availableItems: 'العناصر المتاحة من طلب الشراء',
      additionalNotes: 'ملاحظات إضافية',
      notesPlaceholder: 'أضف أي ملاحظات إضافية حول هذا الإرجاع...',
      clear: 'مسح',
      submit: 'إرسال الإرجاع',
      totalRefund: 'إجمالي الاسترداد',
      refundMethod: 'طريقة الاسترداد',
      cashRefund: 'نقدي',
      creditNote: 'إشعار دائن',
      creditNoteHint: 'سيُضاف إلى حساب المورد ويقلل من رصيده.',
      currentBalance: 'الرصيد الحالي',
      quantity: 'الكمية',
      reason: 'السبب',
      condition: 'الحالة',
      addToReturn: 'إضافة للإرجاع',
      returnDetails: 'تفاصيل الإرجاع',
      returnInfo: 'معلومات الإرجاع',
      returnedItems: 'العناصر المرجعة',
      notes: 'الملاحظات',
      close: 'إغلاق',
      tableHeaders: {
        id: 'رقم الإرجاع',
        date: 'التاريخ',
        purchaseId: 'رقم الشراء',
        supplier: 'المورد',
        refund: 'إجمالي الاسترداد',
        status: 'الحالة',
        action: 'إجراء',
      },
      searchPlaceholder: 'بحث في المردودات...',
      reasons: {
        damaged: 'تالف',
        expired: 'منتهي الصلاحية',
        wrong_item: 'صنف خاطئ',
        defective: 'معيب',
        overage: 'فائض - أدخل بالخطأ',
        other: 'أخرى',
      },
      conditions: {
        damaged: 'تالف',
        expired: 'منتهي الصلاحية',
        other: 'أخرى',
      },
      status: {
        pending: 'قيد الانتظار',
        approved: 'تمت الموافقة',
        completed: 'مكتمل',
        rejected: 'مرفوض',
      },
      messages: {
        selectPurchaseAlert: 'يرجى اختيار طلب شراء وإضافة عناصر للإرجاع',
        noReturnsFound: 'لم يتم العثور على مردودات تطابق بحثك',
        noReturnsYet: 'لا توجد مردودات مشتريات حتى الآن',
      },
      packs: 'علب',
      pack: 'علبة',
      inReturnList: 'في قائمة الإرجاع',
      available: 'متاح',
    },
    pos: {
      closedTabsTitle: 'التابات المغلقة مؤخراً',
      viewClosedTabs: 'عرض سجل التابات المغلقة',
      closedTabsEmpty: 'لا يوجد تابات مغلقة مؤخراً.',
      restoreTab: 'استعادة التاب',
      searchPlaceholder: 'امسح الباركود، الاسم، الوصف...',
      categories: {
        all: 'الكل',
        medicine: 'أدوية',
        cosmetics: 'تجميل',
        general: 'عام',
      },
      cartTitle: 'البيع الحالي',
      emptyCart: 'السلة فارغة',
      customerInfo: 'معلومات العميل',
      customerName: 'اسم العميل',
      customerCode: 'كود العميل',
      paymentMethod: 'طريقة الدفع',
      cash: 'كاش',
      visa: 'كرت',
      discount: 'خصم %',
      subtotal: 'المجموع',
      deliveryFee: 'مصاريف التوصيل',
      orderDiscount: 'الخصم%',
      total: 'الإجمالي',
      completeOrder: 'إتمام الطلب',
      itemLeft: 'متبقي',
      viewCart: 'عرض السلة',
      backToProducts: 'عودة للمنتجات',
      items: 'عناصر',
      drugTypes: 'صنف',
      totalQty: 'قطعة',
      selectDriverPlaceholder: 'اختر السائق (اختياري)',
      feePlaceholder: 'الرسوم',
      netProfit: 'صافي الربح',
      expMargin: 'هامش الربح المتوقع:',
      netProfitDesc: 'صافي الربح بعد خصم التكلفة والخصومات.',
      pack: 'علبة',
      unit: 'وحدة',
      perUnit: '/وحدة',
      actions: {
        checkStock: 'فحص المخزون',
        showSimilar: 'عرض بدائل',
        discount: 'تطبيق خصم',
        changeQty: 'تغيير الكمية',
        searchInTable: 'بحث في الجدول',
      },
      change: 'الباقي',
      allStock: 'كل المخزون',
      inStock: 'متوفر',
      outOfStock: 'غير متوفر',
      posTitle: 'نقطة البيـــع',
      name: 'اسم الصنف',
      code: 'الكود',
      category: 'الفئة',
      publicPrice: 'السعر',
      stock: 'الاستوك',
      batches: 'الباتش',
      inCart: 'في السلة',
      address: 'العنوان',
      changeCustomer: 'تغيير العميل',
      customerSearchPlaceholder: 'بحث بالاسم، الهاتف، الكود...',
      noStock: 'لا يوجد مخزون',
      productDetails: 'تفاصيل المنتج',
      close: 'إغلاق',
      copy: 'نسخ',
      paste: 'لصق',
      clear: 'مسح',
      deliveryOrder: 'طلب توصيل',
      deliveryOrders: 'طلبات التوصيل',
      deliveryCalculator: {
        title: 'حساب بالمسافة',
        reset: 'إعادة للقيمة الافتراضية',
        customDistance: 'مسافة مخصصة...',
        km: 'كم',
        perKm: '/ كم',
        egp: 'ج.م',
      },
      noOpenShift: 'افتح وردية قبل إتمام البيع',
      switchToPack: 'تبديل إلى عبوة',
      switchToUnit: 'تبديل إلى وحدة',
      removeItem: 'إزالة الصنف',
      viewDetails: 'عرض التفاصيل',
      copyName: 'نسخ الاسم',
      addToCart: 'إضافة للسلة',
      icon: 'أيقونة',
      pendingValueDetails: 'تفاصيل القيمة المعلقة',
      timeDistribution: 'توزيع الوقت',
      today: 'اليوم',
      older: 'أقدم من ذلك',
      todaysOrders: 'طلبات اليوم',
      last24Hours: 'آخر 24 ساعة',
      olderOrders: 'طلبات متأخرة',
      pendingValueInsightFooter: 'تحليل القيم المعلقة يساعد في تحسين سرعة التوصيل',
      orderPlural: 'طلب',
      orderSingular: 'طلب',
      cancelOrderConfirm:
        'هل أنت متأكد من إلغاء هذا الطلب؟ لا يمكن التراجع عن هذه العملية وسيتم إرجاع جميع المواد للمخزون.',
      keepOrder: 'تراجع',
      modal: {
        stock: 'المخزون',
        publicPrice: 'السعر',
        category: 'الفئة',
        expiry: 'تاريخ الانتهاء',
        location: 'الموقع',
        description: 'الوصف',
        noDescription: 'لا يوجد وصف متاح.',
        shelf: 'الرف',
        history: 'سجل العميل',
      },
      invoice: 'فاتورة',
      invoiceDetails: 'تفاصيل الفاتورة',
      favorites: 'المفضلة',
      phone: 'الهاتف',
      itemAdded: 'تم إضافة الصنف للسلة',
      customerHistory: {
        title: 'سجل العميل',
      },
      startSearching: 'ابدأ البحث...',
      noResults: 'لم يتم العثور على نتائج.',
      resultsLabel: 'نتيجـة',
      tryDifferentKeywords: 'جرب كلمات بحث مختلفة.',
      tabs: {
        newTab: 'فاتورة جديدة',
        closeTab: 'إغلاق الفاتورة',
        closeOthers: 'إغلاق الأخريات',
        duplicateTab: 'تكرار الفاتورة',
        pin: 'تثبيت',
        unpin: 'إلغاء التثبيت',
        rename: 'إعادة تسمية',
      },
      selectDriver: 'اختر السائق',
      assignDriver: 'تعيين السائق',
      all: 'الكل',
      pending: 'قيد الانتظار',
      active: 'نشط',
      history: 'السجل',
      startDelivery: 'بدء التوصيل',
      onWay: 'في الطريق',
      completed: 'مكتمل',
      cancelled: 'ملغي',
      start: 'ابدأ',
      complete: 'إتمام',
      orderId: 'رقم الطلب',
      deliveryMan: 'مندوب التوصيل',
      searchOrder: 'بحث في الطلبات...',
      noOrders: 'لا توجد طلبات توصيل',
      totalPending: 'إجمالي القيمة المعلقة',
      edit: 'تعديل',
      editOrder: 'تعديل الطلب',
      printReceipt: 'طباعة الإيصال',
      saveChanges: 'حفظ التعديلات',
      discard: 'تراجع',
      editGuestInfo: 'تعديل بيانات الضيف',
      tempCustomerName: 'الاسم المؤقت',
      tempCustomerAddress: 'العنوان المؤقت',
      addressPlaceholder: 'أدخل عنوان التوصيل بالتفصيل...',
      orderItems: 'أصناف الطلب',
      loginToEdit: 'سجل دخول للتعديل',
      restore: 'استعادة',
      delete: 'حذف',
      deleted: 'محذوف',
      by: 'بواسطة',
      time: 'الوقت',
      yesterday: 'أمس',
      am: 'ص',
      pm: 'م',
      confirmDelete: 'هل أنت متأكد من حذف هذا الصنف؟',
      remainder: 'الباقي',
      justNow: 'الآن',
      ago: 'منذ',
      modification: 'تعديل',
      modifications: 'تعديلات',
      customer: 'العميل',
      guestCustomer: 'عميل غير مسجل',
      stockLimitReached: 'تم الوصول للحد الأقصى للمخزن',
      units: 'وحدات',
      packs: 'علب',
    },
    salesHistory: {
      title: 'سجل المبيعـــــات',
      subtitle: 'عرض وإدارة المعاملات السابقة',
      status: 'الحالة',
      completed: 'مكتمل',
      cancelled: 'ملغي',
      searchPlaceholder: 'بحث بالعميل، المعرف أو اسم المنتج...',
      dateFrom: 'من',
      dateTo: 'إلى',
      sortBy: 'ترتيب حسب',
      sortNewest: 'الأحدث أولاً',
      sortOldest: 'الأقدم أولاً',
      sortHighTotal: 'الأعلى سعراً',
      sortLowTotal: 'الأقل سعراً',
      exportCSV: 'تصدير CSV',
      totalRevenue: 'إجمالي الإيرادات',
      pending: 'قيد الانتظار',
      with_delivery: 'مع المندوب',
      on_way: 'في الطريق',
      returned: 'مرجع',
      fullReturn: 'إرجاع كلي',
      partialReturn: 'إرجاع',
      headers: {
        date: 'التاريخ',
        customer: 'العميل',
        payment: 'الدفع',
        items: 'عدد العناصر',
        total: 'الإجمالي',
        actions: 'إجراءات',
        soldBy: 'بواسطة',
        delivery: 'توصيل',
      },
      noResults: 'لم يتم العثور على سجلات مبيعات.',
      visa: 'كرت',
      cash: 'كاش',
      modal: {
        title: 'تفاصيل البيع',
        id: 'رقم المعاملة',
        date: 'التاريخ',
        customer: 'العميل',
        payment: 'طريقة الدفع',
        items: 'أصناف الفاتورة',
        subtotal: 'الإجمالي',
        discount: 'الخصم العام',
        total: 'الإجمالي النهائي',
        close: 'إغلاق',
        print: 'طباعة الإيصال',
        qty: 'الكمية',
        publicPrice: 'السعر',
        unitType: 'النوع',
        receipt: 'الإيصال',
        return: 'إرجاع',
        history: 'سجل التعديلات',
        modificationHistory: 'سجل التعديلات',
        noHistory: 'لا توجد تعديلات سابقة',
      },
      invoice: 'فاتورة',
      invoiceDetails: 'تفاصيل الفاتورة',
      customerHistory: {
        title: 'سجل العميل',
      },
      customer: 'العميل',
      favorites: 'المفضلة',
      phone: 'الهاتف',
      history: 'السجل',
      items: 'أصناف',
      discount: 'خصم',
      globalDiscount: 'خصم إجمالي',
      total: 'الإجمالي',
      favoritesEmpty: 'لا توجد مفضلات بعد',
      order: 'طلب',
      orders: 'طلبات',
      addToCart: 'شراء',
      noSalesFound: 'لم يتم العثور على سجل مبيعات',
      returns: {
        processReturn: 'معالجة الإرجاع',
        returnType: 'نوع الإرجاع',
        fullReturn: 'إرجاع كامل',
        partialReturn: 'إرجاع جزئي',
        unitReturn: 'إرجاع وحدات',
        selectItems: 'اختر العناصر للإرجاع',
        returnQuantity: 'كمية الإرجاع',
        returnReason: 'سبب الإرجاع',
        reasons: {
          customer_request: 'طلب العميل',
          wrong_item: 'منتج خاطئ',
          damaged: 'تالف',
          expired: 'منتهي الصلاحية',
          defective: 'معيب',
          other: 'أخرى',
        },
        notes: 'ملاحظات إضافية',
        refundAmount: 'مبلغ الاسترداد',
        confirmReturn: 'تأكيد الإرجاع',
        cancelReturn: 'إلغاء',
        returnSuccess: 'تمت معالجة الإرجاع بنجاح',
        inventoryRestored: 'تم استعادة المخزون',
        returned: 'مُرجع',
        partiallyReturned: 'مُرجع جزئياً',
        netTotal: 'الإجمالي الصافي',
        originalTotal: 'الإجمالي الأصلي',
        returnedAmount: 'المبلغ المُرجع',
        viewReturns: 'عرض الإرجاعات',
        returnHistory: 'سجل الإرجاعات',
        noReturns: 'لا توجد إرجاعات لهذا البيع',
        step1: 'الخطوة 1: اختر نوع الإرجاع',
        step2: 'الخطوة 2: اختر العناصر',
        step3: 'الخطوة 3: السبب والملاحظات',
        step4: 'الخطوة 4: المراجعة والتأكيد',
        maxQuantity: 'الحد الأقصى',
        itemsToReturn: 'العناصر المراد إرجاعها',
        reviewReturn: 'مراجعة الإرجاع',
        willBeRestored: 'سيتم استعادتها إلى المخزون',
        selectReturnType: 'اختر كيفية معالجة هذا الإرجاع',
        fullReturnDesc: 'إرجاع جميع العناصر من هذا البيع',
        partialReturnDesc: 'إرجاع عناصر محددة',
        unitReturnDesc: 'إرجاع وحدات فردية من العبوات',
        next: 'التالي',
        back: 'رجوع',
        noItemsSelected: 'الرجاء اختيار عنصر واحد على الأقل',
        enterReason: 'الرجاء اختيار سبب الإرجاع',
        processing: 'جاري معالجة الإرجاع...',
        totalItems: 'إجمالي العناصر',
        itemsSelected: 'عناصر محددة',
        selectAll: 'تحديد الكل',
        deselectAll: 'إلغاء التحديد',
        noItemsAvailable: 'تم إرجاع جميع العناصر',
        validation: {
          noOpenShift: 'لا يمكن معالجة المرتجع - لا توجد وردية مفتوحة',
          insufficientBalance: 'مبلغ المرتجع يتجاوز رصيد المبيعات المتاح',
        },
        item: 'عنصر',
        items: 'عناصر',
        pack: 'علبة',
        packs: 'عبوات',
        unit: 'وحدة',
        units: 'وحدات',
        returnByPack: 'إرجاع بالعبوة',
        returnByUnit: 'إرجاع بالوحدة',
        switchToUnit: 'تحويل إلى وحدات',
        switchToPack: 'تحويل إلى عبوات',
        perUnit: 'للوحدة',
      },
    },
    customers: {
      title: 'إدارة العملاء',
      subtitle: 'إدارة قاعدة بيانات العملاء',
      summary: {
        total: 'إجمالي العملاء',
        newThisMonth: 'جديد هذا الشهر',
        newToday: 'جديد اليوم',
        activeTotal: 'نشط (سنوي)',
        activeRecently: 'نشط (٣٠ يوم)',
      },
      addCustomer: 'إضافة عميل',
      searchPlaceholder: 'بحث بالاسم، الهاتف، أو البريد...',
      headers: {
        name: 'الاسم',
        contact: 'معلومات الاتصال',
        purchases: 'إجمالي المشتريات',
        points: 'النقاط',
        lastVisit: 'آخر زيارة',
        status: 'الحالة',
        actions: 'إجراءات',
        branch: 'الفرع',
      },
      customerOverview: 'نظرة عامة على العملاء',
      allCustomers: 'جميع العملاء',
      customerAddedSuccess: 'تم إضافة العميل بنجاح!',
      addCustomerSubtitle: 'تسجيل عميل جديد',
      basicInfo: 'المعلومات الأساسية',
      globalView: 'العرض العام',
      showSummary: 'عرض الإحصائيات',
      hideSummary: 'إخفاء الإحصائيات',
      status: {
        active: 'نشط',
        inactive: 'غير نشط',
      },
      conditions: {
        diabetes: 'السكري',
        hypertension: 'ارتفاع ضغط الدم',
        asthma: 'الربو',
        allergies: 'الحساسية',
        heartDisease: 'أمراض القلب',
        arthritis: 'التهاب المفاصل',
      },
      contactOptions: {
        phone: 'اتصال هاتفي',
        sms: 'رسالة نصية / واتساب',
        email: 'بريد إلكتروني',
      },
      modal: {
        add: 'إضافة عميل جديد',
        edit: 'تعديل بيانات العميل',
        delete: 'حذف',
        code: 'كود العميل',
        name: 'الاسم الكامل',
        phone: 'رقم الهاتف',
        email: 'البريد الإلكتروني',
        address: 'العنوان',
        insurance: 'شركة التأمين',
        policy: 'رقم البوليصة',
        location: 'موقع الصيدلية المفضل',
        contact: 'طريقة التواصل المفضلة',
        conditions: 'الأمراض المزمنة',
        streetAddress: 'العنوان بالتفصيل',
        notes: 'ملاحظات',
        placeholders: {
          code: 'C-001',
          downtownBranch: 'مثال: فرع وسط البلد',
          streetAddress: '١٢٣ شارع الرئيسي، مبنى ٤، شقة ٥...',
          johnDoe: 'أحمد محمد',
          phone: '0106 50 123 4567',
          email: 'ahmed@example.com',
          insurance: 'مثل: بوبا، أكسا',
          policy: 'مثل: 12345678',
          notes: 'أي معلومات إضافية...',
        },
        cancel: 'إلغاء',
        save: 'حفظ العميل',
        kioskMode: 'وضع إدخال المريض',
        generateCode: 'توليد كود',
        preferences: 'التفضيلات والتأمين',
        insuranceDetails: 'تفاصيل التأمين',
        medicalInfo: 'المعلومات الطبية (اختياري)',
        register: 'تسجيل',
        editProfile: 'تعديل الملف الشخصي',
        close: 'إغلاق',
        copied: 'تم نسخ الكود للحافظة',
        kioskDesc: 'يرجى ملء بياناتك للتسجيل معنا.',
        history: 'سجل العميل',
        basicInfo: 'المعلومات الأساسية',
        additionalInfo: 'معلومات إضافية',
      },
      contextMenu: {
        showProfile: 'عرض بيانات الملف الشخصي',
        viewHistory: 'عرض السجل',
      },
      noResults: 'لم يتم العثور على عملاء.',
      invoice: 'فاتورة',
      invoiceDetails: 'تفاصيل الفاتورة',
      customerHistory: {
        title: 'سجل العميل',
        subtitle: 'عرض سجل المبيعات والمرتجعات التفصيلي',
        searchPlaceholder: 'بحث بالاسم، الكود، الهاتف، أو العنوان...',
        noCustomerSelected: 'اختر عميلاً لعرض السجل',
        stats: {
          totalSpent: 'إجمالي المبيعات',
          totalReturns: 'إجمالي المرتجعات',
          points: 'نقاط الولاء',
          visitCount: 'عدد الزيارات',
        },
        tabs: {
          invoices: 'الفواتير',
          returns: 'المرتجعات',
        },
        table: {
          date: 'التاريخ',
          invoiceNo: 'رقم الفاتورة',
          returnNo: 'رقم الإرجاع',
          items: 'الأصناف',
          total: 'الإجمالي',
          refundAmount: 'المسترد',
          status: 'الحالة',
          reason: 'السبب',
        },
        statuses: {
          completed: 'مكتمل',
          pending: 'قيد الانتظار',
          cancelled: 'ملغي',
          with_delivery: 'مع المندوب',
          on_way: 'في الطريق',
          returned: 'مرجع',
        },
      },
    },
    customerOverview: {
      title: 'نظرة عـــــامة على العمـــــلاء',
      subtitle: 'تحليلات ورؤى شاملة للعملاء',
      totalCustomers: 'إجمالي العملاء',
      active: 'نشط',
      inactive: 'غير نشط',
      lifetimeValue: 'متوسط قيمة العميل',
      perCustomer: 'لكل عميل',
      newCustomers: 'عملاء جدد',
      thisWeek: 'هذا الأسبوع',
      thisMonth: 'هذا الشهر',
      loyaltyPoints: 'نقاط الولاء',
      avgPoints: 'متوسط',
      locationDistribution: 'توزيع المواقع',
      noData: 'لا توجد بيانات',
      // Customer Reports Items
      topCustomers: 'أفضل العملاء',
      noCustomers: 'لا توجد بيانات عملاء',
      healthInsights: 'رؤى صحية',
      withInsurance: 'مع تأمين',
      withoutInsurance: 'بدون تأمين',
      chronicConditions: 'الأمراض المزمنة',
      tracked: 'أنواع متتبعة',
      patients: 'مريض',
      engagement: 'تفاعل العملاء',
      activeRate: 'معدل النشاط',
      avgDays: 'متوسط الأيام منذ الزيارة',
      inactive90: 'غير نشط 90 يوم+',
      segmentation: 'تصنيف العملاء',
      vip: 'VIP',
      regular: 'عادي',
      basic: 'أساسي',
      atRisk: 'في خطر',
      daysInactive: 'أيام غير نشط',
      contactPrefs: 'تفضيلات التواصل',
      phone: 'الهاتف',
      email: 'البريد',
      sms: 'رسائل/واتساب',
      recentActivity: 'نشاط العملاء الأخير',
      satisfaction: 'مقاييس الرضا',
      repeatRate: 'معدل التكرار',
      avgOrder: 'متوسط الطلب',
      returnRate: 'معدل الإرجاع',
      lastVisit: 'آخر زيارة',
      expand: 'توسيع',
      exportCSV: 'تصدير',
    },
    dashboard: {
      title: 'نظـــــرة عـــــامة',
      revenue: 'إجمالي الإيرادات',
      expenses: 'إجمالي المصروفات',
      profit: 'صافي الربح',
      inStock: 'منتجات في المخزون',
      lowStock: 'تنبيهات انخفاض المخزون',
      trend: 'اتجاه الإيرادات',
      attention: 'انتباه: مخزون منخفض',
      restock: 'إعادة تعبئة',
      allGood: 'جميع مستويات المخزون طبيعية.',
      expiringSoon: 'تنتهي صلاحيتها قريباً',
      mostProfitable: 'الأكثر ربحاً',
      topSelling: 'الأكثر مبيعاً',
      recentSales: 'أحدث المعاملات',
      byRevenue: 'قيمة',
      byQuantity: 'عدد',
      sold: 'مباع',
      days: 'أيام متبقية',
      topProducts: 'أفضل المنتجات',
      dailyRevenue: 'الإيرادات اليومية',
      analytics: 'التحليلات المتقدمة',
      customerCode: 'كود',
      paymentMethod: 'دفع',
      cash: 'كاش',
      visa: 'كرت',
      modal: {
        title: 'إعادة تعبئة المنتج',
        qty: 'الكمية المضافة',
        cancel: 'إلغاء',
        confirm: 'تأكيد الإضافة',
      },
      expand: {
        expand: 'توسيع',
        collapse: 'طي',
        close: 'إغلاق',
        export: 'تصدير',
        exportCSV: 'تصدير إلى CSV',
        exportImage: 'تصدير كصورة',
        filter: 'تصفية',
        sort: 'ترتيب حسب',
        dateRange: 'نطاق التاريخ',
        category: 'الفئة',
        allCategories: 'جميع الفئات',
        from: 'من',
        to: 'إلى',
        apply: 'تطبيق',
        reset: 'إعادة تعيين',
        search: 'بحث',
        showAll: 'عرض الكل',
        viewMode: 'وضع العرض',
        daily: 'يومي',
        weekly: 'أسبوعي',
        monthly: 'شهري',
        comparison: 'مقارنة مع الفترة السابقة',
        breakdown: 'التفصيل',
        metrics: 'المقاييس',
        quantity: 'الكمية',
        amount: 'المبلغ',
        profitMargin: 'هامش الربح',
        threshold: 'حد المخزون',
        bulkRestock: 'إعادة تعبئة جماعية',
        details: 'التفاصيل',
        createPurchaseOrder: 'إنشاء أمر شراء',
        timeRange: 'النطاق الزمني',
        thisWeek: 'هذا الأسبوع',
        thisMonth: 'هذا الشهر',
        next3Months: 'الأشهر الثلاثة القادمة',
        groupBy: 'تجميع حسب',
        expiryPeriod: 'فترة الانتهاء',
        bulkActions: 'إجراءات جماعية',
        markForDiscount: 'تحديد للخصم',
        removeFromInventory: 'إزالة من المخزون',
        timeline: 'عرض الجدول الزمني',
        detailedView: 'عرض تفصيلي',
        printReceipt: 'طباعة الإيصال',
        transactionDetails: 'تفاصيل المعاملة',
        historicalTrend: 'الاتجاه التاريخي (30 يوم)',
        previousPeriod: 'مقابل الفترة السابقة',
        noData: 'لا توجد بيانات متاحة',
        loading: 'جاري التحميل...',
        top20: 'أفضل 20 منتج',
        allItems: 'جميع العناصر',
        configureThreshold: 'تكوين الحد',
        supplierInfo: 'معلومات المورد',
        date: 'التاريخ',
        target: 'الهدف',
        revenue: 'الإيرادات',
        achievement: 'الإنجاز',
        upcoming: 'قادم',
      },
    },
    barcodeStudio: {
      title: 'ستوديو الباركود',
      calibration: {
        title: 'معايرة الشاشة',
        subtitle: 'ضبط المقياس لمطابقة الحجم الحقيقي',
        button: 'معايرة',
        reset: 'استعادة الافتراضي',
      },
      selectProduct: 'اختر المنتج',
      settings: 'إعدادات الملصق',
      preview: 'المعاينة الحية',
      printMode: 'وضع الطباعة',
      single: 'ملصق فردي (حراري)',
      sheet: 'ورقة A4 كاملة (30 ملصق)',
      print: 'طباعة الملصقات',
      saveTemplate: 'حفظ القالب',
      templateSaved: 'تم حفظ القالب!',
      templates: 'القوالب',
      gallery: {
        title: 'معرض القوالب',
        subtitle: 'استعرض واختبر القوالب بصورة حقيقية قبل تطبيقها',
        apply: 'تطبيق القالب',
        double: 'مزدوج',
        myTemplates: 'قوالبي المحفوظة',
        store: 'متجر القوالب',
      },
      loadTemplate: 'تحميل قالب',
      templateName: 'اسم القالب',
      saveAsNew: 'حفظ كقالب جديد',
      deleteTemplate: 'حذف',
      templatePlaceholder: 'القالب المخصص',
      createNew: 'إنشاء تصميم جديد',
      undo: 'تراجع',
      redo: 'إعادة',
      searchPlaceholder: 'ابحث عن منتج للملصق...',
      noProductSelected: 'اختر منتجاً للبدء بالتصميم',
      labelSize: 'حجم الملصق',
      width: 'العرض (مم)',
      height: 'الارتفاع (مم)',
      custom: 'حجم مخصص',
      barcodeSource: 'مصدر الباركود',
      sources: { global: 'الباركود العالمي', internal: 'الكود الداخلي' },
      borderStyle: 'الإطار',
      borders: { none: 'بدون', solid: 'متصل', dashed: 'متقطع' },
      modal: {
        cancel: 'إلغاء',
        save: 'حفظ',
      },
      inspector: {
        noSelection: 'الإعدادات العامة',
        properties: 'خصائص العنصر',
        x: 'س (مم)',
        y: 'ص (مم)',
        width: 'العرض (مم)',
        height: 'الارتفاع (مم)',
        fontSize: 'حجم الخط',
        fontWeight: 'وزن الخط',
        align: 'المحاذاة',
        content: 'المحتوى',
        remove: 'حذف العنصر',
        elements: 'العناصر',
        duplicate: 'نسخ العنصر',
        lock: 'قفل الموقع',
        unlock: 'فك القفل',
        bringToFront: 'إحضار للأمام',
        sendToBack: 'إرسال للخلف',
        defaultSet: 'تعيين كقالب افتراضي',
        rotate90: 'تدوير 90°',
      },
      elements: {
        storeName: 'اسم المتجر',
        drugName: 'اسم الدواء',
        category: 'الفئة',
        publicPrice: 'السعر',
        barcode: 'باركود',
        qrcode: 'رمز QR',
        internal: 'الكود الداخلي',
        expiry: 'تاريخ الانتهاء',
        text: 'نص مخصص',
        image: 'شعار/صورة',
        hotline: 'الخط الساخن',
        genericName: 'الاسم العلمي',
      },
      mode: {
        edit: 'تعديل',
        truePreview: 'معاينة حقيقية',
      },
      toolbar: {
        addText: 'إضافة نص',
        addImage: 'إضافة صورة',
        resetLayout: 'إعادة ضبط التصميم',
      },
      printSettings: {
        pairedPreview: 'عرض معاينة الملصق المزدوج',
        pairedPreviewDesc: 'سيتم دائمًا طباعة ملصقين معًا ليتناسب مع الرول المزدوج.',
        showBorders: 'إظهار الحدود عند الطباعة',
        showBordersDesc: 'لمعرفة الحجم الدقيق أثناء الاختبار.',
        printCalibration: 'معايرة الطباعة',
        horizontalOffset: 'أفقي (يمين/شمال)',
        verticalOffset: 'رأسي (فوق/تحت)',
        resetCalibration: 'إعادة ضبط المعايرة',
        labelGap: 'المسافة بين الملصقات',
        labelGapDesc: 'المسافة الرأسية بين الملصقات في نفس الصفحة.',
        currency: 'رمز العملة',
        currencyEGP: 'جنيه (ج.م)',
        currencyUSD: 'دولار ($)',
      },
      tooltips: {
        doubleClickRename: 'انقر مرتين لتغيير الاسم',
      },
    },
    returnHistory: {
      title: 'سجل المرتجعـــــات',
      subtitle: 'عرض وإدارة المرتجعات التي تمت معالجتها',
      searchPlaceholder: 'بحث في المرتجعات، رقم البيع، أو العميل...',
      dateFrom: 'من',
      dateTo: 'إلى',
      headers: {
        returnId: 'رقم المرتجع',
        date: 'التاريخ',
        saleId: 'رقم الفاتورة',
        customer: 'العميل',
        refundAmount: 'مبلغ الاسترداد',
        reason: 'السبب',
        actions: 'إجراءات',
        notes: 'ملاحظات',
        totalRefund: 'إجمالي الاسترداد',
        items: 'الأصناف',
      },
      noResults: 'لا توجد سجلات مرتجعات.',
      reasons: {
        customer_request: 'طلب العميل',
        wrong_item: 'صنف خاطئ',
        damaged: 'تالف',
        expired: 'منتهي الصلاحية',
        defective: 'معيب',
        other: 'أخرى',
      },
      actions: {
        viewDetails: 'عرض التفاصيل',
      },
      modal: {
        title: 'تفاصيل المرتجع',
        returnInfo: 'معلومات المرتجع',
        itemsReturned: 'الأصناف المرتجعة',
        item: 'الصنف',
        qty: 'الكمية',
        refund: 'الاسترداد',
        unit: 'وحدة',
        pack: 'علبة',
      },
    },
    loyalty: {
      overview: 'نظرة عامة على برنامج الولاء',
      lookup: 'بحث ولاء العملاء',
      activeCustomers: 'العملاء النشطين',
      totalPoints: 'إجمالي النقاط الصادرة',
      avgPoints: 'متوسط النقاط/عميل',
      topTier: 'الفئة العليا (>1000)',
      // Printer Configuration Items
      distribution: 'توزيع النقاط',
      trend: 'النقاط الصادرة (30 يوم)',
      recentActivity: 'نشاط النقاط الأخير',
      topCustomers: 'أفضل عملاء الولاء',
      searchPlaceholder: 'بحث بالاسم، الكود، أو الهاتف...',
      startSearch: 'ابحث عن عميل لعرض تفاصيل الولاء',
      currentPoints: 'النقاط الحالية',
      totalPurchases: 'إجمالي المشتريات',
      totalOrders: 'إجمالي الطلبات',
      avgOrder: 'متوسط الطلب',
      pointsHistory: 'سجل النقاط',
      noHistory: 'لا يوجد سجل مشتريات',
      orderPoints: 'نقاط الطلب',
      itemPoints: 'نقاط الأصناف',
      totalEarned: 'المكتسب',
      balance: 'الرصيد',
      notFound: 'العميل غير موجود',
      tryAgain: 'يرجى التحقق من الكود والمحاولة مرة أخرى',
      order: 'الطلب',
      pts: 'نقطة',
      totalSuffix: 'إجمالي',
    },
    realTimeSales: {
      title: 'مراقبـــــة المبيعـــــات الفورية',
      todayRevenue: 'إيرادات اليوم',
      totalTransactions: 'إجمالي المعاملات',
      itemsSold: 'الأصناف المباعة',
      activeCounters: 'الكاونترات النشطة',
      avgTransaction: 'متوسط/بيع',
      vsYesterday: 'مقابل الأمس',
      recentSales: 'المعاملات الأخيرة',
      salesByHour: 'المبيعات حسب الساعة',
      topProducts: 'أفضل المنتجات اليوم',
      paymentMethods: 'طرق الدفع',
      avgValue: 'متوسط قيمة المعاملة',
      salesVelocity: 'سرعة المبيعات',
      customerTypes: 'أنواع العملاء',
      registered: 'مسجل',
      walkIn: 'عابر',
      guest: 'ضيف',
      live: 'مباشر',
      avg: 'متوسط',
      hold: 'معلق',
      recentTransactions: 'المعاملات الأخيرة',
      filterAll: 'الكل',
      filterVip: 'عميل VIP',
      filterHighValue: 'قيمة عالية',
      vipBadge: 'VIP',
      highValueBadge: 'قيمة عالية',
      tableTime: 'الوقت',
      tableId: 'المسلسل',
      tableItems: 'الأصناف',
      tableTotal: 'الإجمالي',
      tableMethod: 'الطريقة',
      tableStatus: 'الحالة',
      noTransactions: 'لا توجد معاملات اليوم بعد.',
      salesRate: 'معدل المبيعات',
      invoices: 'الفواتير',
      newCust: 'عملاء جدد',
      perHour: 'ساعة',
      orders: 'الطلبات',
      delivery: 'توصيل',
      customers: 'العملاء',
      reg: 'مسجل',
      anon: 'عابر',
      hourlyTrend: 'مبيعات الساعة',
      byQty: 'حسب الكمية',
      noData: 'لا توجد بيانات بعد',
      salesByCategory: 'المبيعات حسب الفئة',
      medicine: 'أدوية',
      cosmetic: 'تجميل',
      general: 'عام',
      noSalesData: 'لا توجد بيانات مبيعات بعد',
      returnActivity: 'نشاط المرتجعات',
      returnsToday: 'اليوم',
      returnsProcessed: 'تم تجهيز المرتجعات',
      returnValue: 'القيمة',
      returnRate: 'المعدل',
      revenueBreakdown: 'تحليل الإيرادات',
      transactionDetails: 'معاملات اليوم',
      itemAnalysis: 'تحليل المخزون',
      topPerforming: 'المنتجات الأكثر مبيعاً',
      byQuantity: 'حسب الكمية',
      byRevenue: 'حسب الإيرادات',
      tableRank: 'الترتيب',
      tableName: 'اسم المنتج',
      tableCategory: 'الفئة',
      tableQtySold: 'الكمية المباعة',
      tableRevenue: 'الإيرادات',
      counterPerformance: 'حالة الكاونتر',
      counter: 'كاونتر',
      mainHall: 'القاعة الرئيسية',
      online: 'متصل',
      offline: 'غير متصل',
      operator: 'المشغل',
      unmanned: 'غير مراقب',
      todaySales: 'مبيعات اليوم',
      cash: 'نقدي',
      card: 'بطاقة',
      visa: 'فيزا',
      insurance: 'تأمين',
      credit: 'ائتمان',
      items: 'أصناف',
      justNow: 'الآن',
      minsAgo: 'دقيقة مضت',
      hoursAgo: 'ساعة مضت',
      noSales: 'لا توجد مبيعات اليوم',
      loadMore: 'تحميل المزيد',
      viewDetails: 'عرض التفاصيل',
      hideDetails: 'إخفاء التفاصيل',
      peakHour: 'ساعة الذروة',
      slowPeriod: 'فترة هادئة',
      salesPerHour: 'مبيعات/ساعة',
    },

    cashRegister: {
      title: 'سجل النقدية',
      subtitle: 'إدارة الورديات النقدية والمعاملات',
      status: {
        open: 'الوردية مفتوحة',
        closed: 'الوردية مغلقة',
        details: 'تفاصيل الوردية',
      },
      actions: {
        openShift: 'فتح',
        closeShift: 'إغلاق',
        addCash: 'إيداع',
        removeCash: 'سحب',
        printReport: 'طباعة التقرير',
      },
      summary: {
        openingBalance: 'الرصيد الافتتاحي',
        cashSales: 'مبيعات نقدية',
        cashIn: 'وارد نقدي',
        cashOut: 'صادر نقدي',
        closingBalance: 'الرصيد الختامي',
        expectedBalance: 'الرصيد المتوقع',
        variance: 'الفروقات',
        cardSales: 'مبيعات الكرت',
        returns: 'المرتجعات',
        cashPurchases: 'مشتريات نقدية',
        cashPurchaseReturns: 'مرتجع مشتريات نقدية',
      },
      transactions: {
        title: 'معاملات الوردية',
        time: 'الوقت',
        type: 'النوع',
        amount: 'المبلغ',
        reason: 'السبب/ملاحظات',
        user: 'المستخدم',
      },
      modal: {
        openTitle: 'فتح وردية جديدة',
        closeTitle: 'إغلاق الوردية',
        amount: 'المبلغ',
        notes: 'ملاحظات',
        cancel: 'إلغاء',
        confirm: 'تأكيد',
      },
      types: {
        opening_balance: 'الرصيد الافتتاحي',
        sale: 'بيع نقدي',
        card_sale: 'بيع كرت',
        in: 'إيداع نقدي',
        out: 'سحب نقدي',
        closing_balance: 'الرصيد الختامي',
        return: 'مرتجع',
        expense: 'مصروف',
        purchase: 'شراء',
        purchase_return: 'مرتجع شراء',
        supplier_payment: 'دفعة مورد',
      },
      messages: {
        loading: 'جاري التحميل...',
        noShift: 'افتح وردية لرؤية تفاصيل الرصيد المباشرة',
        noTransactions: 'لا توجد معاملات بعد',
        started: 'البداية',
        by: 'بواسطة',
        id: 'م',
        expected: 'المتوقع',
        countedCash: 'المبلغ المحصل',
        optionalNotes: 'ملاحظات اختيارية...',
      },
      validation: {
        amountRequired: 'يرجى إدخال المبلغ',
        negativeAmount: 'لا يمكن أن يكون المبلغ سالباً',
        positiveAmount: 'يجب أن يكون المبلغ أكبر من صفر',
        insufficientBalance: 'لا يمكن سحب أكثر من الرصيد الحالي',
        reasonRequired: 'السبب مطلوب لعمليات السحب',
        protectedBalance: 'لا يمكن السحب من الرصيد الافتتاحي. يمكن سحب المبيعات والإيداعات فقط.',
      },
      filters: {
        all: 'الكل',
        sales: 'مبيعات',
        returns: 'مرتجعات',
        purchases: 'مشتريات',
        operations: 'عمليات',
      },
      help: '@@import(./cashRegisterHelp.json)',
    },
    shiftHistory: {
      title: 'سجل الورديـــــات',
      subtitle: 'عرض ورديات سجل النقدية السابقة',
      searchPlaceholder: 'بحث برقم الوردية أو المستخدم...',
      activeNow: 'نشطة حالياً',
      dateFrom: 'من',
      dateTo: 'إلى',
      exportCSV: 'تصدير CSV',
      noResults: 'لا توجد ورديات',
      durationLabels: {
        hourSingular: 'ساعة',
        hourDual: 'ساعتين',
        hours3to10: 'ساعات',
        hours11plus: 'ساعة',
        minuteSingular: 'دقيقة',
        minuteDual: 'دقيقتين',
        minutes3to10: 'دقائق',
        minutes11plus: 'دقيقة',
        and: ' و ',
      },
      headers: {
        shiftId: 'رقم الوردية (القديم)',
        shiftNumber: 'رقم الوردية',
        systemId: 'ID',
        openTime: 'وقت الفتح',
        closeTime: 'وقت الإغلاق',
        duration: 'المدة',
        openingBalance: 'الافتتاحي',
        closingBalance: 'الختامي',
        variance: 'الفروقات',
        openedBy: 'فتحت بواسطة',
        closedBy: 'أغلقت بواسطة',
      },
      summary: {
        totalShifts: 'إجمالي الورديات',
        totalRevenue: 'إجمالي الإيرادات',
        avgPerShift: 'متوسط/وردية',
      },
      details: {
        title: 'تفاصيل الوردية',
        openedBy: 'فتحت بواسطة',
        cashSales: 'مبيعات نقدية',
        cardSales: 'مبيعات الكرت',
        transactions: 'المعاملات',
        transactionLog: 'سجل المعاملات',
        moreTransactions: 'معاملة إضافية',
        cashPurchases: 'مشتريات نقدية',
        noTransactions: 'لا توجد معاملات مسجلة',
      },
    },
    receiptDesigner: {
      title: 'مصمم الفواتير',
      subtitle: 'خصص قوالب إيصالات المبيعات الخاصة بك',
      activeTemplate: 'القالب النشط',
      newTemplate: 'قالب جديد',
      newTemplatePlaceholder: 'اسم القالب الجديد...',
      saveTemplate: 'حفظ القالب',
      deleteTemplate: 'حذف القالب',
      setDefault: 'تعيين كافتراضي',
      printTest: 'طباعة تجريبية',
      renameTemplate: 'إعادة تسمية القالب',
      printingPreferences: 'تفضيلات الطباعة',
      autoPrintDelivery: 'توصيل فقط',
      autoPrintAll: 'كل الطلبات',
      autoPrintNone: 'لا شيء',
      options: {
        storeName: 'اسم الصيدلية',
        storeSubtitle: 'العنوان الفرعي / الشعار',
        hotline: 'الخط الساخن / الهاتف',
        address: 'العنوان - السطر 1',
        area: 'المنطقة / المدينة',
        footer: 'رسالة التذييل',
        inquiry: 'للاستفسار / معلومات الاتصال',
        terms: 'الشروط والأحكام',
        font: 'نمط الخط',
        logo: 'شعار الصيدلية (Base64)',
        addressBox: 'إظهار مربع العنوان',
        delivery: 'معاينة الدليفري',
        returns: 'معاينة المرتجع',
        useBranchInfo: 'استخدام بيانات الفرع',
      },
      preview: {
        memorySize: 'حجم ذاكرة الطباعة الفعلي (الحقيقي)',
        payloadSize: 'حجم نقل البيانات الخام (الحمولة)',
        saleReceipt: 'إيصال المبيعات',
        shiftReceipt: 'إيصال تسليم وردية',
        previewAsDuplicate: 'عرض كنسخة مكررة',
      },
      storage: {
        full: 'الذاكرة ممتلئة!',
        quotaExceeded: 'تم تجاوز حصة التخزين',
        pngSvg: 'PNG / SVG',
        fileTooLarge: 'حجم الملف كبير جداً',
      },
      gallery: {
        title: 'متجر القوالب',
        subtitle: 'اختر تصميم الفاتورة المناسب لصيدليتك.',
        apply: 'تطبيق القالب',
        premium: 'قالب مميز',
        unlock: 'شراء / فتح',
        free: 'مجاني',
        layouts: {
          'layout-1': 'الأساسي (الافتراضي)',
          'layout-2': 'الحديث (داكن)',
          'layout-3': 'المضغوط (بسيط)',
        },
      },
    },
    printerSettings: {
      title: 'إعدادات الطابعة',
      subtitle: 'إعداد الطباعة الصامتة باستخدام QZ Tray',
      connection: 'الاتصال',
      enabled: 'تفعيل QZ Tray',
      status: {
        connected: 'متصل',
        disconnected: 'غير متصل',
        notInstalled: 'غير مثبت',
        connecting: 'جاري الاتصال...',
      },
      reconnect: 'إعادة الاتصال',
      download: 'تحميل QZ Tray',
      labelPrinter: 'طابعة الملصقات',
      receiptPrinter: 'طابعة الفواتير',
      selectPrinter: 'اختر الطابعة...',
      noPrinters: 'لا توجد طابعات',
      refreshList: 'تحديث قائمة الطابعات',
      testPrintLabel: 'تجربة',
      testPrintReceipt: 'تجربة',
      silentMode: 'الطباعة الصامتة',
      silentModes: {
        on: 'تشغيل',
        off: 'إيقاف',
        fallback: 'تلقائي',
      },
      silentHints: {
        on: 'طباعة صامتة دائماً (يتطلب QZ Tray)',
        off: 'إظهار نافذة الطباعة دائماً',
        fallback: 'طباعة صامتة إذا توفر QZ، وإلا نافذة الطباعة',
      },
      alerts: {
        connected: 'تم الاتصال بـ QZ Tray بنجاح',
        disconnected: 'تم قطع الاتصال بـ QZ Tray',
        printSuccess: 'تمت الطباعة بنجاح',
        printFailed: 'فشلت الطباعة',
        notInstalled: 'QZ Tray غير مثبت. يرجى التحميل من qz.io',
      },
    },
    desktop: {
      title: 'إعدادات سطح المكتب',
      subtitle: 'إدارة ميزات سطح المكتب والتكامل مع النظام',
      printer: 'الطابعة الحرارية',
      autoUpdate: 'التحديث التلقائي',
      checkUpdates: 'التحقق من التحديثات',
      version: 'إصدار البرنامج',
      systemInfo: 'معلومات النظام',
      os: 'نظام التشغيل',
      arch: 'المعمارية',
      memory: 'الذاكرة',
      upToDate: 'البرنامج محدث',
      updateAvailable: 'يتوفر تحديث جديد',
      downloading: 'جاري تحميل التحديث...',
      noUpdates: 'لا توجد تحديثات متاحة',
      errorChecking: 'خطأ في التحقق من التحديثات',
      installNow: 'تثبيت وإعادة التشغيل',
      preferredInterface: 'واجهة الطباعة المفضلة',
      interfaceAuto: 'تلقائي — تجربة Tauri أولاً ثم QZ Tray',
      interfaceTauri: 'Tauri — استخدام واجهة الطباعة النظامية فقط',
      interfaceQz: 'QZ Tray — الطباعة الصامتة عبر الشبكة',
      receiptPrinterHelper: 'فواتير حرارية 80 مم. تعمل مع Tauri و QZ Tray.',
      labelPrinterHelper: 'ملصقات صغيرة (مثال 38×12 مم). يُوصى باستخدام QZ Tray للأحجام المخصصة.',
      linuxWarning: 'الطابعات الحرارية عبر Tauri قد تواجه قيوداً على لينكس. يُنصح باستخدام QZ Tray للاستقرار.',
      windowsInfo: 'ويندوز مدعوم بالكامل. يُوصى باستخدام QZ Tray لطباعة الملصقات.',
      osVersion: 'إصدار نظام التشغيل',
    },
    stockMovement: {
      title: 'حركة الصنف',
      summary: 'الملخص',
      showAll: 'إظهار الكل',
      todayOnly: 'صنف محدد',
      allProductsReport: 'تقرير كل الأصناف',
      allMovements: 'جميع الحركات',
      details: 'التفاصيل',
      stockIn: 'وارد',
      stockOut: 'صادر',
      returns: 'مرتجع',
      netMovement: 'صافي الحركة',
      currentStock: 'الرصيد الحالي',
      branch: 'الفرع',
      allBranches: 'كل الفروع',
      export: 'تصدير CSV',
      noMovements: 'لا توجد حركات مسجلة لهذا الصنف.',
      searchDrug: 'ابحث عن صنف لعرض حركته...',
      loading: 'جاري تحميل الحركات...',
      stats: {
        totalIn: 'إجمالي الوارد',
        totalOut: 'إجمالي الصادر',
        netChange: 'صافي التغير في المخزون',
      },
      table: {
        date: 'التاريخ',
        type: 'النوع',
        qty: 'الكمية',
        batch: 'الباتش',
        reason: 'السبب',
        user: 'المستخدم',
        ref: 'المرجع',
        prev: 'قبل',
        next: 'بعد',
      },
      types: {
        sale: 'مبيعات',
        purchase: 'مشتريات',
        return_customer: 'مرتجع مبيعات',
        return_supplier: 'مرتجع مشتريات',
        adjustment: 'تسوية مخزنية',
        damage: 'هالك / تالف',
        transfer_in: 'تحويل وارد',
        transfer_out: 'تحويل صادر',
        correction: 'تصحيح بيانات',
        initial: 'رصيد أول المدة',
      },
      filters: {
        dateRange: 'الفترة الزمنية',
        allTypes: 'كل الأنواع',
      },
    },
    expiryManagement: {
      invalidQuantity: 'الكمية غير صحيحة',
      actionSuccess: 'تم تحديث المخزون بنجاح',
      actions: 'إجراءات',
      returnToSupplier: 'إرجاع للمورد',
      damageStock: 'إعدام توالف',
      potentialLoss: 'خسارة محتملة',
      status: {
        valid: 'صالح',
        expired: 'منتهي الصلاحية',
      },
      time: {
        days: 'يوم',
      },
      details: {
        packs: 'علبة',
      },
      title: 'إدارة الصلاحيـــــات',
      subtitle: 'تتبع تواريخ انتهاء الصلاحية والتنبيهات',
      expiredItems: 'أصناف منتهية',
      nearExpiry30: 'ينتهي < 30 يوم',
      nearExpiry90: 'ينتهي < 90 يوم',
      all: 'الكل',
      expired: 'منتهي',
      searchPlaceholder: 'ابحث في التشغيلات...',
      noRecords: 'لم يتم العثور على نتائج تطابق بحثك',
      batch: 'التشغيلة',
      available: 'المتاح',
      quantity: 'الكمية',
      notes: 'ملاحظات',
      optional: 'اختياري',
      addNotes: 'أضف أي تفاصيل أو أسباب...',
      cancel: 'إلغاء',
      confirm: 'تأكيد',
      table: {
        codeOrBatch: 'الكود / التشغيلة',
        name: 'الاسم',
        remainingQty: 'الكمية المتبقية',
        costPrice: 'سعر التكلفة',
        potentialLoss: 'خسارة محتملة',
        expiryDate: 'تاريخ الانتهاء',
      },
    },
    // ═══════════════════════════════════════════
    // Attendance Module
    // ═══════════════════════════════════════════
    attendance: {
      title: 'لوحة الحضور',
      subtitle: 'تسجيل الحضور بالبصمة',
      reportsTitle: 'تقارير الحضور',
      reportsSubtitle: 'مراقبة حضور وانضباط الموظفين',
      showPresentOnly: 'عرض الحاضرين فقط',
      profileSubtitle: 'مراجعة سجل الحضور الشهري والمؤشرات',
      // Terminal States
      terminalLocked: 'الجهاز مقفل',
      enterToken: 'أدخل رمز الجهاز',
      tokenPlaceholder: 'الصق رمز الجهاز هنا...',
      activateTerminal: 'تفعيل الجهاز',
      invalidToken: 'رمز الجهاز غير صحيح. تواصل مع المسؤول.',
      terminalNotActivated: 'لوحة الحضور غير مفعّلة.',
      contactAdmin: 'تواصل مع المسؤول لتفعيلها.',
      accessDenied: 'ليس لديك صلاحية الوصول إلى لوحة الحضور.',
      // Clock Actions
      clockIn: 'تسجيل دخول',
      clockOut: 'تسجيل انصراف',
      selectEmployee: 'اختر الموظف',
      searchEmployee: 'بحث عن موظف...',
      // Timeline
      todayTimeline: 'سجل اليوم',
      noEventsToday: 'لا توجد أحداث حضور مسجلة اليوم.',
      eventIn: 'دخول',
      eventOut: 'انصراف',
      // Status
      currentlyIn: 'حاضر حالياً',
      currentlyOut: 'منصرف حالياً',
      notClockedIn: 'لم يسجل حضور بعد',
      // Biometric
      touchSensor: 'المس مستشعر البصمة',
      biometricVerified: 'تم التحقق من البصمة',
      biometricFailed: 'فشل التحقق من البصمة',
      noBiometric: 'لا توجد بصمة مسجلة لهذا الموظف',
      // Branch Settings
      terminalToken: 'رمز جهاز الحضور',
      generateToken: 'توليد رمز جديد',
      regenerateToken: 'إعادة توليد الرمز',
      tokenGenerated: 'تم توليد رمز جديد بنجاح',
      tokenWarning: 'توليد رمز جديد سيلغي الرمز السابق فوراً.',
      copyToken: 'نسخ الرمز',
      tokenCopied: 'تم نسخ الرمز',
      shiftStartDesc: 'تحديد موعد بدء العمل لاحتساب التأخير',
      saveTime: 'حفظ الوقت',
      terminalTokenDesc: 'رمز تفعيل جهاز الحضور لهذا الفرع',
      generateTokenHint: 'ولّد رمز جديد وانسخه والصقه في جهاز الحضور لتفعيله',
      // Success/Error
      clockInSuccess: 'تم تسجيل الدخول بنجاح',
      clockOutSuccess: 'تم تسجيل الانصراف بنجاح',
      clockError: 'فشل تسجيل الحضور',
      // PIN Fallback
      enterPin: 'أدخل رمز PIN',
      pinPlaceholder: 'رمز PIN ٤ أرقام',
      invalidPin: 'رمز PIN غير صحيح',
      setPin: 'تعيين رمز حضور PIN',
      unverifiedClock: 'غير موثق (بدون بصمة أو PIN)',
      pinOrBiometric: 'استخدم البصمة أو أدخل PIN',
      noPinNoBiometric: 'لا توجد بصمة أو PIN مسجل',
      fingerprintOnly: 'يجب تسجيل الحضور بالبصمة فقط لهذا الموظف.',
      // Working Hours
      totalHours: 'إجمالي الساعات',
      ongoing: 'جارٍ',
      sessionDuration: 'مدة الجلسة',
      // Reports & Lateness
      shiftStartTime: 'بداية الوردية',
      graceMinutes: 'فترة السماح (دقائق)',
      attendanceReports: 'تقارير الحضور',
      present: 'حاضر',
      absent: 'غائب',
      late: 'متأخر',
      avgHours: 'متوسط الساعات',
      firstIn: 'أول دخول',
      lastOut: 'آخر انصراف',
      lateStatus: 'الحالة',
      lateMinutes: 'تأخير (دقيقة)',
      noReportData: 'لا توجد بيانات حضور لهذا التاريخ.',
      employeeCode: 'الكود',
      employeeName: 'الموظف',
      lateStatusOk: 'في الموعد',
      lateStatusLate: 'متأخر',
      // Employee Attendance Profile
      attendanceProfile: 'ملف الحضور',
      attendanceRate: 'نسبة الحضور',
      totalHoursMonth: 'إجمالي الساعات',
      lateDays: 'أيام التأخير',
      absentDays: 'أيام الغياب',
      day: 'اليوم',
      date: 'التاريخ',
      backToReports: 'رجوع للتقارير',
      avgDailyHours: 'متوسط يومي',
      totalLateMinutes: 'إجمالي التأخير',
      nextDay: '+1',
      noDataFound: 'لا توجد بيانات',
      employeeNotFound: 'الموظف غير موجود',
      quickAttendance: 'تسجيل الحضور السريع',
      usernamePlaceholder: 'اسم المستخدم...',
      authFailed: 'فشل التحقق',
      hourSymbol: 'ساعة',
      minuteSymbol: 'د',
    },
    shortages: {
      title: 'النواقص والإنذارات التنبؤية',
      subtitle: 'تحسين المخزون بالذكاء الاصطناعي والتنبؤ بنقص الأدوية',
      statsTotal: 'المنتجات المراقبة',
      statsCritical: 'نفد من المخزون',
      statsPredictive: 'تحذيرات تنبؤية',
      statsHealthy: 'مخزون سليم',
      searchPlaceholder: 'البحث باسم الدواء، المادة الفعالة أو الباركود...',
      filterCategory: 'كل الأقسام',
      filterAlert: 'كل أنواع الإنذارات',
      tableProduct: 'الاسم',
      tableStock: 'المخزون الحالي',
      tableMinStock: 'الحد اليدوي',
      tableVelocity: 'معدل البيع اليومي',
      tableStockDays: 'الأيام المتبقية',
      tableSuggestedQty: 'الطلب المقترح',
      tableAbcClass: 'أولوية ABC',
      tableAlertStatus: 'حالة الإنذار',
      tableActions: 'إجراءات',
      reorderBtn: 'إنشاء طلب شراء',
      statusOutOfStockSold: 'بيع مؤخراً',
      statusManualMinimumReached: 'تجاوز الحد الأدنى',
      statusPredictiveShortage: 'نفاد متوقع',
      statusOutOfStockDefault: 'غير نشط',
      statusNormal: 'طبيعي',
      abcDescription:
        'يصنف تحليل ABC المنتجات حسب سرعة المبيعات: A = أعلى 80% من المساهمة، B = المتوسط 15%، C = منخفض 5%',
      predictedRunoutText: 'النفاد المتوقع خلال {days} يوم',
      suggestedOrderTooltip:
        'يتم احتسابه كالتالي: معدل بيع 14 يوم × 1.5 هامش أمان - المخزون الحالي',
      noShortagesFound:
        'لم يتم العثور على أي نواقص أو إنذارات في هذا الفرع. مستويات المخزون سليمة!',
      warningNoDataToExportTitle: 'لا توجد بيانات لتصديرها!',
      warningNoDataToExportDesc: 'يرجى تحديد أدوية أو التأكد من وجود نواقص.',
      exportSuccess: 'تم تصدير النواقص بنجاح!',
      exportSuccessDesc: 'تم تحميل ملف CSV يحتوي على {count} منتج.',
      warningNoItemsSelectedTitle: 'لم يتم تحديد أدوية!',
      warningNoItemsSelectedDesc: 'الرجاء اختيار الأدوية التي ترغب في إعادة طلبها أولاً.',
      navigatingToPurchases: 'جاري التحويل لصفحة المشتريات...',
      navigationError: 'خطأ في التوجيه',
      navigationErrorDesc: 'عذراً، لم نتمكن من العثور على محرك التوجيه لصفحة المشتريات.',
      preparedOrderDesc: 'تم تجهيز طلب شراء يحتوي على {count} منتج.',
      exportButtonLabel: 'تصدير النواقص',
      exportSelectedButtonLabel: 'تصدير المحدد',
      units: 'وحدة',
      daysLeft: 'يوم متبقي',
      itemsSelectedText: '{count} عنصر تم تحديده',
      clearSelection: 'إلغاء التحديد',
      focusModeToggle: 'وضع التركيز (المحدد فقط)',
      allGoodText: 'كل شيء على ما يرام!',
      exportCsvFilename: 'تقرير_النواقص',
      csvHeaderProduct: 'اسم المنتج',
      csvHeaderGeneric: 'الاسم العلمي',
      csvHeaderCategory: 'القسم',
      generalCategory: 'عام',
      weekAbbreviation: 'أسبوع',
    },
    newTests: {
      title: 'مختبر شريط التمرير',
      subtitle: 'بيئة اختبار متميزة لأشرطة التمرير المخصصة وأنماط الواجهة',
      description:
        'بيئة تفاعلية مصممة للمطورين لاختبار ونسخ تصميمات أشرطة التمرير المخصصة والمتقدمة.',
      welcome: 'مرحباً بك في مختبر شريط التمرير',
      getStarted: 'هل أنت جاهز لكتابة ميزتك التالية؟',
      designAesthetic: 'لغة التصميم المتميزة نشطة',
    },
    a5InvoiceDesigner: {
      title: 'مصمم فواتير المشتريات A5',
      subtitle: 'تصميم ومعاينة واختبار طباعة فواتير شركات ومخازن الأدوية المصرية بدقة فائقة',
      description:
        'مختبر تفاعلي لتخصيص فواتير A5 لأشهر موزعي الأدوية في مصر وطباعتها بمقاسات وهوامش مثالية.',
    },
    expenses: {
      title: 'متتبع المصروفات',
      recordExpense: 'تسجيل مصروف',
      filters: {
        today: 'اليوم',
        thisWeek: 'هذا الأسبوع',
        thisMonth: 'هذا الشهر',
        custom: 'فترة مخصصة',
      },
      summary: {
        total: 'إجمالي المصروفات',
        cashTotal: 'المصروفات النقدية',
        count: 'عدد المعاملات',
        largestCategory: 'الفئة الأكبر',
      },
      categories: {
        utilities: 'الخدمات (كهرباء/مياه)',
        rent: 'الإيجار',
        maintenance: 'الصيانة',
        supplies: 'المستلزمات والأدوات',
        petty_cash: 'نثرية / سحب نقدي',
        transportation: 'الانتقالات / الشحن',
        salaries: 'المرتبات والأجور',
        misc: 'مصاريف أخرى متنوعة',
      },
      modal: {
        title: 'تسجيل مصروف جديد',
        amount: 'المبلغ',
        category: 'الفئة',
        description: 'البيان / الوصف',
        paymentMethod: 'طريقة الدفع',
        cash: 'نقدي (خزينة)',
        bank_transfer: 'تحويل بنكي',
        card: 'بطاقة / فيزا',
        confirm: 'تسجيل المصروف',
        cancel: 'إلغاء',
        shiftBalance: 'النقدية الحالية بالوردية',
        noShiftWarning: 'لا توجد وردية مفتوحة حالياً. سيتم تسجيل المصروف ولكن لن يؤثر على الخزينة.',
      },
      validation: {
        amountRequired: 'المبلغ مطلوب',
        positiveAmount: 'يجب أن يكون المبلغ أكبر من 0',
        descriptionRequired: 'الوصف مطلوب',
        insufficientBalance: 'المبلغ يتجاوز النقدية المتاحة في الوردية الحالية فوق مبلغ البداية',
      },
      table: {
        time: 'الوقت',
        id: 'المرجع',
        category: 'الفئة',
        description: 'البيان',
        paymentMethod: 'طريقة الدفع',
        amount: 'المبلغ',
        employee: 'بواسطة',
      },
      empty: 'لم يتم تسجيل أي مصروفات في هذه الفترة',
    },
    dashboardExperiments: {
      title: 'تجارب بطاقات لوحة التحكم',
      card1: 'اتجاه المبيعات (مخطط خطي)',
      card2: 'الإيرادات حسب الفئة (مخطط شريطي)',
      card3: 'شرائح العملاء (مخطط دائري)',
      card4: 'حالة المخزون (مخطط دائري مفرغ)',
      card5: 'حركة الزوار اليومية (مخطط مساحي)',
      card6: 'أهداف الأداء (مخطط شعاعي)',
      card7: 'إنجاز المهام (أشرطة التقدم)',
      card8: 'إحصائية سريعة (مع خط مصغر)',
      card9: 'نشاط أسبوعي (خريطة حرارية)',
      card10: 'السعر مقابل المبيعات (مخطط انتشار)',
      card11: 'المبيعات الشهرية (أعمدة مكدسة)',
      card12: 'قمع المبيعات',
      card13: 'مؤشر الهدف',
      card14: 'أفضل المنتجات (أعمدة أفقية)',
      card15: 'الإيرادات مقابل الطلبات (مخطط مركب)',
      totalRevenue: 'إجمالي الإيرادات',
      vsLastMonth: 'مقارنة بالشهر الماضي',
      morning: 'صباحاً',
      afternoon: 'ظهراً',
      evening: 'مساءً',
      targetAchieved: 'تحقيق الهدف',
      salesTarget: 'هدف المبيعات',
      customerSatisfaction: 'رضا العملاء',
      inventoryTurnover: 'معدل دوران المخزون',
    },
    employeeProfile: {
      profile: 'الملف',
      workHistory: 'السجل',
      documents: 'المستندات',
      sessions: 'الجلسات',
      independentEmployee: 'موظف مستقل',
      personalInformation: 'المعلومات الشخصية',
      cancel: 'إلغاء',
      save: 'حفظ',
      edit: 'تعديل',
      fullName: 'الاسم الكامل',
      nameArabic: 'الاسم بالعربية',
      loginAndFingerprint: 'بيانات الدخول والبصمة',
      workspacesAndCredentials: 'بيانات العمل والصلاحيات',
      duration: 'فترة العمل',
      achievementsAndNotes: 'الإنجازات والملاحظات',
      changePassword: 'تغيير كلمة المرور',
      newPassword: 'كلمة المرور الجديدة',
      setupPasskey: 'إعداد البصمة',
      days: 'أيام',
      months: 'شهور',
      years: 'سنين',
      and: 'و',
      unknownBranch: 'فرع غير معروف',
      noFingerprint: 'لا توجد بصمة',
      fingerprintEnabled: 'البصمة مفعلة',
      username: 'اسم المستخدم',
      email: 'البريد الإلكتروني',
      phone: 'الهاتف',
      licenseNo: 'رقم الترخيص',
      memberSince: 'عضو منذ',
      overview: 'نظرة عامة',
      employers: 'جهات العمل',
      pending: 'معلق',
      totalRequests: 'إجمالي الطلبات',
      officialDocuments: 'المستندات الرسمية',
      nationalIdFront: 'البطاقة الشخصية (الوجه الأمامي)',
      nationalIdBack: 'البطاقة الشخصية (الوجه الخلفي)',
      syndicateCard: 'كارنيه النقابة',
      subSyndicateCard: 'كارنيه النقابة الفرعية',
      noWorkHistory: 'لا يوجد سجل وظيفي بعد',
      pendingRequests: 'الطلبات',
      noFile: 'لا يوجد ملف',
      operationFailed: 'فشل في تنفيذ العملية',
      sentBy: 'أرسل من:',
      rejecting: 'جارٍ الرفض...',
      accepting: 'جارٍ القبول...',
      darkMode: 'الوضع الليلي',
      signOut: 'تسجيل الخروج',
      language: 'اللغة',
      menu: 'القائمة',
      prescriptionPricing: 'التسعير',
      copied: 'تم النسخ ✓',
      noPendingRequestsDesc:
        'عندما ترسل لك أي صيدلية طلب توظيف، سيظهر هنا. يمكنك قبوله للوصول إلى نظام إدارة الصيدلية الخاص بهم.',
      noWorkHistoryDesc: 'عندما تقبل طلب توظيف من إحدى الصيدليات، سيظهر هنا في سجل عملك.',
      fileTooLarge: 'حجم الملف كبير جداً (الحد الأقصى {{size}} كيلوبايت)',
    },
    employeeSetup: {
      expired: 'انتهت الصلاحية',
      invitationRejected: 'تم رفض الدعوة من قبل المستخدم',
      pleaseEnterUsername: 'يرجى إدخال البريد الإلكتروني أو اسم المستخدم',
      userNotFound: 'المستخدم غير موجود. يجب عليه إنشاء حساب أولاً.',
      errorOccurred: 'حدث خطأ',
      failedToCancel: 'فشل إلغاء الدعوة، ربما تم قبولها بالفعل',
      fillRequiredFields: 'يرجى ملء جميع الحقول المطلوبة',
      passwordsDoNotMatch: 'كلمات المرور غير متطابقة',
      passwordLength: 'يجب أن تتكون كلمة المرور من 4 أحرف على الأقل',
      goBack: 'الرجوع',
      accountSetup: 'إعداد الحساب',
      inviteAdminInstructions:
        'أرسل دعوة لمدير أو مالك الصيدلية لإكمال إعدادات النظام وتفعيل الحساب.',
      globalEmailOrUsername: 'البريد الإلكتروني أو اسم المستخدم العالمي',
      emailExample: 'مثال: ahmed@email.com',
      autoAcceptNote: 'إذا قمت بإدخال بريدك الحالي، سيتم قبول الدعوة تلقائياً.',
      role: 'الصلاحية',
      expirationTime: 'صلاحية الدعوة',
      oneHour: 'ساعة واحدة',
      oneDay: 'يوم واحد',
      noLimit: 'بلا حد',
      sendInvitation: 'إرسال الدعوة',
      invitationExpiresIn: 'تنتهي الدعوة في:',
      cancelInvitation: 'إلغاء الدعوة',
      accepted: 'تم القبول',
      localUsernameAuto: 'اسم المستخدم المحلي (تلقائي)',
      quickLoginNote: 'هذا هو الرمز الموحد للدخول السريع الخاص بالفرع. لا يمكن تغييره.',
      localPassword: 'كلمة المرور المحلية',
      confirmPassword: 'تأكيد المرور',
      finishAndSave: 'إنهاء وحفظ',
      step3Admin: 'الخطوة ٣: قم بدعوة مدير الفرع للبدء في استخدام النظام.',
      cannotInviteSelf: 'لا يمكنك دعوة حساب الإدارة. يرجى إرسال الدعوة إلى حساب موظف منفصل.',
      waitingForAcceptance: 'جاري انتظار قبول الدعوة من',
    },
    secureGate: {
      identityVerification: 'تحقق من الهوية',
      tooManyAttempts: 'تم قفل المحاولات',
      passwordRequired: 'تأكيد كلمة المرور',
      biometricRequired: 'التحقق بالبصمة',
      exceededAttempts: 'لقد تجاوزت الحد الأقصى للمحاولات.',
      enterPassword: 'يرجى إدخال كلمة المرور للمتابعة',
      incorrectPassword: 'كلمة المرور غير صحيحة',
      unlockArea: 'دخول المنطقة',
      protectedArea: 'منطقة محمية',
      requiresPassword: 'هذه المنطقة تتطلب التحقق للوصول إليها',
      unlockNow: 'فتح المنطقة',
      authorizedTitle: 'مسموح بالدخول',
      authorizedDesc: 'لديك الصلاحيات اللازمة للوصول إلى هذه المنطقة.',
      unauthorizedTitle: 'تم رفض الوصول',
      unauthorizedDesc:
        'ليس لديك الصلاحية للوصول إلى هذه المنطقة. الدخول يقتصر على المدراء والملاك.',
      confirmUnlock: 'تأكيد الهوية',
      lockoutTimer: 'تم القفل.. حاول بعد {{timeLeft}} ثانية',
      usernamePlaceholder: 'اسم المستخدم',
      passwordPlaceholder: 'كلمة المرور',
      verifying: 'جاري التحقق...',
      confirmUnlockBtn: 'تأكيد الدخول',
      useBiometrics: 'استخدام البصمة بدلاً من ذلك',
      scanBiometric: 'مسح البصمة للدخول',
      usePassword: 'استخدام كلمة المرور بدلاً من ذلك',
      usernameRequiredError: 'يرجى إدخال اسم المستخدم',
      passwordRequiredError: 'يرجى إدخال كلمة المرور',
      usernameNotFoundError: 'اسم المستخدم غير صحيح أو غير مسجل',
      unauthorizedErrorMessage: 'ليس لديك صلاحية الوصول لهذه المنطقة',
      verificationFailedError: 'فشل التحقق من الهوية. يرجى المحاولة مرة أخرى.',
      biometricFailedError: 'فشل التحقق بالبصمة. يرجى المحاولة مرة أخرى.',
    },
    updater: {
      updateAvailableTitle: 'تحديث جديد متوفر',
      updateAvailableMsg: 'يوجد تحديث جديد (v{{version}}) جاهز للتثبيت.',
      restartNow: 'إعادة التشغيل الآن',
      updateNow: 'تحديث الآن',
      later: 'لاحقاً',
      downloading: 'جاري تنزيل التحديث في الخلفية...',
      appVersion: 'نسخة النظام',
      error: 'فشل تحديث التطبيق.',
    },
  },
};
