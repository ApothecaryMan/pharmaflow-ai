...
function checkTranslations() {
    ...
    try {
        const menuTranslationContent = fs.readFileSync(path.join(__dirname, '../menuTranslations.ts'), 'utf8');
...
function checkFileContent(content, filename) {
    ...
    if (!enMatch || !arMatch) {
        ...
        console.log(`${colors.yellow}Could not parse structure of ${filename}. Skipping detail check.${colors.reset}`);
        return false; 
    }
