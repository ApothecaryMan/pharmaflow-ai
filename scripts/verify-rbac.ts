...
import { ROLE_PERMISSIONS, PermissionAction } from '../config/permissions.ts';
import { PHARMACY_MENU, MenuItem } from '../config/menuData.ts';
                ...
                sub.items.forEach(subItem => {
                    if (typeof subItem === 'object' && subItem.permission) {
                        if (!DEFINED_PERMISSIONS.has(subItem.permission)) {
                            menuErrors.push(`[MENU] Undefined permission '${subItem.permission}' at: ${subPath} > ${subItem.label}`);
                        }
                    }
                });
            });
