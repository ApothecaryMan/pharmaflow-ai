...
import { loadState, saveState, cssToProps, propsToCss, type ThemeState } from './state.js';
...
export function registerAllTools(server: McpServer, broadcast: BroadcastFn) {
  ...
  const getCssString = () => propsToCss(state.properties);
  ...
  server.registerTool(
    'set_css_property',
    {
      description: 'Set a single CSS property (e.g. "border-radius" = "12px"). Use empty string to remove.',
      inputSchema: z.object({
    ...
    async (args) => {
      ...
      return {
        content: [{ type: 'text' as const, text: `Set ${args.name}: ${args.value || '<removed>'}` }],
      };
    }
  );
  ...
  server.registerTool(
    ...
    async (args) => {
      ...
      await sync({ ...loaded, properties: props });
      return {
        content: [{ type: 'text' as const, text: `Updated ${Object.keys(args.properties).length} properties` }],
      };
    }
  );
  ...
  server.registerTool(
    ...
    async (args) => {
      ...
      await sync({ ...loaded, properties: props });
      return {
        content: [{ type: 'text' as const, text: `Applied ${Object.keys(props).length} properties from raw CSS` }],
      };
    }
  );
  ...
  server.registerTool(
    ...
    async (args) => {
      ...
      return {
        content: [{ type: 'text' as const, text: `Custom CSS ${args.enabled ? 'enabled' : 'disabled'}` }],
      };
    }
  );
  ...
  server.registerTool(
    ...
    async () => {
      return {
        content: [{
          type: 'text' as const,
          text: JSON.stringify(PRESETS.map((p) => ({
            name: p.name,
            label: p.label,
  ...
  server.registerTool(
    ...
    async (args) => {
      ...
      if (!preset) {
        return {
          content: [{ type: 'text' as const, text: `Preset "${args.name}" not found. Use list_presets to see available options.` }],
          isError: true,
        };
      }
      const loaded = await loadState();
      ...
      return {
        content: [{ type: 'text' as const, text: `Applied preset "${preset.label}": ${Object.keys(preset.properties).length} properties set` }],
      };
    }
  );
}
