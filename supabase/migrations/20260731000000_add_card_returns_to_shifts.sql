-- ============================================================
-- Migration: Add card_returns to shifts to separate Visa returns
-- Description: Adds card_returns column, updates atomic_increment_shift, and process_return
-- ============================================================

BEGIN;

-- 1. Add column to shifts
ALTER TABLE shifts ADD COLUMN IF NOT EXISTS card_returns NUMERIC DEFAULT 0;

-- 2. Update atomic_increment_shift
DROP FUNCTION IF EXISTS public.atomic_increment_shift(UUID, NUMERIC, NUMERIC, NUMERIC, NUMERIC, NUMERIC, NUMERIC, NUMERIC);
...
CREATE OR REPLACE FUNCTION public.atomic_increment_shift(
  ...
  p_card_returns NUMERIC DEFAULT 0
)
RETURNS void AS $$
DECLARE
...
BEGIN
  ...
  UPDATE shifts SET
    ...
    card_returns = COALESCE(card_returns, 0) + p_card_returns
  WHERE id = p_shift_id;
END;
$$ LANGUAGE plpgsql SECURITY DEFINER;


-- 3. Update process_return
