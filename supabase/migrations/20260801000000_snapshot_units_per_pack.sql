-- ============================================================
-- Snapshot units_per_pack at sale time for historical accuracy
--
-- Why: Velocity calculations use the CURRENT drug.units_per_pack
-- to normalize historical sales, but unitsPerPack can change
-- over time, corrupting any historical report.
--
-- This migration:
-- 1. Adds units_per_pack to sale_items
-- 2. Backfills historical rows from the current drug value
-- 3. Updates process_checkout to snapshot the value at sale time
-- ============================================================
...
-- 2. Backfill existing rows with the drug's current units_per_pack
UPDATE sale_items si
SET units_per_pack = COALESCE(
  (SELECT d.units_per_pack FROM drugs d WHERE d.id = si.drug_id),
  1
)
WHERE si.units_per_pack IS NULL;

-- 3. Redefine process_checkout to snapshot units_per_pack
