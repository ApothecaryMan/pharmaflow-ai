-- ============================================================
-- Fix null name in return_items when client sends wrong drugId
-- The process_return RPC used the payload's drugId to look up
-- the drug name. If the client sent a wrong drugId (e.g., sale
-- item ID instead of drug ID), the drug lookup returned NULL
-- for name, causing a NOT NULL violation.
--
-- Fix: override v_drug_id with v_sale_item_record.drug_id
-- after the sale item lookup, ensuring all subsequent lookups
-- use the correct drug ID from the sale_items table.
-- ============================================================
...
BEGIN
    ...
    LOOP
        ...
        -- Use the actual drug_id from sale_items (FK-guaranteed to exist in drugs)
        -- instead of trusting the client-supplied drugId
