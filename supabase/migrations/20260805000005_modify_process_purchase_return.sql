-- ============================================================
-- T2.2 — process_purchase_return → credit-note ledger path
-- Date: 2026-08-05
--
-- 1. Stores the refund method on purchase_returns.payment_method so
--    aging / fn_purchase_open_amount can attribute credit notes to the
--    exact invoice.
-- 2. Cash returns   → unchanged (cash-in + shift cash_purchase_returns).
-- 3. Credit returns → NO cash; inserts a ledger entry
--    ('credit_note', −total_refund) which reduces the supplier's
--    payable AND the original purchase's open amount (aging).
-- ============================================================

BEGIN;

ALTER TABLE purchase_returns ADD COLUMN IF NOT EXISTS payment_method purchase_pay_type;
