-- ============================================================
-- T2.1 — process_purchase_receipt → unified credit + linked payment
-- Date: 2026-08-05
--
-- Changes vs the 20260722 version:
--   1. ALWAYS inserts the full-payable ledger entry
--      ('purchase', +total_cost, due_date = date + supplier.credit_days).
--   2. Sets purchases.due_date.
--   3. If paidNow > 0 (Cash = total, Partial = portion), routes the
--      immediate payment through record_supplier_payment('receipt')
--      which writes the linked supplier_payments row + allocation +
--      cash tx ('purchase') + shift cash_purchases counter.
--      The old inline cash block is removed — single shared path.
--   4. Back-compat: when paidNow is omitted, cash → paidNow = total,
--      credit → paidNow = 0 (existing UI keeps working unchanged).
-- ============================================================
