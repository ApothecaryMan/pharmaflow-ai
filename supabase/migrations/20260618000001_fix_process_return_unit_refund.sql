-- Migration: Fix process_return unit refund calculation
-- When a unit was returned, the refund was incorrectly calculated using the full pack price
-- because sale_items.public_price stores the pack price.
-- This migration updates process_return to scale the public_price by units_per_pack
-- when processing a unit return.
...
BEGIN
    ...
    LOOP
        ...
        IF v_sale_item_record.is_unit THEN
            v_effective_price := v_sale_item_record.public_price / COALESCE(NULLIF(v_drug_record.units_per_pack, 0), 1);
        ELSE
            v_effective_price := v_sale_item_record.public_price;
        END IF;
        ...
        v_item_refund := ROUND(v_qty * v_effective_price * (v_sale_record.total / NULLIF(v_sale_record.subtotal, 0)), 2);
