-- Migration: Half-open date range boundaries [start, end)
--
-- Problem: All financial RPCs used <= p_date_to (fully inclusive), but the
-- date range service sent 23:59:59.999 of the last day. Current and previous
-- comparison periods overlapped by one full day, skewing KPI trends.
--
-- Fix: Change to < p_date_to (exclusive end). Callers pass the start of the
-- day AFTER the last day (e.g., 2026-06-01T00:00:00.000Z for "all of May").
--
-- Convention: [p_date_from, p_date_to) — from inclusive, to exclusive.
...
BEGIN
    ...
    -- End is exclusive: start of the next month
    v_date_from_local := date_trunc('month', (p_month_key || '-01')::DATE);
    v_date_from_utc := v_date_from_local AT TIME ZONE 'Africa/Cairo';
    v_date_to_utc := (v_date_from_local + INTERVAL '1 month') AT TIME ZONE 'Africa/Cairo';

    -- Calculate metrics using compute_financial_summary (now uses < p_date_to)
...
-- 3. RPC: compute_financial_summary_with_snapshots
...
BEGIN
    ...
    LOOP
        ...
        v_month_end := (v_month.m + INTERVAL '1 month') AT TIME ZONE 'Africa/Cairo';
        v_month_key := to_char(v_month.m, 'YYYY-MM');

        -- Check if this month is fully within the range and is in the past
        -- v_month_end is exclusive (start of next month)
...
-- 4. RPC: get_daily_financial_breakdown
...
-- 5. RPC: get_top_products_financial
...
-- 6. RPC: get_category_financial_breakdown
