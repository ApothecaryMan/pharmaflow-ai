-- =============================================================================
-- Fix Stock Movement Double-Counting Bug
-- Makes the fn_log_stock_movement trigger respect the 'app.disable_stock_sync'
-- session variable. Functions like 'process_order_modification' use this
-- to prevent the trigger from automatically recording stock changes when they
-- intend to do it manually.
-- =============================================================================
...
DECLARE
    ...
    v_ref_serial TEXT;
BEGIN
    -- 0. Check if sync is disabled by the calling function
    IF current_setting('app.disable_stock_sync', true) = 'true' THEN
        RETURN NEW;
    END IF;

    -- 1. Get Context from Session Variables
    ...
    -- 2. Identify Actor if not set (fallback to auth.uid)
    ...
    -- 3. Get Drug Metadata for Snapshot
    SELECT 
    ...
    -- 3.2 Look up human-readable serial ID from the referenced entity
    ...
    -- 6. Auto-Sync Aggregate Stock in drugs table
    UPDATE public.drugs 
