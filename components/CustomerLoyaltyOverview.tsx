...
export const CustomerLoyaltyOverview: React.FC<CustomerLoyaltyOverviewProps> = ({
  ...
  const loyaltyStats = useMemo(() => {
    ...
    const distribution = [
      ...
      { range: '101-500', count: activeCustomers.filter(c => (c.points || 0) > 100 && (c.points || 0) <= 500).length },
      { range: '501-1000', count: activeCustomers.filter(c => (c.points || 0) > 500 && (c.points || 0) <= 1000).length },
      { range: '1000+', count: activeCustomers.filter(c => (c.points || 0) > 1000).length },
    ];
    
    return { activeCustomers: activeCustomers.length, totalPoints, avgPoints, topTierCount, distribution };
  }, [customers]);
  ...
  const pointsTrend = useMemo(() => {
    ...
    return last30Days.map(d => ({
      name: new Date(d.date).toLocaleDateString(language === 'AR' ? 'ar-EG' : 'en-US', { month: 'short', day: 'numeric' }),
    ...
    <div className="h-full overflow-y-auto pe-2 space-y-4 animate-fade-in pb-10" dir={isRTL ? 'rtl' : 'ltr'}>
      ...
      <div className="grid grid-cols-1 md:grid-cols-2 xl:grid-cols-4 gap-3">
        ...
        <div className={`p-5 rounded-3xl bg-${color}-50 dark:bg-${color}-900/20 ${CARD_BASE} flex flex-col justify-between min-h-[120px]`}>
          ...
          <div>
            <p className={`text-xs font-bold text-${color}-800 dark:text-${color}-300 uppercase opacity-70`}>
              {t.loyalty?.activeCustomers || 'Active Customers'}
            </p>
            <p className={`text-2xl font-bold text-${color}-900 dark:text-${color}-100`}>{loyaltyStats.activeCustomers}</p>
          </div>
        </div>
      ...
      <div className="grid grid-cols-1 lg:grid-cols-2 gap-4">
        ...
        <div className={`p-5 rounded-3xl ${CARD_BASE} h-80`}>
          ...
          <ResponsiveContainer width="100%" height="90%">
            <BarChart data={loyaltyStats.distribution} margin={{ top: 0, right: 0, left: -20, bottom: 0 }}>
              <CartesianGrid strokeDasharray="3 3" vertical={false} stroke="#e2e8f0" />
              <XAxis dataKey="range" axisLine={false} tickLine={false} tick={{fill: '#94a3b8', fontSize: 11}} />
              <YAxis axisLine={false} tickLine={false} tick={{fill: '#94a3b8', fontSize: 11}} orientation={isRTL ? 'right' : 'left'} />
        ...
        <div className={`p-5 rounded-3xl ${CARD_BASE} h-80`}>
          ...
          <ResponsiveContainer width="100%" height="90%">
            <LineChart data={pointsTrend} margin={{ top: 0, right: 0, left: -20, bottom: 0 }}>
              ...
              <Line type="monotone" dataKey="points" stroke="var(--primary-500)" strokeWidth={3} dot={{ fill: 'var(--primary-500)', r: 4 }} />
            </LineChart>
          </ResponsiveContainer>
        </div>
      </div>
