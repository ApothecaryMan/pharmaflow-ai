...
import type { EmploymentRequest, Employee } from '../../../types';
...
export const HistoryTab: React.FC<HistoryTabProps> = ({ requests, workspaces = [], isRTL, t, isLoading }) => {
        ...
        <div className="relative before:absolute before:top-2 before:bottom-2 before:start-3.5 before:w-0.5 before:bg-(--border-divider) space-y-4 animate-pulse">
          {[...Array(3)].map((_, i) => (
            <div key={i} className="relative ps-8 flex items-start gap-3">
              ...
              <div className={`${PROFILE_GLASS_CARD_BASE} flex-1 h-20`}></div>
            </div>
          ))}
        </div>
      ) : workspaces.length === 0 ? (
            ...
            <div key={ws.id} className="relative ps-8 flex items-start gap-3">
              ...
              <div className={`${PROFILE_GLASS_CARD_BASE} flex-1`}>
                <div className="flex justify-between items-start flex-wrap gap-2">
                  ...
                  <span className={`inline-flex items-center gap-1 px-2 py-0.5 rounded-full text-[10px] font-bold ${ws.status === 'active' ? 'text-emerald-600 dark:text-emerald-400 bg-emerald-500/10 dark:bg-emerald-500/20' : 'text-red-600 dark:text-red-400 bg-red-500/10 dark:bg-red-500/20'}`}>
                    {ws.status === 'active' ? <CheckCircle2 className="w-3.5 h-3.5" /> : <XCircle className="w-3.5 h-3.5" />}
                    {ws.status === 'active' ? (isRTL ? 'نشط' : 'Active') : (isRTL ? 'غير نشط' : 'Inactive')}
                  </span>
                </div>
                <div className="flex items-center gap-3 mt-1.5 ms-6">
                  <span className="text-[11px] font-semibold text-emerald-600 dark:text-emerald-400 capitalize">{ws.role}</span>
                  {ws.startDate && (
