  ...
  return (
    <svg viewBox='0 0 128 128' fill='none' overflow='visible' className='w-full h-full'>
      ...
      <g className='urv-origin urv-rainbow-engine'>
        ...
        <circle cx='64' cy='64' r='56' stroke='url(#urv-rainbow-1)' strokeWidth='6' filter='url(#urv-glow)' opacity='0.5' className='urv-origin' style={{ animation: 'urv-pulse 2s ease-in-out infinite' }} />
        
        {/* Layer 2: Fast spinning inner segmented tracks */}
        <g className='urv-origin' style={{ animation: 'urv-spin-cw 8s linear infinite' }}>
          <circle cx='64' cy='64' r='53' stroke='url(#urv-rainbow-1)' strokeWidth='1.5' strokeDasharray='10 5' fill='none' />
          <circle cx='64' cy='64' r='55' stroke='url(#urv-rainbow-2)' strokeWidth='1.5' strokeDasharray='4 8' fill='none' />
        </g>
        ...
        <g className='urv-origin' style={{ animation: 'urv-spin-ccw 12s linear infinite' }}>
          <circle cx='64' cy='64' r='57' stroke='url(#urv-rainbow-2)' strokeWidth='2' strokeDasharray='20 10 5 10' fill='none' />
          <circle cx='64' cy='64' r='59' stroke='url(#urv-rainbow-1)' strokeWidth='3' strokeDasharray='2 16' fill='none' opacity='0.9' />
        </g>

        {/* Layer 4: Intense Sunburst Rays shooting outwards */}
        <g className='urv-origin' style={{ animation: 'urv-spin-cw 20s linear infinite' }}>
          <circle cx='64' cy='64' r='57' stroke='url(#urv-rainbow-1)' strokeWidth='10' strokeDasharray='2 30' fill='none' opacity='0.9' />
          <circle cx='64' cy='64' r='59' stroke='url(#urv-rainbow-2)' strokeWidth='8' strokeDasharray='4 40' fill='none' opacity='0.8' style={{ transform: 'rotate(15deg)', transformOrigin: '64px 64px' }} />
        </g>
        ...
        <g className='urv-origin' style={{ animation: 'urv-spin-ccw 22s linear infinite' }}>
          {particles.map((p, i) => {
            const animClass = p.type === 0 ? 'urv-fly-radial' : p.type === 1 ? 'urv-fly-comet' : 'urv-fly-chaos';
            const duration = p.type === 0 ? '4s' : p.type === 1 ? '2.5s' : '5s';
            
            return (
              <g key={`particle-wrapper-${i}`} transform={`translate(64, 64) rotate(${p.angle})`}>
                <g style={{ animation: `${animClass} ${duration} ease-in-out infinite`, animationDelay: `${p.delay}s` }}>
                  {/* Particle glow */}
                  <circle cx={p.r} cy="0" r={p.size * 1.5} fill={p.color} filter='url(#urv-glow)' opacity='0.8' />
                  {/* Particle solid core */}
                  <circle cx={p.r} cy="0" r={p.size * 0.5} fill='#ffffff' />
                  {/* Tiny cross highlights for larger particles */}
                  {p.size > 2 && (
                    <path d={`M ${p.r - p.size*1.5} 0 L ${p.r + p.size*1.5} 0 M ${p.r} ${-p.size*1.5} L ${p.r} ${p.size*1.5}`} stroke='#ffffff' strokeWidth='0.5' opacity='0.9' />
                  )}
                </g>
              </g>
            );
