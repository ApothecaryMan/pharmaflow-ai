  ...
  css.replace(regex, (match, p1, p2, offset) => {
    ...
    if (p1) {
      elements.push(
        <span
          key={`prop-${offset}`}
          className={isOverlay ? '' : 'text-primary-600 dark:text-primary-400 font-medium'}
        >
          {match}
        </span>
      );
        ...
        <span key={`color-${offset}`} className='relative'>
          ...
          {isOverlay && (
            <Tooltip
              ...
              content={
                <span className='font-mono text-[11px] font-bold tracking-wider'>{match}</span>
              }
            >
              <span
...
export const CustomCssEditor = React.memo<CustomCssEditorProps>(
  ({ value, onChange, placeholder, className = '' }) => {
    ...
    return (
      <div
        className={`relative w-full rounded-lg bg-gray-100 dark:bg-(--bg-input) border border-(--border-divider) focus-within:border-primary-500 focus-within:ring-1 focus-within:ring-primary-500 transition-colors ${className}`}
      >
        <div
          ref={backdropRef0}
