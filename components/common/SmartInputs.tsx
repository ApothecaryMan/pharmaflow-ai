...
export const useSmartDirection = (v?: string | null, p?: string | null) =>
  useMemo(() => getSmartDirection(v, p), [v, p]);
...
export const SmartInput: React.FC<InputHTMLAttributes<HTMLInputElement>> = ({
  value,
  className,
  placeholder,
  ...p
  ...
  <input
    ...
    dir={useSmartDirection(String(value || ''), placeholder)}
    className={`${INPUT_BASE} ${className || ''}`}
  />
);

export const SmartTextarea: React.FC<React.TextareaHTMLAttributes<HTMLTextAreaElement>> = ({
  value,
  className,
  placeholder,
  ...p
...
export const SmartDateInput: React.FC<{
  value: string;
  onChange: (v: string) => void;
  className?: string;
  required?: boolean;
  placeholder?: string;
}> = ({ value, onChange, className, required, placeholder }) => {
  ...
  return (
    <input
      ...
      required={required}
      placeholder={foc ? 'MMYY' : placeholder || 'MM/YY'}
      value={disp}
      maxLength={4}
      onChange={(e) => setDisp(e.target.value.replace(/\D/g, '').slice(0, 4))}
      onFocus={() => setFoc(true)}
      onBlur={() => {
        setFoc(false);
...
const SpecialtyInput =
  ...
  ({ value, onChange, className, ...p }: any) => (
    <input
      {...p}
      type={type}
      dir='ltr'
...
export interface SmartAutocompleteProps
  ...
  onSuggestionAccept?: (s: string) => void;
...
export const SmartAutocomplete: React.FC<SmartAutocompleteProps> = ({
  ...
  const [caps, setCaps] = useState(false),
    intRef = useRef<HTMLInputElement>(null),
    ref = extRef || intRef;
  ...
  const match = useMemo(() => {
    ...
    return (
      suggestions.find(
        (s) => (caseSensitive ? s : s.toLowerCase()).startsWith(sv) && s.length > value.length
      ) || ''
    );
  ...
  return (
    <div className='relative inline-block w-full'>
      <input
        ...
        disabled={disabled}
        placeholder={placeholder}
        className={`${INPUT_BASE} shadow-xs ${className || ''}`}
        onChange={(e) => onChange(e.target.value)}
        onKeyUp={(e) => setCaps(e.getModifierState('CapsLock'))}
        onKeyDown={(e) => {
          setCaps(e.getModifierState('CapsLock'));
          if (match && (e.key === 'ArrowRight' || e.key === 'Tab')) {
            e.preventDefault();
            onChange(match);
            onSuggestionAccept?.(match);
          } else if (match && e.key === 'Escape') {
            e.preventDefault();
          }
          p.onKeyDown?.(e);
        }}
      ...
      {ghost && (
        <div
          ...
          style={{
            paddingInlineStart: ref.current?.style.paddingInlineStart || '0.75rem',
            direction: dir,
          }}
        >
