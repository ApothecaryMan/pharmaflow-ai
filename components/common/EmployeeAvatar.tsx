  ...
  return (
    <div
      ...
      style={{
        ...
        borderStyle: 'solid',
        borderColor: (frameColor && ringStyle === 'solid' && !ringAnimated) ? frameColor : (frameColor ? 'transparent' : 'var(--border-divider)'),
        borderWidth: frameColor ? ringThickness : (image || initials ? 0 : 1), 
      }}
    >
      <div className='absolute inset-0 rounded-full overflow-hidden' style={{ borderWidth: frameColor ? 0 : 1, borderColor: 'var(--border-divider)' }}>
        {image ? (
          <img src={image} alt='Avatar' className='w-full h-full object-cover' />
        ) : initials ? (
          <div className='w-full h-full flex items-center justify-center bg-gradient-to-br from-primary-500/20 to-primary-600/30 text-primary-500 font-bold' style={{ fontSize: size * 0.35 }}>
            {initials}
          </div>
        ) : (
