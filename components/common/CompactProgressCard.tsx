    ...
    >
      <div className="flex items-center justify-between w-full">
          <div className="flex items-center gap-3 w-full">
            <span className={`material-symbols-rounded text-2xl text-${progressColor}-500 dark:text-${progressColor}-400 shrink-0`}>{icon}</span>
            
            <div className="flex-1 min-w-0">
                 <div className="flex justify-between items-center mb-1">
                    <p className="text-xs text-gray-500 dark:text-gray-400 font-medium truncate pr-2">{title}</p>
                    <div className="flex items-baseline gap-1">
                        <h4 className="text-sm font-bold text-gray-900 dark:text-white leading-none">{value}</h4>
                        <span className="text-[10px] text-gray-400 font-medium">({percentage.toFixed(0)}%)</span>
                    </div>
                 </div>
