    ...
    <div className={`lg:col-span-2 bg-white dark:bg-gray-900 p-5 rounded-2xl shadow-sm border-2 border-gray-200 dark:border-transparent ${className || ''}`}>
        ...
        <div className={`flex items-center justify-between mb-4 ${headerClassName || ''}`}>
            ...
            <div className="flex items-center gap-2">
              ...
              {onComparisonChange && dataKeys.comparison && dataKeys.comparison.length > 0 && activeChartType === 'area' && (
                  <div className="ltr:mr-2 rtl:ml-2 scale-90 origin-right rtl:origin-left">
                    <SegmentedControl
                        options={[
                        { label: '', value: 'single', icon: 'person' },
                        { label: '', value: 'group', icon: 'group', count: `+${dataKeys.comparison.length}` }
                        ]}
        ...
        <div className={`w-full ${chartClassName || 'h-[250px]'}`}>
            <ResponsiveContainer width="100%" height="100%">
                <ComposedChart data={data} margin={chartMargin || { top: 15, right: 0, left: language === 'AR' ? 30 : 0, bottom: 20 }}>
                    ...
                    {showComparison && activeChartType === 'area' && dataKeys.comparison?.map(key => (
                        <Area 
                            key={key}
                            type="monotone" 
                            dataKey={key}
