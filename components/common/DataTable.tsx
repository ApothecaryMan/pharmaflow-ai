...
export const DataTable = <T extends { id: string }>({
  ...
  const gridColumns = useMemo<GridColDef[]>(() => {
    return columns.map((col) => {
      ...
      const isLtrField = ['phone', 'contact', 'email', 'barcode', 'code', 'sku'].some((key) =>
      ...
      return {
        ...
        align:
          (col.align === 'end' ? 'right' : col.align === 'start' ? 'left' : col.align) || 'left',
        headerAlign:
          (col.align === 'end' ? 'right' : col.align === 'start' ? 'left' : col.align) || 'left',
        sortable: col.sortable ?? true,
        renderHeader: col.headerDir
          ? (params) => (
              <div
                style={{
                  direction: col.headerDir,
                  textAlign: 'start',
                  width: '100%',
                  display: 'flex',
                  alignItems: 'center',
      ...
      >
        <DataGrid
          ...
          onColumnWidthChange={(params) =>
            saveState({
              width: { ...savedState?.width, [params.colDef.field]: params.colDef.width },
            })
          }
