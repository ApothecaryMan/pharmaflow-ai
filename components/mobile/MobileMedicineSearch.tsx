...
export const MobileMedicineSearch: React.FC<MobileMedicineSearchProps> = ({
    ...
    return inventory
      .filter((drug) => {
        ...
        if (!priceRange && mode === 'generic') {
          ...
          return (isArray ? drug.genericName : [drug.genericName as unknown as string]).some(
            (gn) => gn && regex.test(gn)
          );
        }
 ...
 `}
        ...
        <div
          className={`grid transition-all duration-300 ease-out ${searchTerm ? 'grid-rows-[1fr] opacity-100' : 'grid-rows-[0fr] opacity-0'}`}
        >
          <div className='overflow-hidden'>
            <div
              className='pt-3 px-2 shrink-0 flex items-center justify-between'
...
const DrugActionPill: React.FC<{
  ...
  const currentQtyInCart = cart
    ...
    .reduce(
      (sum, c) => sum + (c.isUnit ? convertToPacks(c.quantity, drug.unitsPerPack) : c.quantity),
      0
    );
  ...
  return (
    <div className='shrink-0 relative'>
      <div
        className={`absolute -top-2.5 end-1 z-20 px-1.5 py-0.5 rounded-full text-[9px] font-black shadow-lg border-2 border-white dark:border-(--bg-secondary) animate-in zoom-in duration-300 ${
          currentQtyInCart > 0
      ...
      >
        {currentQtyInCart > 0
          ...
          : drug.stock <= 0
            ? '0'
            : convertToPacks(drug.stock, drug.unitsPerPack).toLocaleString('en-US', {
                maximumFractionDigits: 1,
              })}
      </div>

      <div
        className={`flex items-center h-8 rounded-full border bg-transparent transition-all duration-300 ${isExpanded ? 'border-primary-500/50 ring-1 ring-primary-500/10' : 'border-gray-200 dark:border-white/20'}`}
      >
        ...
        <button
          onClick={(e) => {
            e.stopPropagation();
            if (!isOutOfStock) onAddToCart(drug);
          }}
        ...
        <div className='flex flex-col w-full px-4 text-left'>
              ...
              >
                {highlightMatch(
                  ...
                  searchTerm.startsWith('@') ||
                    !new RegExp(parseSearchTerm(searchTerm).regex.source, 'i').test(displayName)
                )}
              </p>
            </div>
            <DrugActionPill
          ...
          >
            <div className='overflow-hidden'>
              <div className='flex items-center justify-between pt-3 border-t border-gray-100 dark:border-gray-800/50 mt-1'>
                <div className='flex flex-col'>
                  <span className='text-[10px] text-gray-400 uppercase font-black tracking-widest'>
                    {language === 'AR' ? 'تحكم في الكمية' : 'Quantity'}
                  </span>
                ...
                <CartItemQuantityControl
                  item={
                    cart.find((c) => c.id === drug.id) ||
                    ({ ...drug, quantity: 0, isUnit: false } as CartItem)
                  }
                  ...
                  allBatches={inventory.filter(
                    (b) => b.name === drug.name && b.dosageForm === drug.dosageForm
                  )}
                  isMobile={true}
                  t={t}
              ...
              {drug.description && (
                <p className='pt-2 mt-2 text-[10px] text-gray-400 italic leading-relaxed'>
                  {drug.description}
                </p>
              )}
            </div>
