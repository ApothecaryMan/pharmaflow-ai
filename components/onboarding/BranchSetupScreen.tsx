...
export const BranchSetupScreen: React.FC<BranchSetupScreenProps> = ({
  ...
  // Attempt to capture browser geolocation on mount
  useEffect(() => {
    if (typeof window !== 'undefined' && navigator.geolocation) {
      navigator.geolocation.getCurrentPosition(
        ...
        (error) => {
          console.log('[BranchSetupScreen] Geolocation not acquired:', error);
        },
  ...
  return (
    <div
      className={`min-h-screen flex items-center justify-center bg-zinc-50 dark:bg-black p-4 ${isRTL ? 'font-arabic' : ''}`}
      dir={isRTL ? 'rtl' : 'ltr'}
    >
      {/* Custom Animations */}
      <style
