...
import { DrugSearchInput, useSmartDirection } from '../common/SmartInputs';
...
export const PurchasesTest: React.FC<PurchasesProps> = ({
  inventory,
  suppliers,
  purchases,
  purchaseReturns,
  ...
  const renderHistoryCell = (p: Purchase, columnId: string) => {
    ...
    switch (columnId) {
      ...
      case 'discount':
        return (
          <span className='text-xs font-bold text-green-600'>{discountPercent.toFixed(1)}%</span>
        );
      case 'total':
        return (
                      ...
                      {filteredDrugs.length > 0 ? (
                        filteredDrugs.slice(0, 10).map((drug, index) => (
                          ...
                          >
                            ...
                            <span
                              className={`font-medium text-gray-900 dark:text-white flex-1 truncate ${typeof window !== 'undefined' && localStorage.getItem('pharma_textTransform') === 'uppercase' ? 'uppercase' : ''}`}
                            >
                      ...
                      <div className='flex items-center gap-2 px-3 py-1.5 rounded-xl bg-gray-50 dark:bg-gray-800/50 border border-gray-100 dark:border-gray-700/50'>
                        ...
                        <div className='flex flex-col'>
                          ...
                          <span className='text-xs font-bold text-gray-700 dark:text-gray-200 font-mono leading-none'>
                            {formatStock(invItem.stock, invItem.unitsPerPack)}
                          </span>
                        </div>
                      </div>
                  ...
                  {columnOrder
                    ...
                    .map((columnId) => (
                      <th
                        ...
                        {...headerTriggerProps}
                        style={{
                          width: historyColumnWidths[columnId]
                            ? `${historyColumnWidths[columnId]}px`
                            : 'auto',
                        }}
                      >
                        <div className='flex items-center justify-between h-full w-full'>
                          <span className='truncate flex-1'>
                            {historyColumnsDef[columnId as keyof typeof historyColumnsDef].label}
                          </span>
                ...
                {purchases.length === 0 && (
                  <tr>
                    <td colSpan={10} className='p-12 text-center text-gray-400'>
                      {t.noHistory || 'No purchase history found'}
                    </td>
                  </tr>
                )}
              </tbody>
