  ...
  const SectionCard: React.FC<{ title: string; icon: string; children: React.ReactNode }> = ({ 
    ...
    <div className="bg-white dark:bg-gray-900 rounded-2xl p-6 shadow-sm border border-gray-200 dark:border-gray-800">
      <h3 className="text-lg font-semibold text-gray-800 dark:text-white flex items-center gap-2 mb-4">
        <span className="material-symbols-rounded text-[24px]" style={{ color: `var(--color-${color}-500, #3b82f6)` }}>{icon}</span>
        {title}
      </h3>
  ...
  return (
    <div className="space-y-6">
      ...
      <SectionCard title={language === 'AR' ? 'اختبار الأحجام' : 'Size Tests'} icon="aspect_ratio">
        <TestButton label="SM" config={{ size: 'sm', title: 'Small Modal' }} icon="photo_size_select_small" />
        ...
        <TestButton label="XL" config={{ size: 'xl', title: 'Extra Large Modal' }} icon="crop_free" />
        ...
        <TestButton label="6XL" config={{ size: '6xl', title: '6XL Modal' }} icon="fullscreen_exit" />
        <TestButton label="Full" config={{ size: 'full', title: 'Full Width Modal' }} icon="aspect_ratio" variant="primary" />
      </SectionCard>
      ...
      <Modal
        ...
        tabs={
          modalConfig.testType === 'multipage' ? [
            { label: language === 'AR' ? 'الصفحة 1' : 'Page 1', value: 'page1', icon: 'description' },
            { label: language === 'AR' ? 'الصفحة 2' : 'Page 2', value: 'page2', icon: 'settings' }
          ] : undefined
        }
        activeTab={modalConfig.testType === 'multipage' ? activePage : undefined}
        onTabChange={modalConfig.testType === 'multipage' ? (val) => setActivePage(val as string) : undefined}
             ...
             <div className="p-6 text-center border-2 border-dashed border-gray-200 dark:border-gray-700 rounded-xl">
                {activePage === 'page1' ? (
                  <div className="animate-fade-in">
                    <span className="material-symbols-rounded text-6xl text-blue-500 mb-4">description</span>
                  ...
                  <div className="animate-fade-in">
                    <span className="material-symbols-rounded text-6xl text-purple-500 mb-4">settings</span>
                    <h3 className="text-xl font-bold text-gray-800 dark:text-white mb-2">
