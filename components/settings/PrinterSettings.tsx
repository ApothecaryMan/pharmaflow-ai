  ...
  const StatusBadge = () => {
    const statusConfig = {
      connected: {
        ...
        text: t.printerSettings?.status?.connected || 'Connected',
        icon: 'check_circle',
      },
      disconnected: {
        color: 'bg-gray-400',
        text: t.printerSettings?.status?.disconnected || 'Disconnected',
        icon: 'cancel',
      },
      connecting: {
        color: 'bg-amber-500',
        text: t.printerSettings?.status?.connecting || 'Connecting...',
        icon: 'sync',
      },
      not_installed: {
        color: 'bg-red-500',
        text: t.printerSettings?.status?.notInstalled || 'Not Installed',
        icon: 'error',
      },
    };
  ...
  return (
    ...
    >
      <div className='space-y-6'>
        ...
        {isTauri() && (
          <div
            className={`p-4 bg-${color}-50 dark:bg-${color}-900/20 border border-${color}-100 dark:border-${color}-800/30 rounded-2xl flex items-center gap-4`}
          >
            <div
              className={`w-10 h-10 rounded-full bg-${color}-100 dark:bg-${color}-800/50 flex items-center justify-center flex-shrink-0`}
            >
            ...
            <button
              onClick={() => {
                ...
                window.dispatchEvent(
                  new CustomEvent('navigate-to-view', { detail: 'desktop-settings' })
                );
              }}
          ...
          <div className='space-y-3 pt-4 border-t border-(--border-divider)'>
            ...
            <p className='text-xs text-gray-500 dark:text-gray-400'>
              {settings.silentMode === 'on' &&
                (pt.silentHints?.on || 'Always print silently (requires QZ Tray)')}
              {settings.silentMode === 'off' && (pt.silentHints?.off || 'Always show print dialog')}
              {settings.silentMode === 'fallback' &&
                (pt.silentHints?.fallback || 'Silent if QZ available, dialog otherwise')}
            </p>
          </div>
        )}
