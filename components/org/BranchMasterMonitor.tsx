      ...
      <div className='flex items-center gap-6'>
        {isLoading ? (
          ...
          <>
            ...
            <div className='flex items-center gap-2'>
              <div
                className={`w-2 h-2 rounded-full ${isActive ? 'bg-emerald-500 animate-pulse' : 'bg-red-400'}`}
              />
              <span
                className={`text-xs font-bold uppercase ${isActive ? 'text-emerald-600 dark:text-emerald-400' : 'text-red-500'}`}
              >
                {isActive
                  ? language === 'ar'
                    ? 'نشط'
...
export const BranchMasterMonitor: React.FC<BranchMasterMonitorProps> = ({
  branches,
  color = 'primary',
  language,
  isLoading,
    ...
    <div className={`p-5 rounded-3xl ${CARD_BASE} flex flex-col group h-full`}>
      ...
      <div className='flex-1 overflow-y-auto scrollbar-hide space-y-0 divide-y divide-zinc-100 dark:divide-zinc-800'>
        {isLoading ? (
          ...
          branches.map((branch) => (
            <BranchRow key={branch.id} branch={branch} language={language} color={color} />
          ))
        )}
      </div>
    </div>
