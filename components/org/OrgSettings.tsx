...
export const OrgSettings: React.FC<OrgSettingsProps> = ({ language, color }) => {
  ...
  return (
    <div className='p-6 max-w-4xl mx-auto animate-fade-in space-y-8'>
      ...
      <div>
        <h1
          className={`text-2xl font-bold tracking-tight text-zinc-900 dark:text-zinc-100 ${language === 'AR' ? 'font-arabic' : ''}`}
        >
          {language === 'AR' ? 'إعدادات المنظمة' : 'Organization Settings'}
        </h1>
        <p className='text-sm text-zinc-500 dark:text-zinc-400 mt-1'>
          {language === 'AR'
            ? 'إدارة هوية المنظمة والاشتراكات والوصول'
            : 'Manage your organization identity, subscriptions, and access'}
        </p>
      </div>
        ...
        <div className='md:col-span-2 space-y-6'>
          ...
          <section className='bg-white dark:bg-zinc-900 border border-zinc-200 dark:border-zinc-800 rounded-2xl overflow-hidden shadow-sm'>
            ...
            <div className='divide-y divide-zinc-100 dark:divide-zinc-800'>
              {members.map((member) => {
                const employee =
                  allEmployees.find((e) => e.id === member.id) ||
                    ...
                    <div className='flex items-center gap-3'>
                      ...
                      <div>
                        <h4 className='text-sm font-bold text-zinc-900 dark:text-zinc-100'>
                          {employee?.name ||
                            (language === 'AR' ? 'عضو غير معروف' : 'Unknown Member')}
                        </h4>
                ...
                <div className='flex items-center gap-4 p-4 rounded-xl bg-primary-50/30 dark:bg-primary-500/5 border border-primary-100/50 dark:border-primary-500/10'>
                  ...
                  >
                    <span className='material-symbols-rounded text-2xl'>
                      {subscription.status === 'trial' ? 'hourglass_top' : 'verified'}
                    </span>
                  </div>
                  <div className='flex-1'>
                    ...
                    <p className='text-xs text-zinc-500 dark:text-zinc-400'>
                      {subscription.status === 'trial'
                        ? language === 'AR'
                          ? `تنتهي في ${new Date(subscription.trialEndsAt!).toLocaleDateString('ar-EG')}`
                          : `Ends on ${new Date(subscription.trialEndsAt!).toLocaleDateString()}`
                        : language === 'AR'
                          ? 'تاريخ التجديد القادم غير متوفر'
            ...
            <div className='p-6 relative'>
              {logs.length > 0 ? (
                          ...
                          <div className='bg-zinc-50 dark:bg-zinc-800/40 border border-zinc-100 dark:border-zinc-800 rounded-xl p-3'>
                            ...
                            <div className='mt-2 flex items-center gap-2 pt-2 border-t border-zinc-100 dark:border-zinc-800/50'>
                              ...
                              <span className='text-[10px] text-zinc-400'>
                                {branch?.name ||
                                  (language === 'AR' ? 'فرع غير معروف' : 'Unknown Branch')}
                              </span>
                            </div>
                          </div>
                        </div>
                ...
                <div className='text-center py-8'>
                  <span className='material-symbols-rounded text-4xl text-zinc-200 dark:text-zinc-800 mb-2'>
                    event_busy
                  </span>
                  <p className='text-sm text-zinc-500'>
                    {language === 'AR' ? 'لا يوجد نشاط مسجل حالياً' : 'No recent activity found'}
                  </p>
                </div>
              )}
            </div>
            ...
            <div className='space-y-3'>
              <div className='flex justify-between items-center text-sm'>
                <span className='text-zinc-500'>
                  {language === 'AR' ? 'عدد الفروع' : 'Branches'}
                </span>
              ...
              <div className='flex justify-between items-center text-sm'>
                <span className='text-zinc-500'>
                  {language === 'AR' ? 'نوع الاشتراك' : 'Plan Type'}
                </span>
