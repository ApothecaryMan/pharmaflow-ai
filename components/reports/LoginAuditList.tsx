...
import { authService, LoginAuditEntry } from '../../services/auth/authService';
    ...
    const getActionInfo = (action: string) => {
        switch(action) {
            case 'login': return {
                ...
                icon: 'login',
                colors: 'border-emerald-200 dark:border-emerald-900/50 text-emerald-700 dark:text-emerald-400'
            };
            case 'logout': return {
    ...
    const translateDetails = (details: string | undefined): React.ReactNode => {
        ...
        if (accountMatch) {
            ...
            return (
                <span className="text-gray-500 dark:text-gray-400">
                    {translated[0]}
                    <span className="text-gray-900 dark:text-white font-semibold mx-1">{accountMatch[1]}</span>
                    {translated[1]}
                </span>
            );
        }
        ...
        if (details === 'Employee signed out') {
            return <span className="text-gray-500 dark:text-gray-400">{t.loginAudit.detailPatterns.employeeSignedOut}</span>;
        }

        // Pattern 4: Account Logout
        if (details === 'Account Logout') {
            return <span className="text-gray-500 dark:text-gray-400">{t.loginAudit.detailPatterns.accountLogout}</span>;
        }

        // Pattern 5: System session started
        if (details === 'System session started') {
            return <span className="text-gray-500 dark:text-gray-400">{t.loginAudit.detailPatterns.systemSessionStarted}</span>;
        }

        // Pattern 6: Employee session started
        if (details === 'Employee session started') {
            return <span className="text-gray-500 dark:text-gray-400">{t.loginAudit.detailPatterns.employeeSessionStarted}</span>;
        }
    ...
    const columns = useMemo<ColumnDef<LoginAuditEntry>[]>(() => [
        {
            ...
            cell: info => <span className="font-semibold text-gray-800 dark:text-gray-200">{String(info.getValue()).substring(0, 8)}</span>,
                ...
                return (
                    <div className="flex items-center gap-2">
                        <div className={`w-6 h-6 flex items-center justify-center shrink-0 ${
                            ...
                            {isAdmin ? (
                                <>
                                    <img src="/logo_icon_black.svg" className="w-6 h-6 dark:hidden object-contain" alt="System" />
                                    <img src="/logo_icon_white.svg" className="w-6 h-6 hidden dark:block object-contain" alt="System" />
                                </>
                            ) : hasImage ? (
        ...
        },
        {
            ...
            cell: info => {
                ...
                return (
                    <span className={`inline-flex items-center gap-1.5 px-1.5 py-0.5 rounded-lg border bg-transparent text-xs font-bold uppercase tracking-wider ${colors}`}>
        ...
        },
        {
            ...
            cell: info => <div className="text-[11px]">{translateDetails(info.getValue() as string | undefined)}</div>,
