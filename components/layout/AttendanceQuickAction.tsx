...
export const AttendanceQuickAction: React.FC<AttendanceQuickActionProps> = ({ language }) => {
  ...
  const handleBiometric = async () => {
    ...
    try {
      ...
      const currentStatus = await attendanceService.getEmployeeStatus(
        selectedEmployee.id,
        activeBranchId
      );
      ...
      setStep('error');
      setErrorMessage(
        err.message === 'INVALID_TERMINAL_TOKEN'
  ...
  const handlePinSubmit = async (e?: React.FormEvent) => {
    ...
    try {
      const isValid = await attendanceService.verifyEmployeePin(
        pin,
