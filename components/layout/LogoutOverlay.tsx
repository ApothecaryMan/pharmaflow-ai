...
export const LogoutOverlay: React.FC<LogoutOverlayProps> = ({
  ...
  const stats = React.useMemo(() => {
    ...
    sales.forEach((s) => {
      s.items?.forEach((i: any) => {
        counts[i.drugId] = (counts[i.drugId] || 0) + i.quantity;
      });
      if (s.total > maxInvoice) maxInvoice = s.total;
    });
    const topId = Object.entries(counts).sort((a, b) => b[1] - a[1])[0]?.[0];
    const bestSeller =
      inventory.find((i) => i.id === topId)?.name || (language === 'AR' ? 'لا يوجد' : 'None');
  ...
  return (
    <div
      className='h-screen bg-zinc-50 dark:bg-zinc-950 flex items-center justify-center select-none overflow-hidden relative'
      ...
      <div className='absolute inset-0 overflow-hidden pointer-events-none'>
        {[...Array(8)].map((_, i) => (
          <motion.div
            ...
            animate={{ y: '120vh', opacity: [0, 0.2, 0] }}
            transition={{ duration: 5 + i, repeat: Infinity, delay: i * 0.8, ease: 'linear' }}
            className='absolute w-px h-32 bg-gradient-to-b from-transparent via-zinc-400/20 to-transparent'
            style={{ left: `${15 + i * 12}%` }}
          />
        ))}
      </div>
      ...
      >
        ...
        <div className='relative w-72 h-72 mb-14 flex items-center justify-center'>
          ...
          <motion.div
            ...
            className='absolute inset-8 border border-zinc-400/40 dark:border-zinc-700/60 rounded-full border-dashed'
          />
          <motion.div
            animate={{ rotate: -360 }}
            transition={{ duration: 15, repeat: Infinity, ease: 'linear' }}
            className='absolute inset-16 border border-zinc-300/30 dark:border-zinc-800/20 rounded-full'
          />
          ...
          <div className='relative z-10 w-32 h-32 rounded-[2.5rem] bg-white/[0.03] dark:bg-zinc-900/20 backdrop-blur-3xl border border-zinc-200/30 dark:border-zinc-700/30 flex items-center justify-center overflow-hidden'>
            <img
              ...
              alt='Logo'
              className='w-20 h-20 opacity-95 relative z-10'
            />
            <motion.div
              animate={{ top: ['-10%', '110%'] }}
              transition={{ duration: 3, repeat: Infinity }}
              className='absolute inset-x-0 h-px bg-zinc-400 dark:bg-zinc-500 opacity-20'
            />
          </div>
        ...
        <div className='text-center space-y-6'>
          <div className='space-y-2'>
            <div className='flex items-center justify-center gap-4'>
              ...
              <h2 className='text-7xl font-bold text-zinc-900 dark:text-zinc-50 tracking-tighter uppercase'>
                ZINC
              </h2>
              <div className='h-px w-10 bg-zinc-200 dark:bg-zinc-800' />
            </div>
          ...
          <div className='flex flex-col items-center gap-5 mt-4'>
            ...
            <div className='flex items-center gap-5 opacity-50 text-[7px] font-bold uppercase tracking-[0.4em] text-zinc-500'>
              <div className='flex items-center gap-2'>
                <span>{t.best}</span>
                <span className='text-[8px] font-mono text-zinc-900 dark:text-zinc-100'>
                  {stats.bestSeller}
                </span>
              </div>
