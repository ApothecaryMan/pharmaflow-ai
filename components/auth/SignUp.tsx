  ...
  return (
    <>
      ...
      <div className='text-center mb-10'>
        {!state.success && registrationType === 'org' && (
          <p className='text-zinc-400 text-sm'>
            {t.signUpSubtitle || 'Join us to access the system.'}
          </p>
        )}
      </div>
      ...
      {!registrationType ? (
        <div className='space-y-4 animate-in fade-in slide-in-from-bottom-4 duration-500'>
          ...
          >
            <svg
              className='w-10 h-10 text-zinc-500 group-hover:text-white transition-colors duration-300 shrink-0'
              fill='currentColor'
              viewBox='0 0 24 24'
            >
            ...
            <div className='flex-1'>
              <h4 className='text-sm font-medium text-zinc-200 group-hover:text-white transition-colors duration-300 mb-1'>
                {language === 'AR' ? 'موظف مستقل' : 'Independent Employee'}
              </h4>
              <p className='text-xs text-zinc-500 group-hover:text-zinc-400 transition-colors duration-300 leading-relaxed'>
                {language === 'AR'
                  ? 'سجل مستخدم للحصول على طلبات عمل من الصيدليات'
                  : 'Register with a username to receive employment requests from pharmacies'}
              </p>
            </div>
          </button>
          <button
          ...
          >
            ...
            <div className='flex-1'>
              <h4 className='text-sm font-medium text-zinc-200 group-hover:text-white transition-colors duration-300 mb-1'>
                {language === 'AR' ? 'مدير صيدلية' : 'Pharmacy Owner/Manager'}
              </h4>
              <p className='text-xs text-zinc-500 group-hover:text-zinc-400 transition-colors duration-300 leading-relaxed'>
                {language === 'AR'
                  ? 'سجل كمنظمة لإدارة الفروع والموظفين'
                  : 'Register as an organization to manage branches and employees'}
              </p>
            </div>
          </button>
            ...
            <fieldset disabled={state.isLoading} className='space-y-4'>
              <div className='flex flex-col gap-1.5'>
                ...
                {state.validationErrors.email && (
                  <p className='text-xs text-red-500 mt-1 pl-1'>{state.validationErrors.email}</p>
                )}
              </div>
              ...
              <div className='flex flex-col gap-1.5'>
                <div className='relative'>
                  <input
                    ...
                    onChange={(e) =>
                      setState((prev) => ({
                        ...
                        validationErrors: { ...prev.validationErrors, password: '' },
                      }))
                    }
                  ...
                  <button
                    ...
                    className='absolute right-3 top-1/2 -translate-y-1/2 text-zinc-500 hover:text-zinc-300 focus:text-white cursor-pointer'
                  >
                  ...
                  <input
                    ...
                    onChange={(e) =>
                      setState((prev) => ({
                        ...
                        validationErrors: { ...prev.validationErrors, confirmPassword: '' },
                      }))
                    }
