...
export const SignUp: React.FC<SignUpProps> = ({ onViewChange, onLoginSuccess, language = 'EN' }) => {
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    const { success, message } = await authService.signUp(state.username, state.email, state.password);
            ...
            <fieldset disabled={state.isLoading} className='space-y-4'>
              <div className='space-y-2'>
                ...
                <input
                  ...
                  onChange={(e) => setState((prev) => ({ ...prev, username: e.target.value, validationErrors: { ...prev.validationErrors, username: '' } }))}
                  className={`w-full bg-zinc-900 border ${state.validationErrors.username ? 'border-red-500/50 focus:border-red-500' : 'border-zinc-800 focus:border-green-500/50'} rounded-lg px-4 py-3 text-sm text-white placeholder:text-zinc-600 outline-hidden transition-all duration-200 focus:ring-2 focus:ring-green-500/10 text-left placeholder:text-left cursor-text focus:bg-zinc-800/50`}
                  dir='ltr'
                />
