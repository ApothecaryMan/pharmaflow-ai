  ...
  <div className='flex items-center justify-between p-2 hover:bg-gray-50 dark:hover:bg-gray-800/50 rounded-xl transition-colors group'>
    ...
    <div className='flex items-center gap-4'>
      <div className='text-end'>
        ...
        {badge && (
          <p className={`text-[10px] font-bold uppercase ${badgeColor || 'text-primary-500'}`}>
            {badge}
          </p>
        )}
      </div>
...
export const RealTimeSalesMonitor: React.FC<RealTimeSalesMonitorProps> = ({
  ...
  const {
    ...
    activeCountersStats,
    revenueTooltip: revT,
    transactionsTooltip: transT,
    itemsSoldTooltip: itemsT,
    activeCountersTooltip: countersT,
  ...
  useEffect(() => {
    ...
    if (activeFilter === 'ALL') {
      ...
      const filtered = todaysSales.filter((s) =>
        activeFilter === 'VIP' ? isVIP(s) : highValueAnalysis.highValueIds.has(s.id)
      );
  ...
  const monitorViews = useMemo(() => {
    const chartView = (data: any, dataKey: string, stroke: string) => (
      <div
        className={`p-6 rounded-3xl ${CARD_BASE} h-[450px] w-full min-w-0 flex flex-col relative`}
      >
        <ResponsiveContainer width='100%' height='100%' debounce={50}>
          <AreaChart data={data}>
            ...
            <YAxis
              ...
              tickLine={false}
              tickFormatter={(val) =>
                formatCurrency(val, 'EGP', language === 'AR' ? 'ar-eg' : 'en-us', 0)
              }
              tick={{ fontSize: 12, fill: '#94a3b8' }}
            />
            <Tooltip
        ...
        children: (
          <div className='grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-4'>
            {[1, 2, 3, 4, 5].map((id) => (
              <div
                key={id}
                className={`p-6 rounded-2xl border ${id <= 3 ? 'border-green-100 bg-green-50/50 dark:bg-green-900/10' : 'border-gray-100 bg-gray-50 dark:bg-gray-800'}`}
              >
                  ...
                  <div className='flex items-center gap-3'>
                    ...
                    <div>
                      <h4 className='font-bold'>
                        {t.realTimeSales?.counter || 'Counter'} {id}
                      </h4>
                      <p className='text-xs text-gray-500'>Main Hall</p>
                    </div>
                  </div>
                  <span
                    className={`px-2 py-1 rounded-sm text-[10px] font-bold ${id <= 3 ? 'bg-green-100 text-green-700' : 'bg-gray-200 text-gray-600'}`}
                  >
                    {id <= 3 ? 'ONLINE' : 'OFFLINE'}
                  </span>
                </div>
                <div className='flex justify-between items-end'>
                  ...
                  <div className='text-right'>
                    ...
                    <p className='font-bold'>
                      {id <= 3 ? formatCurrency(Math.random() * 1000) : '-'}
                    </p>
                  </div>
                </div>
              </div>
  ...
  const categoryPieData = useMemo(() => {
    ...
    todaysSales.forEach((sale) => {
      sale.items.forEach((item) => {
        ...
        else if (cat.match(/cream|lotion|skin|hair|cosmetic|beauty|shampoo/))
          groups.cosmetic = money.add(groups.cosmetic, itemTotal);
  ...
  return (
    ...
    >
      ...
      <div className='grid grid-cols-1 md:grid-cols-2 xl:grid-cols-4 gap-3'>
        <StatCard
          title={t.realTimeSales?.todayRevenue}
          value={revenue}
          icon='payments'
          iconColor='primary'
          trend={revenueChange > 0 ? 'up' : 'neutral'}
        ...
        <StatCard
          ...
          icon='receipt_long'
          iconColor='blue'
          subValue={`${formatCurrency(highValueAnalysis.avgTransactionValue)} ${t.realTimeSales?.avg || 'avg'}`}
        ...
        <StatCard
          ...
          icon='inventory_2'
          iconColor='purple'
          subValue={itemsAnalysis.topCategory}
        ...
        <StatCard
          ...
          value={activeCountersStats.activeCounters}
          icon='point_of_sale'
          iconColor='amber'
          suffix={`/${activeCountersStats.totalCounters}`}
          subValue={`${activeCountersStats.onHoldCount} on hold`}
      ...
      <div className='grid grid-cols-1 lg:grid-cols-5 gap-4'>
        <div className='lg:col-span-3 flex flex-col gap-4'>
              ...
              <div className='flex items-center gap-2'>
                <SegmentedControl
                  ...
                  options={[
                    ...
                    { label: 'VIP', value: 'VIP' },
                    {
                      label: t.realTimeSales?.filterHighValue || 'High Value',
                      value: 'HIGH_VALUE',
                    },
                  ]}
                />
          ...
          <div className='grid grid-cols-1 sm:grid-cols-2 md:grid-cols-6 gap-3'>
            <div
              className={`md:col-span-2 p-4 rounded-2xl ${CARD_BASE} flex flex-col justify-center`}
            >
              <p className='text-[10px] font-bold text-gray-500 uppercase mb-0.5'>
                {t.realTimeSales?.salesRate}
              </p>
            ...
            >
              ...
              <div className='text-xl font-bold flex items-center'>
                {isLoading ? (
                  ...
                  <>
                    <AnimatedCounter
                      value={hourlyAnalysis.hourlyNewCustomerRate}
                      fractionDigits={1}
                    />
                    <span className='text-[10px] text-gray-400 font-normal ms-1'>/hr</span>
                  </>
            ...
            <div className='md:col-span-3'>
              <FlexDataCard
                ...
                items={[
                  {
                    label: t.realTimeSales?.walkIn,
                    value: `${orderTypeAnalysis.walkInRate.toFixed(0)}%`,
                    percentage: orderTypeAnalysis.walkInRate,
                    color: 'indigo',
                  },
                  {
                    label: t.realTimeSales?.delivery,
                    value: `${orderTypeAnalysis.deliveryRate.toFixed(0)}%`,
                    percentage: orderTypeAnalysis.deliveryRate,
                    color: 'orange',
                  },
                ]}
              />
            ...
            <div className='md:col-span-3'>
              <FlexDataCard
                ...
                items={[
                  {
                    label: t.realTimeSales?.reg,
                    value: `${customerAnalysis.registeredRate.toFixed(0)}%`,
                    percentage: customerAnalysis.registeredRate,
                    color: 'primary',
                  },
                  {
                    label: t.realTimeSales?.anon,
                    value: `${customerAnalysis.anonymousRate.toFixed(0)}%`,
                    percentage: customerAnalysis.anonymousRate,
                    color: 'gray',
                  },
                ]}
              />
