      ...
      calculations: [
        {
          ...
          math: (
            <span className='font-bold text-amber-300'>
              {formatCurrency(debtsValue, undefined, language)}
            </span>
          ),
        },
      ],
      details: [
        {
          ...
          value: filteredData.purchases.filter(
            (p) => p.paymentMethod === 'credit' && p.status !== 'rejected'
          ).length,
          isCurrency: false,
        },
      ],
        ...
        <div className={`p-5 rounded-3xl ${CARD_BASE} h-80 flex flex-col group lg:col-span-4`}>
          <div className='flex justify-between items-center mb-3'>
            ...
            <div className='flex items-center gap-2 shrink-0'>
              <ExpandButton
                onClick={() => setExpandedView('topSelling')}
                title={t.expand?.details}
              />
              <SegmentedControl
                options={[
                  {
                ...
                >
                  <div className='flex items-center gap-4 flex-1 min-w-0'>
                    ...
                    <span
                      data-no-convert='true'
                      dir='ltr'
                      className='text-xs font-bold text-gray-900 dark:text-gray-100 truncate shrink-0'
                    >
                      #{sale.serialId || sale.id}
                    </span>
