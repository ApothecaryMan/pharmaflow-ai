...
export const LiveWidget = () => {
  ...
  return (
    ...
    >
      <div className='w-full max-w-[500px] h-full flex items-center justify-between px-2 bg-white dark:bg-gray-900 border border-gray-200 dark:border-gray-800 rounded-full cursor-grab active:cursor-grabbing'>
        <div className='flex items-center gap-2 min-w-0 flex-1 px-1'>
          ...
          <div className='flex flex-col items-start leading-tight pointer-events-none shrink-0'>
            ...
            <span
              className='text-[15px] font-black font-mono tracking-tighter text-gray-900 dark:text-white'
              dir='ltr'
            >
              {isPrivacyMode ? (
                '••••••'
              ) : (
                <AnimatedCounter value={data.revenue} fractionDigits={0} mode='countup' />
              )}
            </span>
          </div>
          ...
          <div className='flex flex-col items-start leading-tight pointer-events-none shrink-0'>
            ...
            <span className='text-[15px] font-black font-mono tracking-tighter text-gray-900 dark:text-white'>
              {isPrivacyMode ? (
                ...
                <AnimatedCounter value={data.transactions} fractionDigits={0} mode='countup' />
              )}
            </span>
          </div>
          ...
          <div className='flex items-center gap-2 pointer-events-none min-w-0 flex-1'>
            ...
            <div className='flex items-center gap-1.5 min-w-0 w-full'>
              <span
                className='text-[11px] font-bold font-mono tracking-tighter text-gray-900 dark:text-white shrink-0'
                dir='ltr'
              >
                {isPrivacyMode
                  ? '••/••'
                  : `${Math.round((data.revenue / (data.dailyTarget || 1)) * 100)}%`}
              </span>
              <div className='flex-1 h-1.5 rounded-full overflow-hidden bg-gray-200 dark:bg-gray-700 min-w-[20px]'>
                <div
                  ...
                  style={{
                    ...
                    backgroundColor:
                      data.revenue >= data.dailyTarget
                        ? '#22c55e'
                        : data.revenue >= data.dailyTarget * 0.5
                          ? '#f59e0b'
                          : '#ef4444',
                  }}
                />
