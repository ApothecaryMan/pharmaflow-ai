...
import { Sale, Customer, Drug } from '../../types';
...
export const useRealTimeSalesAnalytics = ({ sales, customers, products, language }: RealTimeSalesAnalyticsProps) => {
  ...
  const hourlyAnalysis = useMemo(() => {
    ...
    const newCustomersToday = todaysSales.filter(s => s.customerName && !customers.find(c => c.name === s.customerName))?.length || 0;
  ...
  const customerAnalysis = useMemo(() => {
    ...
    todaysSales.forEach(s => {
      ...
      if (s.customerCode) {
        customer = customers.find(c => c.code === s.customerCode || c.serialId?.toString() === s.customerCode);
    ...
    const vipRate = coreMetrics.transactions > 0 ? (vipTransactions / coreMetrics.transactions) * 100 : 0;
    const registeredRate = coreMetrics.transactions > 0 ? (registeredTransactions / coreMetrics.transactions) * 100 : 0;
  ...
  const paymentAnalysis = useMemo(() => {
    ...
    const cashRate = coreMetrics.transactions > 0 ? (cashCount / coreMetrics.transactions) * 100 : 0;
  ...
  const highValueAnalysis = useMemo(() => {
    ...
    const avgTransactionValue = coreMetrics.transactions > 0 
      ? coreMetrics.revenue / coreMetrics.transactions 
      : 0;
    
    // Calculate high value threshold (top 5% minimum)
    const highValueThreshold = sortedToday[topCount - 1]?.netTotal ?? sortedToday[topCount - 1]?.total ?? 0;
  ...
  const itemsAnalysis = useMemo(() => {
    const itemsPerTransaction = coreMetrics.transactions > 0 
      ? coreMetrics.itemsSold / coreMetrics.transactions 
      : 0;
    ...
    todaysSales.forEach(s => {
      ...
      if (s.itemReturnedQuantities) {
        returnedItems += Object.values(s.itemReturnedQuantities).reduce((sum, qty) => sum + (qty as number), 0);
      }
    });
  ...
  const orderTypeAnalysis = useMemo(() => {
    ...
    const deliveryRate = coreMetrics.transactions > 0 ? (deliveryCount / coreMetrics.transactions) * 100 : 0;
    ...
    calculations: [
      ...
      },
      {
        ...
        math: React.createElement(React.Fragment, null,
          ...
          React.createElement('span', { className: 'opacity-50 text-[10px] ms-1' }, 
    ...
    calculations: [
      ...
      },
      {
        ...
        math: React.createElement(React.Fragment, null,
          ...
          React.createElement('span', { className: 'opacity-50 text-[10px] ms-2' }, 
  ...
  const activeCountersTooltip = useMemo(() => {
    ...
    const avgTransactionsPerCounter = coreMetrics.transactions > 0 && activeCounters > 0
      ? coreMetrics.transactions / activeCounters
      : 0;
    const avgRevenuePerCounter = coreMetrics.revenue > 0 && activeCounters > 0
      ? coreMetrics.revenue / activeCounters
      : 0;
      ...
      calculations: [
        {
          ...
          math: React.createElement(React.Fragment, null,
            React.createElement('span', { className: utilizationRate >= 60 ? 'text-emerald-400' : 'text-amber-400' }, 
