...
export const useRealTimeSalesAnalytics = ({
  ...
  const itemsAnalysis = useMemo(() => {
    ...
    todaysSales.forEach((s) => {
      s.items.forEach((i) => {
        ...
        const returnedQty =
          s.itemReturnedQuantities?.[lineKey] || s.itemReturnedQuantities?.[i.id] || 0;
    ...
    return {
      itemsPerTransaction:
        coreMetrics.transactions > 0
          ? money.divide(coreMetrics.itemsSold, coreMetrics.transactions)
          : 0,
      topCategory,
  ...
  const activeCountersTooltip = useMemo(() => {
    ...
    const avgTransactionsPerCounter =
      activeCounters > 0 ? coreMetrics.transactions / activeCounters : 0;
