            ...
            <tbody className="divide-y divide-gray-100 dark:divide-gray-800">
              {filteredSuppliers.map(supplier => (
                <tr key={supplier.id} className="hover:bg-gray-50 dark:hover:bg-gray-800/50 transition-colors">
                  <td className="p-4">
                    <div className="font-bold text-gray-900 dark:text-gray-100 text-sm">{supplier.name}</div>
                  ...
                  <td className="p-4">
                    <div className="flex flex-col gap-1">
                        <a href={`tel:${supplier.phone}`} className="flex items-center gap-1 text-xs text-gray-600 dark:text-gray-400 hover:text-${color}-600">
                            <span className="material-symbols-rounded text-[14px]">call</span> {supplier.phone}
                        </a>
                        <a href={`mailto:${supplier.email}`} className="flex items-center gap-1 text-xs text-gray-600 dark:text-gray-400 hover:text-${color}-600">
            ...
            <form onSubmit={handleSubmit} className="p-5 space-y-4">
              ...
              <div className="grid grid-cols-2 gap-4">
                 ...
                 <div className="space-y-1">
                    ...
                    <input className="w-full p-2.5 rounded-xl bg-gray-50 dark:bg-gray-800 border-none focus:ring-2 focus:ring-inset transition-all" 
                      style={{ '--tw-ring-color': `var(--color-${color}-500)` } as any}
              ...
              <div className="pt-4 flex gap-3">
                 <button type="button" onClick={() => setIsModalOpen(false)} className="flex-1 py-3 rounded-full font-medium text-gray-600 hover:bg-gray-100 dark:text-gray-300 dark:hover:bg-gray-800 transition-colors">
                  {t.modal.cancel}
                </button>
