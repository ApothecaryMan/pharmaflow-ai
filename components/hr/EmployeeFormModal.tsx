...
export const EmployeeFormModal: React.FC<EmployeeFormModalProps> = ({
  ...
  const handleSave = async () => {
    ...
    if (wantsToChangePassword && formData.password && formData.password.trim().length > 0) {
      ...
      if (employee?.password && !isOldPasswordVerified) {
        playError();
    ...
    <Modal
      ...
      footer={
        <div className='flex items-center justify-end gap-3'>
          <button onClick={handleClose} className={MODAL_FOOTER_BTN_CANCEL} type='button'>
            {t.modal.cancel}
          </button>
          <button onClick={handleSave} className={MODAL_FOOTER_BTN_PRIMARY} type='button'>
            {isSaving ? (
              <>
            ...
            <div className={`${PROFILE_GLASS_CARD_BASE} p-5 space-y-4`}>
              ...
              <div className='space-y-6 pt-4'>
                ...
                <div className='space-y-2'>
                  <span className='text-xs font-semibold text-gray-500 uppercase px-1 flex items-center gap-2'>
                    <span
                      className='material-symbols-rounded'
                      style={{ fontSize: 'var(--icon-md)' }}
                    >
                  ...
                  <div className='flex items-center gap-4'>
                    ...
                    {formData.nationalIdCard ? (
                      ...
                      <div className='flex-1'>
                        <label
                          htmlFor='field-1307'
                          className='flex items-center justify-center gap-2 px-4 py-3 border-2 border-dashed border-(--border-divider) rounded-xl hover:border-primary-400 dark:hover:border-primary-500 transition-colors cursor-pointer bg-(--bg-input)/50'
                        >
                          <span
