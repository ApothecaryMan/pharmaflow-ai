...
export const EmployeeList: React.FC<EmployeeListProps> = ({ color, t, language, onViewChange }) => {
  ...
  const { data: employees = [], isLoading } = useEmployees(activeBranchId);
  ...
  // Sounds
  const { playSuccess: _playSuccess, playError: _playError, playBeep } = usePosSounds();
  ...
  const columns = useMemo<ColumnDef<Employee>[]>(
    ...
    [t, branches, handleView, handleEdit, handleDelete]
  );
  ...
  const handleFormSave = async (finalFormData: Partial<Employee>) => {
    ...
    try {
      if (isEdit) {
        await updateEmployee.mutateAsync({ id: editingEmployee?.id, updates: finalFormData });
      } else {
