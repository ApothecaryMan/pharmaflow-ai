...
export const useStaffAnalytics = ({
  ...
  return useMemo(() => {
    ...
    Object.entries(perfMap).forEach(([eid, perf]) => {
      const salesWithReturns = todaysSales.filter(
        (s) => s.soldByEmployeeId === eid && s.hasReturns
      ).length;
    ...
    Object.entries(perfMap).forEach(([eid, perf]) => {
      ...
      else if (relativeReturn <= RETURN_THRESHOLDS.ACCEPTABLE)
        returnScore = RETURN_SCORES.ACCEPTABLE;
      ...
      let speedScore = 0;
      if (loyaltyHealthy) {
        if (secondsPerItem < SPEED_THRESHOLDS.EXCELLENT)
          speedScore = SPEED_SCORES.HEALTHY.EXCELLENT;
        else if (secondsPerItem < SPEED_THRESHOLDS.VERY_GOOD)
          speedScore = SPEED_SCORES.HEALTHY.VERY_GOOD;
        ...
        else if (secondsPerItem < SPEED_THRESHOLDS.AVERAGE)
          speedScore = SPEED_SCORES.HEALTHY.AVERAGE;
    ...
    const champions = staffStats.reduce(
      (acc, staff) => {
        ...
        if (!acc.loyalty || staffLoyaltyCount > currentLoyaltyCount) {
          ...
          staffLoyaltyCount === 0 &&
          (staff.repeatRatio || 0) > (acc.loyalty?.repeatRatio || 0)
        ) {
    ...
    staffStats.forEach((staff) => {
      ...
      if (speedMaster && staff.id === speedMaster.id)
        titles.push({
          type: 'speed',
      ...
      if (csatHero && staff.id === csatHero.id)
        titles.push({
          type: 'csat',
      ...
      if (loyaltyMaster && staff.id === loyaltyMaster.id)
        titles.push({
          type: 'loyalty',
    ...
    // Tooltip generator
    const getHeroTooltip = (
      hero: StaffStats | undefined,
      ...
      if (type === 'revenue') {
        return {
          ...
          calculations: [
            ...
            {
              label: language === 'AR' ? 'متوسط الفاتورة' : 'Avg Invoice',
              math: hero.revenue / (hero.count || 1),
              isCurrency: true,
            },
          ],
          details: [
            {
              ...
              value: language === 'AR' ? 'الأعلى مبيعات' : 'Top Seller',
              isCurrency: false,
            },
            {
              icon: 'inventory_2',
              label: language === 'AR' ? 'المنتجات' : 'Items',
              value: hero.items,
              isCurrency: false,
            },
          ],
        ...
        return {
          ...
          details: [
            {
              icon: 'bolt',
              label: language === 'AR' ? 'السرعة' : 'Speed',
              value: language === 'AR' ? 'الأسرع' : 'Fastest',
              isCurrency: false,
            },
            {
              icon: 'check_circle',
              label: language === 'AR' ? 'الكفاءة' : 'Efficiency',
              value: language === 'AR' ? 'عالية' : 'High',
              isCurrency: false,
            },
          ],
        };
          ...
          details: [
            {
              ...
              value: language === 'AR' ? 'ممتازة' : 'Excellent',
              isCurrency: false,
            },
            {
              icon: 'thumb_up',
              label: language === 'AR' ? 'الثقة' : 'Trust',
              value: language === 'AR' ? 'عالية جداً' : 'Very High',
              isCurrency: false,
            },
          ],
        };
        ...
        return {
          ...
          calculations: [
            ...
            {
              label: language === 'AR' ? 'عملاء راجعين' : 'Repeat Customers',
              math: repeatCust,
              isCurrency: false,
            },
          ],
          details: [
            {
              icon: 'group_add',
              label: language === 'AR' ? 'عملاء جدد' : 'New Registered',
              value: hero.loyalty,
              isCurrency: false,
            },
            {
              icon: 'star',
              label: language === 'AR' ? 'التقييم' : 'Trust Score',
              value: hero.csat.toFixed(1),
              isCurrency: false,
            },
          ],
          ...
          details: [
            {
              ...
              value: language === 'AR' ? 'الأكثر نشاطاً' : 'Most Active',
              isCurrency: false,
            },
            {
              icon: 'shopping_cart',
    ...
    const performanceColumns: ColumnDef<StaffStats>[] = [
      {
        ...
        cell: ({ row, table }) => {
          ...
          return (
            <div className='flex justify-center'>
              <span
                className={`text-3xl font-black italic tracking-tighter ${textColors[idx]} leading-none`}
              >
                {idx + 1}
              </span>
            </div>
