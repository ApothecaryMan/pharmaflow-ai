...
import { StaffSpotlightTickerProps } from './types/staffOverview.types';
  ...
  return (
    <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 xl:grid-cols-5 gap-4">
      {achievements.map((item, idx) => (
        ...
        >
          {/* Background Glow */}
          <div className={`absolute -right-10 -top-10 w-32 h-32 blur-3xl opacity-10 bg-${item.color}-500 transition-opacity group-hover:opacity-20`} />
          ...
          <div className="flex items-center gap-2 mb-4 relative z-10 min-h-[40px]">
            ...
            {language === 'AR' && AR_IMAGE_MAP[item.type] ? (
              ...
              <span className={`text-lg font-bold uppercase tracking-wider text-${item.color}-700 dark:text-${item.color}-400`}>
                {item.label}
              </span>
            )}
          </div>
          ...
          <div className="flex items-start relative z-10">
            ...
            <div className="flex-1 min-w-0">
              ...
              <div className="mt-1 flex items-center gap-1 text-xs font-medium text-gray-600 dark:text-gray-400">
                 {item.type === 'revenue' && <span className="material-symbols-rounded text-sm">payments</span>}
                 {item.type === 'speed' && <span className="material-symbols-rounded text-sm">timer</span>}
                 {item.type === 'invoices' && <span className="material-symbols-rounded text-sm">receipt_long</span>}
                 {item.type === 'csat' && <span className="material-symbols-rounded text-sm">star</span>}
                 {item.type === 'loyalty' && <span className="material-symbols-rounded text-sm">group_add</span>}
                 
                 <span>
                   {item.type === 'revenue' ? formatCurrency(item.hero?.revenue || 0, 'EGP', language === 'AR' ? 'ar' : 'en', 0) :
                    item.type === 'speed' ? `${(item.hero?.avgTime || 0).toFixed(1)}m` :
                    item.type === 'csat' ? (item.hero?.csat || 0).toFixed(1) :
                    item.type === 'loyalty' ? `${((item.hero?.repeatRatio || 0) * 100).toFixed(0)}%` :
                    (item.hero?.count || 0)}
                 </span>
              </div>
            </div>
