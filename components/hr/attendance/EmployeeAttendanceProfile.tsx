      ...
      <PageHeader
        leftContent={
          <button
            ...
            className='flex items-center gap-2 px-3 py-1.5 rounded-xl bg-[var(--bg-skeleton)] text-zinc-600 dark:text-zinc-400 hover:bg-zinc-200 dark:hover:bg-zinc-700'
            type='button'
          >
        ...
        title={
          report
            ...
            : t.attendance.attendanceProfile
        }
              ...
              <DataPortButton
                ...
                columns={columns.map((c) => ({
                  ...
                  format: (val, row) => {
                    ...
                    if (c.id === 'day') {
                      return new Date(row.date).toLocaleDateString(languageLocale, {
                        weekday: 'short',
                      });
                    }
                    return val;
                    ...
                    <td colSpan={columns.length} className='px-6 py-32 text-center text-zinc-500'>
                      <div className='flex flex-col items-center justify-center gap-6'>
                        <img
                          ...
                          alt='Empty'
                          className='w-48 h-48 object-contain opacity-80 drop-shadow-sm'
                        />
                      ...
                      >
                        ...
                        <td className='px-6 py-4 text-center'>
                          <span className='text-sm font-bold text-zinc-900 dark:text-zinc-100'>
                            {new Date(day.date).toLocaleDateString(languageLocale, {
                              day: 'numeric',
                              month: 'short',
                            })}
                          </span>
                          ...
                          {day.firstIn ? (
                            <div className='flex items-center justify-center gap-2 text-emerald-600 dark:text-emerald-400'>
                              ...
                              <span className='text-sm font-medium'>
                                {new Date(day.firstIn).toLocaleTimeString(languageLocale, {
                                  hour: '2-digit',
                                  minute: '2-digit',
                                })}
                              </span>
                          ...
                          {day.lastOut ? (
                            <div className='flex items-center justify-center gap-2 text-rose-600 dark:text-rose-400'>
                              ...
                              <div className='flex items-center justify-center gap-1'>
                                <span className='text-sm font-medium'>
                                  {new Date(day.lastOut).toLocaleTimeString(languageLocale, {
                                    hour: '2-digit',
                                    minute: '2-digit',
                                  })}
                                </span>
                        ...
                        <td className='px-6 py-4 text-center'>
                          <span
                            className={`text-sm font-mono ${day.totalMinutes > 0 ? 'text-zinc-900 dark:text-zinc-100' : 'text-zinc-300 dark:text-zinc-700'}`}
                          >
