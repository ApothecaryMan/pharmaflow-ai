    ...
    <div className="h-full flex flex-col space-y-4 animate-fade-in">
      ...
      <div className="relative">
        ...
        <input 
          ...
          className="w-full ps-12 pe-4 py-3 rounded-full bg-white dark:bg-slate-900 border border-slate-200 dark:border-slate-800 focus:outline-none focus:ring-2 transition-all text-sm"
            ...
            <tbody className="divide-y divide-slate-100 dark:divide-slate-800">
              {filteredSuppliers.map(supplier => (
                <tr key={supplier.id} className="hover:bg-slate-50 dark:hover:bg-slate-800/50 transition-colors">
                  <td className="p-4">
                    <div className="font-bold text-slate-900 dark:text-slate-100 text-sm">{supplier.name}</div>
                  ...
                  <td className="p-4">
                    <div className="flex flex-col gap-1">
                        <a href={`tel:${supplier.phone}`} className="flex items-center gap-1 text-xs text-slate-600 dark:text-slate-400 hover:text-${color}-600">
                            <span className="material-symbols-rounded text-[14px]">call</span> {supplier.phone}
                        </a>
                        <a href={`mailto:${supplier.email}`} className="flex items-center gap-1 text-xs text-slate-600 dark:text-slate-400 hover:text-${color}-600">
                            <span className="material-symbols-rounded text-[14px]">mail</span> {supplier.email}
                        </a>
                    </div>
                  </td>
                  <td className="p-4 text-end space-x-2 rtl:space-x-reverse">
                    <button 
                      ...
                      className={`p-2 rounded-full hover:bg-${color}-50 dark:hover:bg-${color}-900/30 text-slate-400 hover:text-${color}-600 transition-colors`}
                    >
          ...
          <div className="bg-white dark:bg-slate-900 w-full max-w-lg rounded-3xl shadow-2xl overflow-hidden flex flex-col">
            ...
            <form onSubmit={handleSubmit} className="p-5 space-y-4">
              <div className="space-y-1">
                <label className="text-xs font-bold text-slate-500 uppercase">{t.modal.name}</label>
              ...
              <div className="grid grid-cols-2 gap-4">
                 <div className="space-y-1">
                    <label className="text-xs font-bold text-slate-500 uppercase">{t.modal.contact}</label>
                    <input required className="w-full p-2.5 rounded-xl bg-slate-50 dark:bg-slate-800 border-none focus:ring-2 focus:ring-inset transition-all" 
                      style={{ '--tw-ring-color': `var(--color-${color}-500)` } as any}
                      value={formData.contactPerson} onChange={e => setFormData({...formData, contactPerson: e.target.value})} />
                </div>
                 <div className="space-y-1">
                    <label className="text-xs font-bold text-slate-500 uppercase">{t.modal.phone}</label>
              ...
              <div className="space-y-1">
                <label className="text-xs font-bold text-slate-500 uppercase">{t.modal.email}</label>
                <input type="email" className="w-full p-2.5 rounded-xl bg-slate-50 dark:bg-slate-800 border-none focus:ring-2 focus:ring-inset transition-all" 
                  style={{ '--tw-ring-color': `var(--color-${color}-500)` } as any}
                  value={formData.email} onChange={e => setFormData({...formData, email: e.target.value})} />
              </div>

              <div className="space-y-1">
                <label className="text-xs font-bold text-slate-500 uppercase">{t.modal.address}</label>
