...
import { useExpenses, type ExpenseFilterType } from '../../hooks/finance/useExpenses';
...
export const ExpenseTracker: React.FC<ExpenseTrackerProps> = ({
  ...
  const selectedCategoryFilter = useMemo(() => {
    return categoryFilterOptions.find(o => o.value === categoryFilter) || categoryFilterOptions[0];
  }, [categoryFilterOptions, categoryFilter]);

  const selectedPaymentFilter = useMemo(() => {
    return paymentFilterOptions.find(o => o.value === paymentMethodFilter) || paymentFilterOptions[0];
  ...
  return (
    <div className="h-full overflow-y-auto pb-8 scrollbar-hide text-start" dir={isRtl ? 'rtl' : 'ltr'}>
      ...
      <div className="px-4">
        {filterType === 'custom' && (
          <div className="mb-4 p-4 bg-(--bg-card) border border-(--border-divider) rounded-2xl flex flex-wrap gap-4 items-end">
            <div>
              <label htmlFor="custom-date-from" className="block text-xs font-bold text-(--text-secondary) uppercase mb-1">
            ...
            <div>
              <label htmlFor="custom-date-to" className="block text-xs font-bold text-(--text-secondary) uppercase mb-1">
                {t.date}
              </label>
              <input
                id="custom-date-to"
