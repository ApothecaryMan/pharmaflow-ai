...
type ExpandedView = 'total' | 'lifetimeValue' | 'newCustomers' | 'location' | 'topCustomers' | 
                    'loyaltyPoints' | 'healthInsights' | 'recentActivity' | 'engagement' | 
                    'contactPreferences' | 'segmentation' | 'satisfaction' | null;

const COLORS = ['#06b6d4', '#8b5cf6', '#10b981', '#f43f5e', '#f59e0b', '#3b82f6', '#ec4899', '#14b8a6'];
...
export const CustomerOverview: React.FC<CustomerOverviewProps> = ({
  ...
  const locationDistribution = useMemo(() => {
    ...
    customers.forEach(c => {
      ...
      const locationName = c.governorate ? getLocationName(c.governorate, 'gov', language) : (language === 'AR' ? 'غير محدد' : 'Unknown');
  ...
  const totalLoyaltyPoints = useMemo(() => customers.reduce((sum, c) => sum + c.points, 0), [customers]);
  ...
  const engagementMetrics = useMemo(() => {
    ...
    const avgDays = daysSinceLastVisit.length > 0 
    ...
    return {
      ...
      activeRate: customers.length > 0 ? ((customers.length - inactiveCount) / customers.length) * 100 : 0
    };
  }, [customers]);
  ...
  const segmentation = useMemo(() => {
    ...
    const regular = customers.filter(c => c.totalPurchases >= regularThreshold && c.totalPurchases < vipThreshold);
  ...
  const satisfactionMetrics = useMemo(() => {
    ...
    const repeatRate = Object.keys(customerPurchaseCounts).length > 0 
  ...
  const StatCard = ({ 
    ...
    >
      <div className="absolute top-3 right-3 rtl:right-auto rtl:left-3">
        <ExpandButton onClick={(e) => { e?.stopPropagation?.(); onClick?.(); }} />
      </div>
  ...
  return (
    <div className="h-full overflow-y-auto pe-2 space-y-4 pb-10">
      <h2 className="text-2xl font-medium tracking-tight mb-4">{t?.title || 'Customer Overview'}</h2>
        ...
        <div className="p-5 rounded-3xl bg-white dark:bg-slate-900 border border-slate-200 dark:border-slate-800 shadow-sm flex flex-col group">
          ...
          <div className="grid grid-cols-2 gap-4 mb-4">
            <div className="p-3 rounded-xl bg-blue-50 dark:bg-blue-950/20">
              <p className="text-xs font-bold text-blue-600 dark:text-blue-400 uppercase">{t?.withInsurance || 'With Insurance'}</p>
          ...
          <div className="grid grid-cols-2 md:grid-cols-4 gap-3">
            <div className="p-4 rounded-2xl bg-gradient-to-br from-amber-50 to-amber-100 dark:from-amber-950/40 dark:to-amber-900/20 border border-amber-200 dark:border-amber-800/50">
              <div className="flex items-center gap-2 mb-2">
                ...
                <span className="text-xs font-bold text-amber-700 dark:text-amber-400 uppercase">{t?.vip || 'VIP'}</span>
              </div>
              <p className="text-2xl font-bold text-amber-900 dark:text-amber-100">{segmentation.vip.length}</p>
            ...
            <div className="p-4 rounded-2xl bg-gradient-to-br from-blue-50 to-blue-100 dark:from-blue-950/40 dark:to-blue-900/20 border border-blue-200 dark:border-blue-800/50">
              <div className="flex items-center gap-2 mb-2">
                ...
                <span className="text-xs font-bold text-blue-700 dark:text-blue-400 uppercase">{t?.regular || 'Regular'}</span>
              </div>
              <p className="text-2xl font-bold text-blue-900 dark:text-blue-100">{segmentation.regular.length}</p>
            ...
            <div className="p-4 rounded-2xl bg-gradient-to-br from-red-50 to-red-100 dark:from-red-950/40 dark:to-red-900/20 border border-red-200 dark:border-red-800/50">
              <div className="flex items-center gap-2 mb-2">
                ...
                <span className="text-xs font-bold text-red-700 dark:text-red-400 uppercase">{t?.atRisk || 'At Risk'}</span>
              </div>
              <p className="text-2xl font-bold text-red-900 dark:text-red-100">{segmentation.atRisk.length}</p>
              <p className="text-xs text-red-600 dark:text-red-400">60+ {t?.daysInactive || 'days inactive'}</p>
            </div>
          </div>
        </div>
        ...
        <div className="space-y-6">
          <div className="grid grid-cols-1 md:grid-cols-3 gap-4">
            <div className={`p-6 rounded-2xl bg-${color}-50 dark:bg-${color}-950/20 border border-${color}-100 dark:border-${color}-900`}>
              <p className={`text-sm font-bold text-${color}-800 dark:text-${color}-300 uppercase mb-2`}>{t?.totalCustomers || 'Total'}</p>
              <p className={`text-4xl font-bold text-${color}-900 dark:text-${color}-100`}>{totalCustomers}</p>
            </div>
      ...
      <ExpandedModal
        ...
        actions={
          <button
            onClick={() => exportToCSV(topCustomers.map((c, i) => ({
              rank: i + 1,
              code: c.code,
              name: c.name,
              totalPurchases: c.totalPurchases,
        ...
        <div className="space-y-6">
          <div className="grid grid-cols-2 gap-4">
            <div className="p-4 rounded-xl bg-blue-50 dark:bg-blue-950/20">
              <p className="text-sm font-bold text-blue-600 dark:text-blue-400 uppercase">{t?.withInsurance || 'With Insurance'}</p>
