...
import {
  ...
  ComposedChart, Treemap, Funnel, FunnelChart, LabelList, Legend
} from 'recharts';
...
const salesTrendData = [
  ...
  { name: 'Apr', sales: 4780 }, { name: 'May', sales: 5890 }, { name: 'Jun', sales: 4390 },
  { name: 'Jul', sales: 6490 }, { name: 'Aug', sales: 7200 }, { name: 'Sep', sales: 6800 },
  { name: 'Oct', sales: 7800 }, { name: 'Nov', sales: 8200 }, { name: 'Dec', sales: 9400 }
];
...
const dailyTrafficData = [
  { hour: '6AM', visitors: 20 }, { hour: '8AM', visitors: 85 }, { hour: '10AM', visitors: 150 },
  { hour: '12PM', visitors: 180 }, { hour: '2PM', visitors: 160 }, { hour: '4PM', visitors: 200 },
  { hour: '6PM', visitors: 250 }, { hour: '8PM', visitors: 120 }, { hour: '10PM', visitors: 50 }
];
...
const scatterData = [
  { price: 10, sales: 150 }, { price: 15, sales: 120 }, { price: 20, sales: 100 },
  { price: 25, sales: 85 }, { price: 30, sales: 70 }, { price: 35, sales: 55 },
  { price: 40, sales: 40 }, { price: 50, sales: 30 }, { price: 12, sales: 140 },
  { price: 18, sales: 110 }, { price: 22, sales: 95 }, { price: 28, sales: 75 }
];
...
const gaugeData = [
  { name: 'Target', value: 78, fill: '#10b981' }
];
...
// ========== COMPONENT ==========
export const DashboardExperiments: React.FC<DashboardExperimentsProps> = ({ color, t, language }) => {
  ...
  const SimpleTooltip = ({ active, payload, label }: any) => {
    if (active && payload && payload.length) {
      return (
        <div className="bg-white dark:bg-gray-800 p-3 rounded-xl shadow-lg border border-gray-200 dark:border-gray-700">
          <p className="text-xs font-medium text-gray-500 dark:text-gray-400 mb-1">{label || payload[0].payload?.name}</p>
          <p className="text-lg font-bold text-gray-900 dark:text-gray-100">
            {payload[0].value.toLocaleString()}
          </p>
        </div>
  ...
  return (
    <div className="h-full overflow-y-auto pe-2 space-y-6 animate-fade-in pb-10" dir={isRTL ? 'rtl' : 'ltr'}>
      <h2 className="text-2xl font-bold tracking-tight mb-6 type-expressive text-gray-900 dark:text-gray-100">{labels.title}</h2>
      ...
      <div className="grid grid-cols-1 md:grid-cols-2 xl:grid-cols-3 gap-4">
          ...
          <div className="flex-1" dir="ltr">
            <ResponsiveContainer width="100%" height="100%">
              <LineChart data={salesTrendData} margin={{ top: 5, right: 10, left: -15, bottom: 0 }}>
                ...
                <XAxis dataKey="name" axisLine={false} tickLine={false} tick={{ fill: chartStyle.tick, fontSize: 10 }} />
                <YAxis axisLine={false} tickLine={false} tick={{ fill: chartStyle.tick, fontSize: 10 }} />
                <Tooltip content={<CustomTooltip />} cursor={{ stroke: chartStyle.grid, strokeWidth: 1, strokeDasharray: '4 4' }} />
                <Line type="monotone" dataKey="sales" stroke="url(#lineGradient1)" strokeWidth={3} dot={false} />
              </LineChart>
            </ResponsiveContainer>
          </div>
        </div>
        ...
        <div className={`p-5 rounded-3xl ${CARD_BASE} h-72 flex flex-col`}>
          ...
          <div className="flex-1" dir="ltr">
            <ResponsiveContainer width="100%" height="100%">
              <BarChart data={categoryRevenueData} margin={{ top: 5, right: 10, left: -15, bottom: 0 }}>
                <defs>
                ...
                <CartesianGrid strokeDasharray="3 3" vertical={false} stroke={chartStyle.grid} />
                <XAxis dataKey="name" axisLine={false} tickLine={false} tick={{ fill: chartStyle.tick, fontSize: 9 }} />
                <YAxis axisLine={false} tickLine={false} tick={{ fill: chartStyle.tick, fontSize: 10 }} />
        ...
        <div className={`p-5 rounded-3xl ${CARD_BASE} h-72 flex flex-col`}>
          ...
          <div className="flex-1 flex items-center">
            <ResponsiveContainer width="55%" height="100%">
              <PieChart>
                <Pie data={customerSegmentsData} dataKey="value" cx="50%" cy="50%" outerRadius={60} strokeWidth={0}>
            ...
            <div className="flex-1 space-y-2 ps-2">
              {customerSegmentsData.map((item) => (
                <div key={item.name} className="flex items-center gap-2 text-xs">
                  ...
                  <span className="font-bold text-gray-800 dark:text-gray-200 ms-auto">{item.value}%</span>
                </div>
              ))}
            </div>
          </div>
        ...
        <div className={`p-5 rounded-3xl ${CARD_BASE} h-72 flex flex-col`}>
          <h3 className="text-sm font-semibold text-gray-800 dark:text-gray-200 mb-3 flex items-center gap-2">
            <span className="material-symbols-rounded text-emerald-500 text-[20px]">donut_large</span>
            {labels.card4}
          </h3>
          <div className="flex-1 flex items-center">
            <ResponsiveContainer width="55%" height="100%">
              <PieChart>
                <Pie data={stockStatusData} dataKey="value" cx="50%" cy="50%" innerRadius={35} outerRadius={60} strokeWidth={0}>
        ...
        <div className={`p-5 rounded-3xl ${CARD_BASE} h-72 flex flex-col`}>
          ...
          <div className="flex-1" dir="ltr">
            <ResponsiveContainer width="100%" height="100%">
              <AreaChart data={dailyTrafficData} margin={{ top: 5, right: 10, left: -15, bottom: 0 }}>
                <defs>
                ...
                <CartesianGrid strokeDasharray="3 3" vertical={false} stroke={chartStyle.grid} />
                <XAxis dataKey="hour" axisLine={false} tickLine={false} tick={{ fill: chartStyle.tick, fontSize: 10 }} />
                <YAxis axisLine={false} tickLine={false} tick={{ fill: chartStyle.tick, fontSize: 10 }} />
                <Tooltip content={<SimpleTooltip />} cursor={{ stroke: chartStyle.grid, strokeWidth: 1, strokeDasharray: '4 4' }} />
                <Area type="monotone" dataKey="visitors" stroke="#8b5cf6" fill="url(#areaGradient1)" strokeWidth={2} />
              </AreaChart>
            </ResponsiveContainer>
          </div>
        </div>
              ...
              {performanceGoalsData.map((item) => (
                <div key={item.name} className="flex flex-col items-center text-center">
                  <div className="flex items-center gap-1.5 mb-1">
                    ...
                    <span className="font-bold text-gray-800 dark:text-gray-200 text-lg">{item.value}%</span>
                  </div>
                  <span className="text-gray-500 dark:text-gray-400 text-[10px] leading-tight line-clamp-2 w-full">{item.name}</span>
                </div>
              ))}
            </div>
          </div>
              ...
              return (
                <div key={item.name}>
                  <div className="flex justify-between items-center text-xs mb-1.5">
                    ...
                    <span className="font-bold text-gray-800 dark:text-gray-200">{item.completed}/{item.total}</span>
                  </div>
        ...
        >
          ...
          <div className="flex-1 flex flex-col justify-between relative z-10">
            <div>
              <p className="text-xs text-white/70 uppercase font-bold tracking-wider mb-1">{labels.totalRevenue}</p>
        ...
        <div className={`p-5 rounded-3xl ${CARD_BASE} h-72 flex flex-col`}>
          <h3 className="text-sm font-semibold text-gray-800 dark:text-gray-200 mb-3 flex items-center gap-2">
            <span className="material-symbols-rounded text-blue-500 text-[20px]">calendar_view_week</span>
            {labels.card9}
          </h3>
            ...
            <table className="w-full text-xs">
              <thead>
                <tr>
                  ...
                  <th className="text-center text-gray-500 dark:text-gray-400 pb-2 font-medium">{labels.morning}</th>
                  <th className="text-center text-gray-500 dark:text-gray-400 pb-2 font-medium">{labels.afternoon}</th>
                  <th className="text-center text-gray-500 dark:text-gray-400 pb-2 font-medium">{labels.evening}</th>
                </tr>
              </thead>
              <tbody>
                {heatmapData.map((row) => (
                  <tr key={row.day}>
                    <td className="py-1 text-gray-600 dark:text-gray-400 font-medium text-[11px]">{row.day}</td>
        ...
        <div className={`p-5 rounded-3xl ${CARD_BASE} h-72 flex flex-col`}>
          <h3 className="text-sm font-semibold text-gray-800 dark:text-gray-200 mb-3 flex items-center gap-2">
            <span className="material-symbols-rounded text-amber-500 text-[20px]">scatter_plot</span>
            {labels.card10}
          </h3>
          <div className="flex-1" dir="ltr">
            <ResponsiveContainer width="100%" height="100%">
                      ...
                      return (
                        <div className="bg-white dark:bg-gray-800 p-3 rounded-xl shadow-lg border border-gray-200 dark:border-gray-700">
                          <p className="text-xs text-gray-500 dark:text-gray-400">
                            Price: <span className="font-bold text-gray-800 dark:text-gray-200">${payload[0].payload.price}</span>
                          </p>
                          <p className="text-xs text-gray-500 dark:text-gray-400">
                            Sales: <span className="font-bold text-gray-800 dark:text-gray-200">{payload[0].payload.sales}</span>
                          </p>
                        </div>
                      );
                    }
        ...
        <div className={`p-5 rounded-3xl ${CARD_BASE} h-72 flex flex-col`}>
          <h3 className="text-sm font-semibold text-gray-800 dark:text-gray-200 mb-3 flex items-center gap-2">
            <span className="material-symbols-rounded text-blue-500 text-[20px]">stacked_bar_chart</span>
            {labels.card11}
          </h3>
          <div className="flex-1" dir="ltr">
            <ResponsiveContainer width="100%" height="100%">
              <BarChart data={stackedBarData} margin={{ top: 5, right: 10, left: -15, bottom: 0 }}>
                <CartesianGrid strokeDasharray="3 3" vertical={false} stroke={chartStyle.grid} />
                <XAxis dataKey="month" axisLine={false} tickLine={false} tick={{ fill: chartStyle.tick, fontSize: 10 }} />
                <YAxis axisLine={false} tickLine={false} tick={{ fill: chartStyle.tick, fontSize: 10 }} />
                <Tooltip 
                      ...
                      return (
                        <div className="bg-white dark:bg-gray-800 p-3 rounded-xl shadow-lg border border-gray-200 dark:border-gray-700">
                          ...
                          <p className="text-xs font-bold text-gray-700 dark:text-gray-300 mt-1 pt-1 border-t border-gray-200 dark:border-gray-600">
                            Total: ${payload.reduce((sum: number, e: any) => sum + e.value, 0).toLocaleString()}
                          </p>
                        </div>
                      );
                    }
                ...
                />
                <Bar dataKey="medicine" name="Medicine" stackId="a" fill="#3b82f6" radius={[0, 0, 0, 0]} />
                <Bar dataKey="cosmetics" name="Cosmetics" stackId="a" fill="#8b5cf6" />
                <Bar dataKey="equipment" name="Equipment" stackId="a" fill="#10b981" radius={[4, 4, 0, 0]} />
              </BarChart>
            </ResponsiveContainer>
          </div>
        </div>
            ...
            <div className="flex-1 space-y-1.5 ps-2">
              {funnelData.map((item, idx) => (
                <div key={item.name} className="flex items-center gap-2 text-xs">
                  <div className="w-2.5 h-2.5 rounded-full shrink-0" style={{ backgroundColor: item.fill }}></div>
                  <span className="text-gray-600 dark:text-gray-400">{item.name}</span>
                  <span className="font-bold text-gray-800 dark:text-gray-200 ms-auto">{item.value.toLocaleString()}</span>
                </div>
              ))}
            </div>
          </div>
        ...
        <div className={`p-5 rounded-3xl ${CARD_BASE} h-72 flex flex-col`}>
          ...
          <div className="flex-1 flex flex-col items-center justify-center">
            ...
            <div className="text-center -mt-8">
              <p className="text-4xl font-bold text-gray-900 dark:text-gray-100">{gaugeData[0].value}%</p>
              <p className="text-xs text-gray-500 dark:text-gray-400 mt-1">{isRTL ? 'تحقيق الهدف' : 'Target Achieved'}</p>
            </div>
          </div>
        </div>
          ...
          <div className="flex-1" dir="ltr">
            <ResponsiveContainer width="100%" height="100%">
              ...
              >
                ...
                <XAxis type="number" axisLine={false} tickLine={false} tick={{ fill: chartStyle.tick, fontSize: 10 }} />
                <YAxis 
                  type="category" 
                  dataKey="name" 
                  axisLine={false} 
                ...
                <Tooltip 
                  ...
                  content={({ active, payload }) => {
                    if (active && payload && payload.length) {
                      return (
                        <div className="bg-white dark:bg-gray-800 p-3 rounded-xl shadow-lg border border-gray-200 dark:border-gray-700">
                          <p className="text-xs font-medium text-gray-500 dark:text-gray-400">{payload[0].payload.name}</p>
                          <p className="text-lg font-bold text-gray-900 dark:text-gray-100">{payload[0].value} units</p>
                        </div>
                      );
                    }
                    return null;
        ...
        <div className={`p-5 rounded-3xl ${CARD_BASE} h-72 flex flex-col`}>
          ...
          <div className="flex-1" dir="ltr">
            <ResponsiveContainer width="100%" height="100%">
              <ComposedChart data={composedData} margin={{ top: 5, right: 10, left: -15, bottom: 0 }}>
                <CartesianGrid strokeDasharray="3 3" vertical={false} stroke={chartStyle.grid} />
                <XAxis dataKey="month" axisLine={false} tickLine={false} tick={{ fill: chartStyle.tick, fontSize: 10 }} />
                <YAxis yAxisId="left" axisLine={false} tickLine={false} tick={{ fill: chartStyle.tick, fontSize: 10 }} />
                <YAxis yAxisId="right" orientation="right" axisLine={false} tickLine={false} tick={{ fill: chartStyle.tick, fontSize: 10 }} />
                <Tooltip 
                  content={({ active, payload, label }) => {
                    if (active && payload && payload.length) {
                      return (
                        <div className="bg-white dark:bg-gray-800 p-3 rounded-xl shadow-lg border border-gray-200 dark:border-gray-700">
                          <p className="text-xs font-medium text-gray-500 dark:text-gray-400 mb-1">{label}</p>
                          <p className="text-sm text-blue-600 dark:text-blue-400">Revenue: <span className="font-bold">${payload[0]?.value?.toLocaleString()}</span></p>
                          <p className="text-sm text-emerald-600 dark:text-emerald-400">Orders: <span className="font-bold">{payload[1]?.value}</span></p>
                        </div>
                      );
                    }
                    return null;
                ...
                <Line yAxisId="right" type="monotone" dataKey="orders" stroke="#10b981" strokeWidth={3} dot={{ fill: '#10b981', strokeWidth: 2 }} />
              </ComposedChart>
            </ResponsiveContainer>
          </div>
        </div>
