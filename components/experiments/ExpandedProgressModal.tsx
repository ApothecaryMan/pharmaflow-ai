...
export const ExpandedProgressModal: React.FC<ExpandedProgressModalProps> = ({
  isOpen, onClose, title, currentValue, maxValue, color, icon
}) => {
    ...
    >
      <div className="space-y-6">
        ...
        <div className="flex flex-col md:flex-row gap-4">
          ...
          <div className="flex-1 p-6 rounded-3xl relative overflow-hidden flex flex-col items-center justify-center
             ...
             <div className="relative z-10 text-center">
                ...
                <div className="w-40 h-40 relative mx-auto mb-4 flex items-center justify-center">
                    ...
                    <div className="absolute inset-0 flex flex-col items-center justify-center">
                        ...
                        <span className={`text-xs font-bold uppercase tracking-wider text-${color}-600 dark:text-${color}-400 mt-1`}>
                            Complete
                        </span>
                    </div>
                </div>
          ...
          <div className="flex-1 grid grid-cols-2 gap-4">
               <div className="p-4 rounded-2xl bg-white dark:bg-gray-800 shadow-sm border border-gray-100 dark:border-gray-700 flex flex-col justify-center">
                   ...
                   <h4 className="text-2xl font-bold text-gray-900 dark:text-white">+24<span className="text-sm font-medium text-gray-400 ml-1">/day</span></h4>
                   <p className="text-xs text-emerald-500 font-medium mt-1">↑ 12% vs avg</p>
               </div>

               <div className="p-4 rounded-2xl bg-white dark:bg-gray-800 shadow-sm border border-gray-100 dark:border-gray-700 flex flex-col justify-center">
                   <div className="mb-2">
                       <span className="text-xs font-bold text-gray-400 uppercase tracking-wider">Remaining</span>
                   </div>
                   <h4 className="text-2xl font-bold text-gray-900 dark:text-white">{(maxValue - currentValue).toLocaleString()}</h4>
                ...
                <div className="col-span-2 p-4 rounded-2xl bg-white dark:bg-gray-800 shadow-sm border border-gray-100 dark:border-gray-700 flex items-center justify-between">
                    <div>
                         <p className="text-gray-400 text-xs font-bold uppercase tracking-wider mb-1">Project Health</p>
        ...
        <div className="h-80 min-h-[320px] rounded-2xl border border-gray-100 dark:border-gray-800 bg-white dark:bg-gray-900 shadow-sm overflow-hidden relative">
            {viewMode === 'timeline' ? (
                ...
                <div className="w-full h-full overflow-y-auto p-4 space-y-3">
                    {milestones.map((ms) => (
                        <div key={ms.id} className="flex items-start gap-4 p-3 rounded-xl border border-gray-100 dark:border-gray-800 hover:bg-gray-50 dark:hover:bg-gray-800/50 transition-colors">
                            <div className={`mt-1 w-6 h-6 rounded-full flex items-center justify-center border-2 
                                ${ms.status === 'completed' ? 'bg-emerald-100 border-emerald-500 text-emerald-600' : 
                                  ms.status === 'in-progress' ? `bg-${color}-50 border-${color}-500 text-${color}-600` :
                                  'bg-gray-50 border-gray-300 text-gray-300'}
                            `}>
                                {ms.status === 'completed' && <span className="material-symbols-rounded text-sm">check</span>}
                                {ms.status === 'in-progress' && <span className="w-2 h-2 rounded-full bg-current animate-pulse"></span>}
                            </div>
                            <div className="flex-1">
                                <div className="flex justify-between items-start">
                                    <h4 className={`text-sm font-semibold ${ms.status === 'completed' ? 'text-gray-500 line-through' : 'text-gray-900 dark:text-white'}`}>
                                        {ms.title}
                                    </h4>
                            ...
                            <span className={`px-2 py-0.5 rounded text-[10px] font-bold uppercase tracking-wide
                                ${ms.status === 'completed' ? 'bg-emerald-100 text-emerald-700' :
                                  ms.status === 'in-progress' ? `bg-${color}-100 text-${color}-700` :
