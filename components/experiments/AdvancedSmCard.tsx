  ...
  return (
    <div className='flex items-center bg-gray-100 dark:bg-[#2A2A2A] rounded-full px-5 h-[40px] gap-4 w-fit border border-gray-200 dark:border-white/5 shadow-sm'>
      ...
      <div className='flex items-center justify-center gap-1.5 ltr:border-r rtl:border-l border-gray-300 dark:border-gray-700/80 ltr:pr-4 rtl:pl-4 h-5 whitespace-nowrap'>
        <span
          className={`material-symbols-rounded text-[18px] ${overfillPercentage > 0 ? 'text-yellow-500' : 'text-red-500'}`}
        >
          target
        </span>
        <div className='flex items-center gap-1'>
          <span
            className={`text-[15px] leading-none font-bold tracking-tight ${sideStat.valueColor || 'text-gray-900 dark:text-white'}`}
          >
            {sideStat.value}
          </span>
          {sideStat.targetValue && (
        ...
        <div className='relative flex h-2.5 w-full rounded-full overflow-hidden bg-gray-200 dark:bg-gray-800'>
          ...
          {overfillPercentage > 0 && (
            <div
              ...
              className={`bg-yellow-400 absolute ltr:left-0 rtl:right-0 top-0 h-full transition-all duration-300 ease-out shadow-[inset_0_0_8px_rgba(255,255,255,0.4)]`}
            />
          )}
        </div>
      </div>
...
export const ActionCard = ({ title, icon, color, actionLabel }: any) => (
  ...
  >
    <div className={`text-primary-500 dark:text-primary-400 flex items-center justify-center mb-2`}>
    ...
    <div className='h-full overflow-y-auto w-full p-2 space-y-8' dir={isRTL ? 'rtl' : 'ltr'}>
      ...
      <div>
        <h2 className='text-2xl font-bold text-gray-900 dark:text-white mb-2'>
          Advanced Small Cards
        </h2>
        <p className='text-gray-500 dark:text-gray-400'>
          Structured layout: Metric, Charts, Progress, Bar, Actions, Others.
        </p>
      </div>
      ...
      <section>
        <h3 className='text-lg font-semibold text-gray-800 dark:text-gray-200 mb-4 px-1'>
          2. Area Charts (Sparklines) - Click to Expand
        </h3>
      ...
      <section>
        ...
        <div className='grid grid-cols-1 sm:grid-cols-2 md:grid-cols-3 xl:grid-cols-4 gap-4'>
          ...
          <ProgressCard
            ...
            progressColor='amber'
            icon='warehouse'
            onClick={() => handleProgressClick('Stock Capacity', 820, 1000, 'amber', 'warehouse')}
          />
          <ProgressCard
            title='Goal A'
            value={45}
            ...
            icon='flag'
            onClick={() => handleProgressClick('Goal A', 45, 100, 'emerald', 'flag')}
          />
          <ProgressCard
            title='Goal B'
            value={90}
            ...
            icon='star'
            onClick={() => handleProgressClick('Goal B', 90, 100, 'purple', 'star')}
          />
        </div>
      </section>
      ...
      <section>
        ...
        <p className='text-sm text-gray-500 mb-4 px-1'>
          Two compact cards (gap-2) should equal the height of one standard card.
        </p>
        <div className='grid grid-cols-1 sm:grid-cols-2 md:grid-cols-3 xl:grid-cols-4 gap-4'>
      ...
      <section>
        ...
        <div className='flex flex-col gap-6'>
          ...
          <div>
            ...
            <p className='text-sm text-gray-500 mb-4 px-1'>
              Optimized for dense dashboards, aligns with standard Small Cards.
            </p>
