    ...
    <div className='p-2 min-w-[200px]' onClick={(e) => e.stopPropagation()}>
      <div className='text-[10px] font-black text-gray-400 uppercase tracking-widest mb-2 px-1 flex items-center justify-between'>
        ...
        <span className='text-primary-500'>
          {globalDeliveryFee} {t.deliveryCalculator.perKm}
        </span>
      </div>
        ...
        {[1, 1.5, 2, 2.5, 3, 4, 5, 10].map((dist) => (
          ...
          >
            ...
            <span className='text-[10px] font-bold text-gray-500'>
              {dist * globalDeliveryFee}{' '}
              <span className='text-[8px] opacity-50'>{t.deliveryCalculator.egp}</span>
            </span>
          </button>
        ))}
      </div>
            ...
            <div className='flex items-center gap-3'>
              ...
              <div className='flex items-center gap-2 text-[11px] font-bold text-gray-500 tracking-wider'>
                {cart.length > 0 ? (
                  <>
                    <svg
                      className='size-1.5 animate-pulse shrink-0'
                      viewBox='0 0 8 8'
                      fill='none'
                    ...
                    >
                      <span className='uppercase text-emerald-600 dark:text-emerald-400 whitespace-nowrap'>
                        {cart.length} {t.drugTypes}
                      </span>
                    </Tooltip>
                    <svg
                      className='size-1 shrink-0 mx-0.5'
                      viewBox='0 0 6 6'
                      fill='none'
                      xmlns='http://www.w3.org/2000/svg'
                    >
                              ...
                              <div className='flex flex-col gap-1 py-0.5 min-w-[120px]'>
                                ...
                                <div className='flex justify-between gap-4 text-xs'>
                                  ...
                                  <span className='font-bold tabular-nums text-emerald-400'>
                                    {totalProfit >= 0 ? '+' : ''}
                                    {formatCurrency(totalProfit, 'EGP', undefined, 2, true)}
                                  </span>
                                </div>
                      ...
                      <React.Fragment key={itemId}>
                        ...
                        >
                          <SortableCartItem
                            ...
                            allBatches={(batchesMap.get(getGroupingKey(group.common)) || []).filter(
                              (b) => (b.stock || 0) > 0
                            )}
                ...
                <div className='flex items-center justify-between text-[11px] text-emerald-600 dark:text-emerald-400 font-bold uppercase tracking-wider'>
                  <div className='flex items-center gap-1'>
                    ...
                    {orderDiscountPercent > 0 && (
                      <span className='bg-emerald-500/10 px-1 rounded text-[9px]'>
                        {orderDiscountPercent}%
                      </span>
                    )}
                  </div>
                  <span className='tabular-nums font-black text-xs'>
                    -{' '}
                    {formatCurrency(
                      money.subtract(grossSubtotal, cartTotal),
                      'EGP',
                      undefined,
                      2,
                      true
              ...
              {taxAmount !== undefined && taxAmount > 0 && (
                <div className='flex items-center justify-between text-[11px] text-gray-500 font-bold uppercase tracking-wider'>
                  <span className='opacity-70'>
                    {t.tax || (currentLang === 'ar' ? 'الضريبة' : 'Tax')}
                  </span>
                  <span className='tabular-nums font-black text-xs text-gray-700 dark:text-gray-300'>
              ...
              <div className='flex h-[42px] overflow-hidden'>
                ...
                >
                  <button
                    ...
                    disabled={
                      !isValidOrder ||
                      !hasOpenShift ||
                      !permissionsService.can('sale.checkout') ||
                      isProcessing
                    }
                ...
                >
                  ...
                  >
                    ...
                    >
                      {formatCurrency(
                        Math.max(0, money.subtract(parseFloat(amountPaid) || 0, finalTotal)),
                        'EGP',
                        undefined,
                        2,
                        true
