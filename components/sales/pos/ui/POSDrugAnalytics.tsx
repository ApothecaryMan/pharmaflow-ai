...
const InfoRow: React.FC<{
  ...
  <div
    className={`flex justify-between items-center py-2 ${!isLast ? 'border-b border-gray-50 dark:border-gray-800/50' : ''}`}
  >
    <div className='flex flex-col'>
    ...
    {tooltip ? (
      <Tooltip content={tooltip}>
        <span className={`text-xs font-black px-2 py-0.5 rounded-md ${valueClass}`}>{value}</span>
      </Tooltip>
    ) : (
      <span className={`text-xs font-black px-2 py-0.5 rounded-md ${valueClass}`}>{value}</span>
    )}
      ...
      try {
        ...
        drugSales.forEach((sale) => {
          ...
          const item = sale.items.find(
            (i) => i.id === viewingDrug.id || i.barcode === viewingDrug.barcode
          );
        ...
        const daysToExpiry = viewingDrug.expiryDate
          ? Math.ceil(
              (new Date(viewingDrug.expiryDate).getTime() - now.getTime()) / (1000 * 60 * 60 * 24)
            )
          : 999;
        ...
        setStats({
          ...
          daysToExpiry,
          expectedWaste:
            dailyVelocity > 0 && daysToExpiry > 0
        ...
        <section className='p-5 rounded-2xl bg-white dark:bg-gray-900 border border-gray-100 dark:border-gray-800'>
          <div className='flex justify-between items-start mb-6'>
            ...
            <span
              ...
              style={{ fontSize: 'var(--icon-sm)', lineHeight: 1 }}
            >
              insights
            </span>
          </div>
          ...
          <div className='flex flex-col gap-1'>
            <InfoRow
              ...
              valueClass={
                stats?.daysOfCover !== '∞' && Number(stats?.daysOfCover) < 7
              ...
              value={
                stats?.expiryStatus === 'expired'
                  ? currentLang === 'ar'
                    ? 'منتهي'
                    : 'Expired'
                  : stats?.expiryStatus === 'critical'
                    ? currentLang === 'ar'
                      ? 'حرجة (<90 يوم)'
                      : 'Critical (<90d)'
                    : stats?.expiryStatus === 'near'
                      ? currentLang === 'ar'
                        ? 'قريبة (<6 شهور)'
                        : 'Near (<6m)'
                      : currentLang === 'ar'
            ...
            <InfoRow
              ...
              hint={
                currentLang === 'ar' ? 'بناءً على معدل البيع الحالي' : 'Based on current velocity'
              }
              value={formatDrugQty(
                stats?.expectedWaste || 0,
                viewingDrug.unitsPerPack || 1,
                currentLang
              )}
              valueClass={
                (stats?.expectedWaste || 0) > 0
            ...
            <InfoRow
              ...
              value={
                viewingDrug.stock <= (viewingDrug.minStock || 0) * (viewingDrug.unitsPerPack || 1)
                  ? currentLang === 'ar'
                    ? 'يجب الطلب فوراً'
                    : 'Order Now'
                  : currentLang === 'ar'
        ...
        <section className='p-5 rounded-2xl bg-white dark:bg-gray-900 border border-gray-100 dark:border-gray-800 flex flex-col justify-between'>
          ...
          <div className='mt-6 pt-4 border-t border-gray-50 dark:border-gray-800 flex justify-between items-center'>
            <span className='text-[10px] font-bold text-gray-400 uppercase'>
              {currentLang === 'ar' ? 'آخر عملية:' : 'Last Recorded:'}
            </span>
