...
const StatCard: React.FC<{
  ...
  <div
    className={`px-3 py-2 rounded-xl border border-gray-100 dark:border-gray-700 ${bgClass} flex items-center justify-between gap-3`}
  >
    <span className='text-[11px] font-black text-gray-400 uppercase tracking-widest flex-shrink-0'>
      {label}
    </span>
  ...
  return (
    <div className='flex flex-col gap-6 p-1' dir={currentLang === 'ar' ? 'rtl' : 'ltr'}>
      ...
      <div className='flex flex-col md:flex-row justify-between items-start gap-4 pb-6 border-b border-gray-100 dark:border-gray-700'>
        <div className='flex flex-wrap gap-3 order-2 md:order-first'>
          <div className='px-4 py-3 rounded-2xl bg-emerald-50 dark:bg-emerald-900/10 border border-emerald-100 dark:border-emerald-800/50 flex items-center gap-3'>
            ...
            <div>
              <span className='block text-[9px] font-black uppercase text-emerald-600 dark:text-emerald-400 tracking-wider mb-0.5'>
                {t.stock || 'Total Stock'}
              </span>
          ...
          <div className='px-4 py-3 rounded-2xl bg-primary-50 dark:bg-primary-900/10 border border-primary-100 dark:border-primary-800/50 flex items-center gap-3'>
            ...
            <div>
              <span className='block text-[9px] font-black uppercase text-primary-600 dark:text-primary-400 tracking-wider mb-0.5'>
                {t.publicPrice || 'Price'}
              </span>
          ...
          {drugBatches[0] && (
            <div className='px-4 py-3 rounded-2xl bg-amber-50 dark:bg-amber-900/10 border border-amber-100 dark:border-amber-800/50 flex items-center gap-3'>
              ...
              <div>
                <span className='block text-[9px] font-black uppercase text-amber-600 dark:text-amber-400 tracking-wider mb-0.5'>
                  {currentLang === 'ar' ? 'أقرب صلاحية' : 'Next Expiry'}
                </span>
        ...
        <div className='space-y-1 order-1 md:order-last text-start' dir='ltr'>
          ...
          {viewingDrug.nameAr && (
            <h3
              className='text-xl font-bold text-primary-600 dark:text-primary-400 mt-1'
      ...
      <div className='grid grid-cols-1 md:grid-cols-2 lg:grid-cols-4 gap-4'>
        ...
        <StatCard
          label={currentLang === 'ar' ? 'المصنع' : 'MFR'}
          value={viewingDrug.manufacturer || '-'}
        />
        <StatCard
          label={currentLang === 'ar' ? 'الشكل' : 'FORM'}
          value={viewingDrug.dosageForm || '-'}
        />
        <StatCard
          label={currentLang === 'ar' ? 'الفئة' : 'CAT'}
          value={viewingDrug.category || '-'}
        />
        <StatCard
          label={currentLang === 'ar' ? 'المنشأ' : 'ORG'}
          value={viewingDrug.origin || '-'}
        />
        ...
        <div className='lg:col-span-2 p-5 rounded-2xl bg-white dark:bg-gray-900/50 border border-gray-100 dark:border-gray-700 flex flex-col gap-6'>
          ...
          <div>
            <h4 className='text-[10px] font-black text-gray-400 uppercase tracking-widest mb-3 flex items-center gap-2'>
              <span className='material-symbols-rounded text-primary-500 text-sm'>
                business_center
              </span>
            ...
            <div className='space-y-0.5'>
              ...
              {permissionsService.can('reports.view_financial') && (
                <OverviewRow
                  icon='payments'
                  label={currentLang === 'ar' ? 'سعر التكلفة' : 'Cost Price'}
                  value={<PriceDisplay value={viewingDrug.costPrice} />}
                />
              )}
              <OverviewRow
                icon='sell'
                label={currentLang === 'ar' ? 'الخصم الأقصى' : 'Max Discount'}
                value={`${viewingDrug.maxDiscount || 0}%`}
              />
              <OverviewRow
                label={currentLang === 'ar' ? 'وحدات / عبوة' : 'Units / Pack'}
                value={viewingDrug.unitsPerPack || 1}
              />
              <OverviewRow
                label={currentLang === 'ar' ? 'حد الطلب' : 'Reorder Level'}
                value={viewingDrug.minStock || 5}
              />
            </div>
          </div>
        </div>
        ...
        <div className='lg:col-span-2 overflow-hidden rounded-xl border border-gray-100 dark:border-gray-700 bg-white dark:bg-gray-900/50'>
          <table className='w-full text-center text-xs'>
            <thead className='border-b border-gray-100 dark:border-gray-700'>
              <tr>
                <th className='px-4 py-2 font-black text-gray-400 uppercase tracking-wider'>
                  {currentLang === 'ar' ? 'تاريخ الانتهاء' : 'Expiry Date'}
                </th>
                <th className='px-4 py-2 font-black text-gray-400 uppercase tracking-wider'>
                  {currentLang === 'ar' ? 'الكمية الإجمالية' : 'Total Qty'}
                </th>
              </tr>
            </thead>
                ...
                .map(([expiryDate, rawStock], idx) => {
                  ...
                  return (
                    ...
                    >
                      ...
                      <td className='px-4 py-2'>
                        <span
                          className={`font-black tabular-nums ${totalStock < (viewingDrug.minStock || 5) * (viewingDrug.unitsPerPack || 1) ? 'text-red-500' : 'text-emerald-600 dark:text-emerald-400'}`}
                        >
                          {formatDrugQty(totalStock, viewingDrug.unitsPerPack || 1, currentLang)}
                        </span>
                      </td>
        ...
        <div className='flex flex-wrap gap-2'>
          {substitutes.length > 0 ? (
            ...
            <span className='text-xs text-gray-400 italic'>
              {currentLang === 'ar'
                ? 'لا يوجد بدائل مسجلة بنفس الاسم العلمي'
                : 'No substitutes found with same generic name.'}
            </span>
          )}
        </div>
      </section>
      ...
      <div className='p-4 rounded-2xl bg-white dark:bg-gray-900/50 border border-gray-100 dark:border-gray-700'>
        ...
        <p className='text-sm text-gray-600 dark:text-gray-300 leading-relaxed font-medium'>
          {viewingDrug.description ||
            ...
            'No detailed description available for this clinical record.'}
        </p>
      </div>
    </div>
  );
