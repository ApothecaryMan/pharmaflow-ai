...
export const ReceiptDesigner: React.FC<ReceiptDesignerProps> = ({ color: _color, t, language }) => {
  ...
  useEffect(() => {
    ...
    const html = generateInvoiceHTML(activeDummySale, {
      ...options,
      language,
        ...
        <div className='flex items-center gap-3 w-full lg:w-auto'>
          <h1 className='text-lg font-bold text-gray-800 dark:text-white whitespace-nowrap shrink-0'>
            {t.receiptDesigner?.title ||
              (language === 'AR' ? 'تصميم الفواتير' : 'Receipt Designer')}
          </h1>
        </div>
            ...
            {isAddingTemplate || isEditingName ? (
              <div className='flex items-center gap-2 w-full '>
                <SmartInput
                  ...
                  placeholder={
                    isEditingName
                      ? t.receiptDesigner.renameTemplate
                      : t.receiptDesigner.newTemplatePlaceholder
                  }
              ...
              <>
                <div className='relative h-10 w-full sm:w-[240px] md:w-[280px] shrink-0 flex items-center gap-1.5'>
                  ...
                  <div className='flex items-center bg-gray-100 dark:bg-muted/50 rounded-xl p-1 shrink-0 h-10 border border-gray-200 dark:border-border'>
                    <Tooltip
                      content={t.receiptDesigner.renameTemplate || 'Rename'}
                      position='bottom'
                    >
                      <button
                        onClick={() => {
                    ...
                    {templates.find((t) => t.id === activeTemplateId) &&
                      !templates.find((t) => t.id === activeTemplateId)?.isDefault && (
                        <Tooltip
                          content={t.receiptDesigner.setDefault || 'Set Default'}
                          position='bottom'
                        >
                          <button
                    ...
                    {templates.length > 1 && (
                      <Tooltip
                        content={t.receiptDesigner.deleteTemplate || 'Delete'}
                        position='bottom'
                      >
                        <button
                ...
                <Tooltip
                  content={t.receiptDesigner.gallery?.title || 'Templates Market'}
                  position='bottom'
                >
                  <button
                  ...
                  >
                    ...
                    <span className='hidden xl:inline text-[13px] font-bold'>
                      {t.receiptDesigner.gallery?.title || 'Market'}
                    </span>
                  </button>
                </Tooltip>
              </>
      ...
      <div className='flex flex-col lg:flex-row flex-1 gap-6 min-h-0'>
            ...
            <div className='grid grid-cols-5 gap-2'>
              {RECEIPT_TEMPLATES.map((t) => {
                ...
                const translatedName =
                  language === 'AR'
                    ? t.name
                        .replace('Standard (Default)', 'الأساسي (افتراضي)')
                ...
                {options.logoBase64 && !options.logoSvgCode ? (
                  ...
                  >
                    ...
                    <div className='absolute top-1 end-1 z-10'>
                      <Tooltip
                        content={language === 'AR' ? 'مسح اللوجو' : 'Remove Logo'}
                        position='top'
                      >
                        <button
                          onClick={() => {
        ...
        <div className='flex-1 rounded-2xl border border-gray-200 dark:border-gray-800 bg-[radial-gradient(#cbd5e1_1px,transparent_1px)] bg-size-[20px_20px] dark:bg-[radial-gradient(#1e293b_1px,transparent_1px)] bg-gray-100 dark:bg-gray-950 relative flex flex-col overflow-hidden'>
          ...
          <div className='absolute top-4 start-4 flex items-center px-2.5 h-7 bg-white/80 dark:bg-muted/80 backdrop-blur-md rounded-xl border border-gray-200/50 dark:border-border/50 shadow-xs text-[10px] font-bold gap-3 z-20 pointer-events-none'>
            <Tooltip
              content={
                t.receiptDesigner.preview?.memorySize || 'Real Printing Memory Size (Actual)'
              }
              position='bottom'
            >
            ...
            <Tooltip
              content={t.receiptDesigner.preview?.payloadSize || 'Raw Data Transfer Size (Payload)'}
              position='bottom'
            >
              <div className='flex items-center gap-1.2 opacity-80'>
