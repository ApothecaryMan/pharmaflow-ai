...
export const StatusBadge: React.FC<StatusBadgeProps> = ({
  ...
  const getStatusConfig = (statusKey: string): { color: string; icon: string; label: string } => {
    switch (statusKey) {
      // Stock Status
      case 'NORMAL':
        return { color: 'emerald', icon: 'check_circle', label: isAr ? 'طبيعي' : 'Normal' };
      case 'LOW':
        return { color: 'amber', icon: 'warning', label: isAr ? 'منخفض' : 'Low' };
      case 'CRITICAL':
        return { color: 'red', icon: 'error', label: isAr ? 'حرج' : 'Critical' };
      case 'OUT_OF_STOCK':
        return { color: 'red', icon: 'cancel', label: isAr ? 'نافذ' : 'Out of Stock' };
      case 'OVERSTOCK':
        return { color: 'purple', icon: 'inventory_2', label: isAr ? 'فائض' : 'Overstock' };
      ...
      // Risk Category
      case 'HIGH':
        return { color: 'red', icon: 'priority_high', label: isAr ? 'مخاطرة عالية' : 'High Risk' };
      case 'MEDIUM':
        return {
          color: 'amber',
          icon: 'running_with_errors',
          label: isAr ? 'مخاطرة متوسطة' : 'Medium Risk',
        };
      case 'CRITICAL_RISK':
        return { color: 'red', icon: 'dangerous', label: isAr ? 'مخاطرة حرجة' : 'Critical Risk' };
      case 'LOW_RISK':
        return {
          color: 'emerald',
          icon: 'verified_user',
          label: isAr ? 'مخاطرة قليلة' : 'Low Risk',
        };

      // Data Quality
      case 'GOOD':
        return { color: 'emerald', icon: 'verified', label: isAr ? 'جيدة' : 'Good' };
      case 'SPARSE':
