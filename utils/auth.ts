 ...
 * Source: Minimal custom implementation for broad compatibility
 */
function sha256_fallback(ascii: string): string {
    ...
    // Constants and initial hash values (same as standard SHA-256)
    ...
    ascii += '\x80'; // Append '1' bit (plus 7 '0' bits)
    while (ascii[lengthProperty] % 64 - 56) ascii += '\x00'; // Padding
    while (ascii[lengthProperty] % 64) ascii += '\x00';
    
    for (i = 0; i < ascii[lengthProperty]; i++) {
        j = ascii.charCodeAt(i);
        if (j >> 8) return ''; // ASCII check: only support strictly ASCII characters
        words[i >> 2] |= j << ((3 - i) % 4) * 8;
    }
    ...
    for (j = 0; j < words[lengthProperty];) {
        let w = words.slice(j, j += 16); // The message is expanded into 64 words
        let oldHash = hash;
        ...
        for (i = 0; i < 64; i++) {
            ...
            hash = [(temp1 + temp2) | 0].concat(hash); // We don't rearrange them all, just shift
...
export async function hashPassword(password: string): Promise<string> {
  ...
  // Fallback for non-secure contexts (e.g. LAN IP HTTP)
  try {
 ...
 * @param password The plain text input password
 * @param storedHash The stored hex hash
